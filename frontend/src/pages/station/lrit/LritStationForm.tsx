...
export const LritStationForm: React.FC<LritStationFormProps> = ({
  open = true,
  editId,
  initialData,
  mode = 'create',
  ...
  const [record, setRecord] = useState<LritStationItem | null>(null);
  ...
  const canApproveL2 = (hasPerm('coastalstationlrit:approvec2') || hasPerm('coastalstationlrit:approve') || hasPerm('specialstation:approvec2') || hasPerm('specialstation:approve') || hasPerm('data:approvec2') || hasPerm('data:approve') || isAdmin || isCucLevel);
  ...
  useEffect(() => {
    if (!open) return;
    const provided = (externalSymbols && externalSymbols.length > 0) ? externalSymbols : (symbolOptions && symbolOptions.length > 0 ? symbolOptions : null);
  ...
  }, [open, externalSymbols, symbolOptions]);
  ...
  useEffect(() => {
    ...
    if (recId) {
      setLoading(true);
      lritStationService.getById(recId).then((res) => {
        populateForm(res);
      }).catch(() => {
        toast.error('Không thể tải thông tin Đài thông tin LRIT');
      }).finally(() => {
        setLoading(false);
      });
    }
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    ...
    if (record?.id) {
      try {
        await lritStationService.deleteAttachment(record.id, attachmentId);
        setAttachments((prev) => prev.filter((a) => a.id !== attachmentId));
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      let savedRecord: LritStationItem;
      ...
      if (isCreateMode) {
        ...
        if (savedRecord?.id && pendingFiles.length > 0) {
          for (const file of pendingFiles) {
            try {
              await lritStationService.uploadAttachment(savedRecord.id, file);
            } catch { /* ignore */ }
          }
        }
      } else if (record?.id) {
      ...
      toast.error(err?.response?.data?.message || err?.message || 'Có lỗi xảy ra khi lưu Đài thông tin LRIT');
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <>
            {isCreateMode ? (
              ...
              <>
                ...
                {record?.approvalStatus && !['DRAFT', 'NHAP', 'REJECTED_LEVEL1', 'REJECTED_LEVEL2'].includes(String(record.approvalStatus).toUpperCase()) && !canApproveL2 && (
                  <Button
                    type="primary"
                    onClick={() => { actionTypeRef.current = 'draft'; setActionType('draft'); form.submit(); }}
                    loading={isSubmitting && actionType === 'draft'}
                      ...
                      <div style={sectionBoxStyle}>
                        ...
                        <Row gutter={[24, 0]}>
                          <Col span={12}>
                            <Form.Item
                              name="code"
                              label={<span style={{ color: sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd }}>Mã đài thông tin LRIT</span>}
                          ...
                          <Col span={12}>
                            <Form.Item
                              name="name"
                              label={<span style={{ color: sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd }}>Tên đài thông tin LRIT</span>}
                              rules={[
                            ...
                            >
                              <Input
                                placeholder="Nhập tên đài thông tin LRIT"
                                maxLength={255}
                                showCount
                                autoFocus
...
export default LritStationForm;
