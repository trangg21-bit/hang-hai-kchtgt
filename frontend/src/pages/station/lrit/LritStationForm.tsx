/* eslint-disable @typescript-eslint/no-explicit-any, react-hooks/immutability, react-hooks/set-state-in-effect, react-hooks/exhaustive-deps -- Ant Design Form/Table callback values are dynamic; the drawer deliberately resets and hydrates local state when its record changes. */
...
const renderServicesBadges = (services?: string[] | string) => {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
      {list.map((srv) => {
        const found = LRIT_SERVICE_OPTIONS.find((o) => o.value === srv);
...
const SAMPLE_OPERATION_PLANS: OperationPlanItem[] = [
  { id: '1', planCode: 'KH-VH-2026/01', planName: 'Kế hoạch trực vận hành trạm thu phát LRIT quý I/2026', startDate: '2026-01-01', endDate: '2026-03-31' },
  { id: '2', planCode: 'KH-VH-2026/02', planName: 'Kế hoạch trực ca liên lạc định kỳ hệ thống vệ tinh LRIT quý II/2026', startDate: '2026-04-01', endDate: '2026-06-30' },
];

const SAMPLE_MAINTENANCE_PLANS: MaintenancePlanItem[] = [
  { id: '1', planCode: 'KH-BT-2026/01', planName: 'Bảo dưỡng định kỳ khối xử lý tín hiệu và ăng-ten LRIT', startTime: '2026-02-15', endTime: '2026-02-18' },
  { id: '2', planCode: 'KH-BT-2026/02', planName: 'Kiểm định chất lượng kênh truyền dữ liệu LRIT qua vệ tinh', startTime: '2026-05-10', endTime: '2026-05-12' },
];

const SAMPLE_INCIDENTS: IncidentItem[] = [
  { id: '1', incidentCode: 'SC-2026-001', incidentType: 'Mất tín hiệu kết nối máy chủ trung tâm tạm thời', location: 'Phòng máy chủ trạm LRIT', incidentTime: '2026-01-20 04:15:00' },
];

export default function LritStationForm({
  open = true,
  editId,
  initialData,
  mode = 'create',
  ...
  onClose,
  onSuccess,
}: LritStationFormProps) {
  const [form] = Form.useForm();
  const [tabKey, setTabKey] = useState('general');
  const [loading, setLoading] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [record, setRecord] = useState<LritStationItem | null>(initialData || null);
  const actionTypeRef = useRef<'draft' | 'submit' | 'approve' | 'update'>('draft');
  const [submittingAction, setSubmittingAction] = useState<'draft' | 'submit' | 'approve' | 'update'>('draft');
  ...
  useEffect(() => {
    if (symbolOptions && symbolOptions.length > 0) {
      queueMicrotask(() => setSymbols(symbolOptions));
      return;
    }
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      form.setFieldsValue({
        ...
        dataFormat: 'LRIT Standard Format',
        communicationChannel: 'Inmarsat-C / Satellite',
        services: undefined,
      ...
      lritStationService.generateCode().then((res) => {
        ...
        form.setFieldValue('code', 'LRIT-0001');
      });
    }
  }, [open, editId, initialData]);

  const populateFormData = (data: LritStationItem) => {
  ...
  const handleFormFinish = async (values: any) => {
    ...
    try {
      ...
      const payload: CreateLritStationRequest = {
        ...
        // spatial object cũ thay vì giữ lại tọa độ lỗi thời.
        coordinates: wkt,
      ...
      if (isCreateMode) {
        const created = await lritStationService.create(payload, 'DRAFT');
        resultId = created.id;

        if (resultId && pendingFiles.length > 0) {
          try {
            await Promise.all(pendingFiles.map((f) => lritStationService.uploadAttachment(resultId!, f)));
          } catch {
            toast.error('Lỗi khi tải tệp đính kèm');
          }
        }
      } else if (resultId) {
        await lritStationService.update(resultId, payload);
        if (pendingDeletedAttachments.length > 0) {
          try {
            await Promise.all(pendingDeletedAttachments.map((a) => lritStationService.deleteAttachment(resultId!, a.id)));
          } catch (delErr) {
      ...
      if (resultId) {
        if (act === 'submit') {
          await lritStationService.submit(resultId);
          toast.success(isCreateMode ? 'Tạo mới và gửi phê duyệt thành công' : 'Lưu và gửi phê duyệt thành công');
        } else if (act === 'approve') {
          await lritStationService.submit(resultId).catch(() => {});
          await lritStationService.approveL2(resultId);
      ...
      toast.error(err?.response?.data?.message || 'Có lỗi xảy ra khi lưu đài LRIT');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDownloadAttachment = async (attId: string, fileName?: string) => {
    ...
    if (!targetId) return;
    try {
      await lritStationService.downloadAttachment(targetId, attId, fileName);
    } catch {
  ...
  const renderDms = (i: number, field: 'lat' | 'lng', r: { latitude: number | null; longitude: number | null }) => {
    const v = field === 'lat' ? r.latitude : r.longitude;
    const dms = ddToDms(v);
      ...
      title={
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={drawerTitleStyle}>
            {isCreateMode && 'Thêm mới Đài thông tin LRIT'}
            {isEditMode && (record?.name ? `Chỉnh sửa — ${record.name}` : 'Chỉnh sửa Đài thông tin LRIT')}
            {isDetailMode && (record?.name ? `Xem chi tiết — ${record.name}` : 'Xem chi tiết Đài thông tin LRIT')}
          </span>
          <Button
            type="text"
            onClick={onClose}
        ...
        isDetailMode ? null : (
          <div style={drawerFooterStyle}>
            {isCreateMode ? (
              <>
                <Button
                  onClick={() => { actionTypeRef.current = 'draft'; setSubmittingAction('draft'); form.submit(); }}
                ...
                <Button
                  type="primary"
                  onClick={() => { actionTypeRef.current = 'submit'; setSubmittingAction('submit'); form.submit(); }}
                  loading={isSubmitting && submittingAction === 'submit'}
                ...
                {canApproveL2 && (
                  <Button
                    type="primary"
                    onClick={() => { actionTypeRef.current = 'approve'; setSubmittingAction('approve'); form.submit(); }}
              ...
              <>
                ...
                <Button
                  type="primary"
                  onClick={() => { actionTypeRef.current = 'update'; setSubmittingAction('update'); form.submit(); }}
