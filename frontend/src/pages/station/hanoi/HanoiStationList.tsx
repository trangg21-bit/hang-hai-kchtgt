...
export const HanoiStationList: React.FC = () => {
  ...
  const canCreate = hasPerm('coastalstationhaiphong:create') || hasPerm('specialstation:create') || hasPerm('data:create') || (user as any)?.role === 'SUPER_ADMIN' || (user as any)?.role === 'ADMIN';
  ...
  const getRowActions = (record: HanoiStationItem) => {
    ...
    const canApproveC1 = hasPerm('coastalstationhaiphong:approvec1') || hasPerm('specialstation:approvec1') || isDepartmentLevel;
    const canApproveC2 = hasPerm('coastalstationhaiphong:approvec2') || hasPerm('coastalstationhaiphong:approve') || hasPerm('specialstation:approvec2') || hasPerm('specialstation:approve') || isDepartmentLevel;
    ...
    if (canEditApprovalRecord(record.approvalStatus, { hasPerm, resource: 'coastalstationhaiphong', extraApprovePerms: ['specialstation:approvec2', 'specialstation:approve'] })) {
      actions.push({
        key: 'edit',
        label: 'Chỉnh sửa',
        icon: <EditOutlined />,
    ...
    if (canDeleteApprovalRecord(record.approvalStatus, { hasPerm, resource: 'coastalstationhaiphong', extraDeletePerms: ['specialstation:delete'] })) {
      actions.push({
        key: 'delete',
        label: 'Xóa bỏ',
        icon: <DeleteOutlined />,
