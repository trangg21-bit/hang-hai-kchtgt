...
export default function BuoyStationList() {
  ...
  const rowActions = useCallback((r: BuoyStationResponse) => {
    ...
    if ((hasPerm('admin:manage') || hasPerm('data:read')) && (r.status === 'DRAFT' || r.status === 'REJECTED')) a.push({ key: 'del', label: 'Xóa', icon: <DeleteOutlined />, onClick: () => openDelete(r), danger: true });
    return a;
  ...
  const renderDetail = () => {
    ...
    return (<div>
      ...
      {!collapsed.gen && (<Descriptions column={2} size="small" bordered labelStyle={{ width: 160 }}><Descriptions.Item label="Mã">{r.code}</Descriptions.Item><Descriptions.Item label="Tên">{r.name}</Descriptions.Item><Descriptions.Item label="Loại">{BUOY_TYPE_OPTIONS.find(o => o.value === r.type)?.label || r.type || '—'}</Descriptions.Item><Descriptions.Item label="Đơn vị QL">{r.unitId ? (orgMap.get(r.unitId) || r.unitId) : '—'}</Descriptions.Item><Descriptions.Item label="Mô tả" span={2}>{r.description || '—'}</Descriptions.Item></Descriptions>)}
      <Divider orientation="left" style={{ fontSize: fontSizeMd, fontWeight: fontWeightBold, color: colors.sidebarBg, cursor: 'pointer', marginTop: spaceMd }} onClick={() => toggleSec('tech')}>{collapsed.tech ? '▶' : '▼'} Thông tin kỹ thuật</Divider>
      {!collapsed.tech && (<Descriptions column={2} size="small" bordered labelStyle={{ width: 160 }}><Descriptions.Item label="Kinh độ">{r.longitude != null ? r.longitude.toFixed(6) : '—'}</Descriptions.Item><Descriptions.Item label="Vĩ độ">{r.latitude != null ? r.latitude.toFixed(6) : '—'}</Descriptions.Item><Descriptions.Item label="Tầm xa">{r.range != null ? `${r.range} HL` : '—'}</Descriptions.Item><Descriptions.Item label="Màu sắc">{r.color ? (COLOR_MAP[r.color] || r.color) : '—'}</Descriptions.Item><Descriptions.Item label="Hình dạng">{r.shape ? (SHAPE_MAP[r.shape] || r.shape) : '—'}</Descriptions.Item><Descriptions.Item label="Đặc tính AS">{r.lightCharacteristic ? (LIGHT_MAP[r.lightCharacteristic] || r.lightCharacteristic) : '—'}</Descriptions.Item><Descriptions.Item label="Hoạt động"><Tag color={r.isActive ? 'green' : 'default'}>{r.isActive ? 'Có' : 'Ngừng'}</Tag></Descriptions.Item><Descriptions.Item label="KT gần nhất">{fmt(r.lastInspectionDate)}</Descriptions.Item><Descriptions.Item label="KT kế tiếp">{fmt(r.nextInspectionDate)}</Descriptions.Item></Descriptions>)}
