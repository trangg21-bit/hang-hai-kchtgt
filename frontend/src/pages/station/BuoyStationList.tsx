...
export default function BuoyStationList() {
  ...
  const [editingItem, setEditingItem] = useState<BuoyStationResponse | null>(null);
    ...
    if (id && (action === 'detail' || action === 'edit')) {
      (async () => {
        try {
          ...
          const data = cached || await fetchBuoyStationById(id);
  ...
  const handleOpenModal = (record?: BuoyStationResponse) => {
    if (record) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload: CreateBuoyStationRequest = {
      ...
      if (editingItem) {
        await updateBuoyStation(editingItem.id, payload);
        message.success('Cập nhật nhà trạm phao tiêu thành công!');
      } else {
        await createBuoyStation(payload);
  ...
  const columns = [
    ...
    },
    {
      title: 'Hành động',
      key: 'action',
      render: (_: any, record: BuoyStationResponse) => (
        <Space size="middle">
