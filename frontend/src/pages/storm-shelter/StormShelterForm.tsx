...
import {
  ...
  Button, Space, DatePicker, Table, Modal, type InputNumberProps,
} from 'antd';
import type { UploadFile } from 'antd';
import {
  PlusOutlined, DeleteOutlined, EditOutlined, EnvironmentOutlined,
  BankOutlined, SlidersOutlined, FileTextOutlined, CompassOutlined,
  DownOutlined, RightOutlined,
} from '@ant-design/icons';
import {
  colors, DRAWER_TABLE_SCROLL_Y,
  textTertiary, textPrimary, textSecondary, borderDefault, actionPrimary, statusCritical,
...
import { GEOMETRY_POINT_COUNT, parseWktToCoordinates, validateDmsCoordinates } from '../../utils/gisGeometry';
...
// Style cho thẻ phân nhóm (Section Card) chuẩn CHK VTS
  ...
  const inputs = [
    {
      ...
      onEdit: (v: number | null) => onChange(v ?? null, mVal ?? null, sVal ?? null),
    },
    {
      ...
      onEdit: (v: number | null) => onChange(dVal ?? null, v ?? null, sVal ?? null),
    },
    {
      ...
      onEdit: (v: number | null) => onChange(dVal ?? null, mVal ?? null, v ?? null),
    },
  ] as const;
        ...
        {inputs.map((inp) => (
          <div key={inp.key} style={{ display: 'flex', flex: inp.basis, minWidth: 0, width: inp.width }}>
            <InputNumber
              ...
              onChange={(raw) => inp.onEdit(raw ?? null)}
...
/** Nhóm 3 ô nhập Độ/Phút/Giây dùng cho điểm neo trong Drawer khu nước */
...
export default forwardRef(function StormShelterForm({ form, id, onFinish, onSubmittingChange }: StormShelterFormProps, ref) {
  ...
  useEffect(() => {
    ...
    organizationService.list({ pageSize: 1000 }).then(r => setOrgUnits(r.data || [])).catch(() => {});
    lineObjectService.list({ status: 'PUBLISHED', objectType: LineObject.ObjectType.WATERWAY, pageSize: 1000 }).then(r => setWaterwayOptions((r.data || []).map((l: any) => ({ value: l.id, label: l.name || l.code })))).catch(() => {});
    buoyBerthCRUD.search({ page: 1, pageSize: 1000, approvalStatus: 'APPROVED' }).then(r => setBuoyStationOptions((r.data || []).map((s: any) => ({ value: s.id, label: s.buoyBerthName || s.buoyBerthCode || s.id })))).catch(() => {});
    userService.list({ pageSize: 1000 }).then(r => {
      const m = new Map<string, string>();
      (r.data || []).forEach((u: any) => m.set(u.id, u.fullName || u.username));
      setUserMap(m);
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const ec = data.coordinates ? parseWktToCoordinates(data.coordinates) : [];
        setCoordinateList(ec.length > 0 ? ec.map(c => {
          const latDms = ddToDms(c.latitude);
          const lngDms = ddToDms(c.longitude);
          return { latD: latDms.d, latM: latDms.m, latS: latDms.s, lngD: lngDms.d, lngM: lngDms.m, lngS: lngDms.s };
        }) : data.latitude != null ? (() => {
          const latDms = ddToDms(Number(data.latitude));
          const lngDms = ddToDms(Number(data.longitude));
          return [{ latD: latDms.d, latM: latDms.m, latS: latDms.s, lngD: lngDms.d, lngM: lngDms.m, lngS: lngDms.s }];
        })() : []);
  ...
  const handleSave = useCallback(async (saveAction: SaveAction) => {
    ...
    try {
      ...
      if (errFields.some((f) => f.name[0] === 'orgUnitId' || f.name[0] === 'portId' || f.name[0] === 'stormShelterName')) {
        setActiveTabKey('general');
      } else if (errFields.some((f) => f.name[0] === 'mapSymbolId' || f.name[0] === 'coordinateSystem' || f.name[0] === 'displayRule' || f.name[0] === 'geometryType')) {
    ...
    const manualCoords = validCoords;
    const coordinates = (() => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        stormShelterCode: String(values.stormShelterCode || '').trim() || undefined,
        stormShelterName: String(values.stormShelterName || '').trim(),
        ...
        shapeDescription: values.shapeDescription || undefined,
        ...
        publicDecision: values.publicDecision || undefined,
      ...
      if (isEdit && id) {
        ...
        createdStormShelterId = id;
      } else {
        const res: any = await stormShelterCRUD.create(payload as any);
        createdStormShelterId = res?.id ?? res?.data?.id;
      }
  ...
  const tabItems = [
    // Tab 1: Thông tin chung (chuẩn Section Cards VTS CHK)
    {
      key: 'general',
        ...
        <div style={drawerFormScrollStyle}>
          ...
          <div style={sectionBoxStyle}>
            ...
            <Row gutter={[24, 0]}>
              ...
              <Col span={12}>
                <Form.Item name="stormShelterName" {...labelProps('Tên khu tránh, trú bão')} required style={{ marginBottom: spaceFormField }}
          ...
          <div style={sectionBoxStyle}>
            <div
              onClick={() => setIndicatorOpen((prev) => !prev)}
              style={{ ...sectionHeaderStyle, cursor: 'pointer', marginBottom: indicatorOpen ? spaceFormField : 0, borderBottom: indicatorOpen ? '1px solid #f1f5f9' : 'none' }}
            >
              <span style={sectionTitleStyle}>
                <SlidersOutlined style={{ color: actionPrimary }} />
                Thông số kỹ thuật & Năng lực khai thác
              </span>
              <span style={{ color: textSecondary, fontSize: fontSizeSm, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                ...
                <Row gutter={[24, 0]}>
                  <Col span={12}>
                    <Form.Item name="bottomElevationDesign" {...labelProps('Cao độ đáy bến thiết kế')} style={{ marginBottom: spaceFormField }}>
                      <NumberInputWithCount step={0.01} placeholder="0" maxLength={20} style={numberInputStyle} formatter={fmtInputNumber} />
                    </Form.Item>
                  </Col>
                  <Col span={12}>
                    <Form.Item name="activeStormShelterCount" {...labelProps('Số lượng khu tránh, trú bão đang khai thác')} style={{ marginBottom: spaceFormField }}>
                      <NumberInputWithCount min={0} placeholder="0" maxLength={5} style={numberInputStyle} />
                    </Form.Item>
                  </Col>
                </Row>
                <Row gutter={[24, 0]}>
          ...
          <div style={sectionBoxStyle}>
            <div
              ...
              style={{ ...sectionHeaderStyle, cursor: 'pointer', marginBottom: announcementOpen ? spaceFormField : 0, borderBottom: announcementOpen ? '1px solid #f1f5f9' : 'none' }}
            >
              <span style={sectionTitleStyle}>
              ...
              <Row gutter={[24, 0]}>
                <Col span={12}>
                  <Form.Item name="openingAnnouncementDate" {...labelProps('Thời điểm công bố mở, đưa vào sử dụng')} style={{ marginBottom: spaceFormField }}>
                    <DatePicker {...getDatePickerProps({ placeholder: 'Chọn thời điểm...' })} format="DD/MM/YYYY" style={{ width: '100%', borderRadius: radiusPill, height: 40 }} />
                  </Form.Item>
                </Col>
                <Col span={24}>
          ...
          <div style={sectionBoxStyle}>
            <div
              onClick={() => setMooringScopeOpen((prev) => !prev)}
              style={{
                ...sectionHeaderStyle,
                cursor: 'pointer',
                userSelect: 'none',
              ...
              <div>
                ...
                {waterAreaList.length === 0 ? (
                  <div style={{ padding: '32px 16px', textAlign: 'center', border: `1px dashed ${borderDefault}`, borderRadius: radiusMd, background: surfaceCard }}>
                    <span style={{ fontSize: fontSizeMd, color: textTertiary, display: 'block' }}>Chưa có khu nước neo buộc tàu nào.</span>
                  </div>
                ) : (
                  <DetailTable
                    size="small"
                    dataSource={waterAreaList.map((w, i) => ({ ...w, key: i, _idx: i }))}
                    rowKey={(r: any) => r._idx}
                    emptyText="Chưa có khu nước neo buộc tàu"
                    columns={[
                      {
                        title: 'STT',
                        width: 60,
                      ...
                      },
                      {
                        ...
                        render: (_: any, record: any) => (
                          <a
                            ...
                            onClick={() => openEditWaterArea(record._idx)}
                          >
                            {record.description || ''}
                          </a>
                        ),
                      },
                      {
                        ...
                        key: 'geometryType',
                        width: 140,
                        render: (v: any) => (v === 'POINT' ? 'Điểm' : v === 'LINE' ? 'Đường' : v === 'POLYGON' ? 'Vùng' : ''),
                      },
                      {
                        title: 'Điểm neo',
                        key: 'anchorPoints',
                        width: 120,
                        align: 'center' as const,
                        render: (pts: any[]) => `${(pts || []).length} điểm`,
                      },
                      {
                        title: 'Thao tác',
                        key: 'actions',
                        ...
                        render: (_: any, record: any) => (
                          <Space size={4}>
                            <Button type="text" icon={<EditOutlined style={{ color: actionPrimary }} />} onClick={() => openEditWaterArea(record._idx)} title="Chỉnh sửa" />
                            <Button type="text" danger icon={<DeleteOutlined />} onClick={() => removeWaterArea(record._idx)} title="Xóa" />
                          </Space>
                        ),
                      },
                    ]}
      ...
      children: (
        <InfrastructureAttachmentTab
          ...
          onDownload={async (uid, name) => {
            try {
              if (isEdit && id) {
                const res = await api.get(`/v1/storm-shelter/${id}/attachments/${uid}/download`, { responseType: 'blob' });
  ...
  return (
    <>
      ...
      {/* Drawer Thông tin khu nước neo buộc tàu (chuẩn AppDrawer) */}
      <AppDrawer
        width={780}
        title={editingWaterAreaIndex == null ? 'Thêm mới thông tin khu nước neo buộc tàu' : 'Chỉnh sửa thông tin khu nước neo buộc tàu'}
        open={waterAreaDrawerOpen}
        onClose={closeWaterAreaDrawer}
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            <Button style={{ ...outlineButtonStyle, borderRadius: radiusPill }} onClick={closeWaterAreaDrawer}>Hủy</Button>
            <Button type="primary" onClick={saveWaterArea} loading={waterAreaSaving} style={{ ...primaryButtonStyle, borderRadius: radiusPill }}>Lưu</Button>
          </div>
        }
      >
        <Form layout="vertical">
          ...
          <div style={{ marginBottom: spaceFormField, marginTop: 2 }}>
            <span style={{ color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd }}>Vị trí cụ thể điểm neo</span>
          </div>
          ...
          <div style={{ marginBottom: spaceFormField, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={sectionLabelStyle}>Tọa độ điểm neo ({waterAreaAnchorPoints.length})</span>
            ...
            <div style={{ width: '100%', overflowX: 'hidden' }}>
              <Table size="small" dataSource={waterAreaAnchorPoints.map((p, i) => ({ ...p, key: i, _idx: i }))} pagination={false} tableLayout="fixed" locale={{ emptyText: 'Chưa có điểm neo' }}>
                ...
                <Table.Column title="Thao tác" key="actions" width={80} align="center"
                  render={(_: any, record: any) => <Button type="text" danger size="small" icon={<DeleteOutlined />} onClick={() => removeAnchorPoint(record._idx)} />}
