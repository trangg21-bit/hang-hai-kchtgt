            ...
            <div style={{ paddingTop: 3 }}>
              ...
              <div className="chk-detail-grid">
                {[
                  ...
                  ['Số lượng khu tránh, trú bão đang khai thác', r.activeStormShelterCount || '—'],
                  ['Số lượng khu tránh, trú bão đã công bố', r.publishedStormShelterCount || '—'],
                  ['Số lượng khu tránh, trú bão đang được thỏa thuận đầu tư xây dựng', r.underInvestmentStormShelterCount || '—'],
                ].map(([label, value], i) => (
                  <div key={i} className="chk-detail-row">
                    <span className="chk-detail-label">{label}</span>
                    <span className="chk-detail-value">{value}</span>
              ...
              <div style={{ marginTop: spaceSm, padding: '0 12px' }}>
                ...
                <DetailTable
                  ...
                  columns={[
                    ...
                    { title: 'Vĩ độ (N)', key: 'lat', align: 'center' as const, render: (_v: any, rec: any) => { if (rec.latitude == null) return '—'; const d = ddToDms(rec.latitude); return `${d.d}° ${d.m}' ${d.s}" N`; } },
                    { title: 'Kinh độ (E)', key: 'lng', align: 'center' as const, render: (_v: any, rec: any) => { if (rec.longitude == null) return '—'; const d = ddToDms(rec.longitude); return `${d.d}° ${d.m}' ${d.s}" E`; } },
                  ]}
                />
              </div>
            </div>
