...
export default function BerthList() {
  ...
  const filterFields = useMemo(() => [
    ...
    { key: 'portId', type: 'select' as const, label: 'Cảng biển', placeholder: 'Chọn cảng biển', options: cangBienOptions },
  ], [cangBienOptions]);

  const headerActions = useMemo(() => [
    { key: 'create', label: 'Tạo bến cảng', variant: 'primary' as const, icon: <PlusOutlined />, onClick: () => navigate('/Berth/create') },
  ...
  const rowActions = useCallback((record: Berth) => {
    ...
    actions.push({ key: 'view', label: 'Xem chi tiết', icon: <EyeOutlined />, onClick: () => navigate(`/Berth/${record.id}`) });
    actions.push({ key: 'edit', label: 'Sửa', icon: <EditOutlined />, onClick: () => navigate(`/Berth/${record.id}/edit`) });
