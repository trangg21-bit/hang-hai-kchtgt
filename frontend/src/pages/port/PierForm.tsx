...
const PierForm = forwardRef<any, PierFormProps>(({ form, id, onFinish }, ref) => {
  ...
  const handleSave = useCallback(async (saveAction: SaveAction) => {
    ...
    if (vals.geometryType) {
      if (!vals.mapSymbolId) { toast.error('Biểu tượng bản đồ là bắt buộc khi chọn loại đối tượng'); return; }
  ...
  return (
    <Tabs defaultActiveKey="general" tabBarStyle={{ marginBottom: 0, paddingTop: 0, position: 'sticky', top: 0, zIndex: 1, background: surfaceCard }} items={[
