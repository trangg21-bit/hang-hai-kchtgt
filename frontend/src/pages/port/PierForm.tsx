...
const PierForm = forwardRef<any, PierFormProps>(({ form, id, onFinish }, ref) => {
  ...
  useEffect(() => { if (!watchedPortId) { setBerthOptions([]); return; } (async () => { setLoadingBerths(true); try { const r = await berthCRUD.search({ portId: watchedPortId, operationalStatus: 'OPERATIONAL', approvalStatus: 'APPROVED', page: 1, pageSize: 1000 }); setBerthOptions((r.data || []).map((b: any) => ({ value: b.id, label: b.berthName }))); } catch {} finally { setLoadingBerths(false); } })(); }, [watchedPortId]);
  ...
  return (
    <Tabs activeKey={activeTabKey} onChange={setActiveTabKey} tabBarStyle={{ marginBottom: 0, paddingTop: 0, position: 'sticky', top: 0, zIndex: 1, background: surfaceCard }} items={[
      ...
      <Row gutter={16}><Col span={12}><Form.Item name="orgUnitId" {...labelProps('Đơn vị quản lý')} required rules={[{ required: true, message: 'Đơn vị quản lý là bắt buộc' }]} style={{ marginBottom: spaceFormField }}><Select placeholder="Chọn đơn vị quản lý" loading={loadingOrgs} disabled={isEdit} options={orgUnitOptions} showSearch optionFilterProp="label" onChange={handleOrgUnitChange} style={selectStyle} /></Form.Item></Col><Col span={12}><Form.Item name="portId" {...labelProps('Thuộc cảng biển')} required rules={[{ required: true, message: 'Cảng biển là bắt buộc' }]} style={{ marginBottom: spaceFormField }}><Select placeholder={!watchedOrgUnitId ? 'Vui lòng chọn đơn vị quản lý trước' : portOptions.length === 0 && !loadingPorts ? 'Không có cảng biển thuộc đơn vị quản lý' : 'Chọn cảng biển...'} loading={loadingPorts} disabled={!watchedOrgUnitId || (portOptions.length === 0 && !loadingPorts)} options={portOptions} showSearch optionFilterProp="label" notFoundContent="Không có cảng biển thuộc đơn vị quản lý" onChange={handlePortChange} style={selectStyle} /></Form.Item></Col></Row>
      ...
      <Row gutter={16}><Col span={12}><Form.Item name="pierType" {...labelProps('Loại cầu')} style={{ marginBottom: spaceFormField }}><Select placeholder="Chọn loại cầu" allowClear options={[{ value: 'CONTAINER', label: 'Container' }, { value: 'TONG_HOP', label: 'Tổng hợp' }, { value: 'HANH_KHACH', label: 'Hành khách' }, { value: 'CHUYEN_DUNG_XANG_DAU', label: 'Chuyên dùng xăng dầu' }, { value: 'CHUYEN_DUNG_ROI_QUANG', label: 'Chuyên dùng rời/quặng' }, { value: 'KHAC', label: 'Khác' }]} style={selectStyle} /></Form.Item></Col><Col span={12}><Form.Item name="designLoad" {...labelProps('Tải trọng thiết kế (T/m²)')} style={{ marginBottom: spaceFormField }}><InputNumber min={0} max={999} step={0.01} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="operationalFunction" {...labelProps('Công năng khai thác')} style={{ marginBottom: spaceFormField }}><Input placeholder="Nhập công năng khai thác" maxLength={255} style={inputStyle} /></Form.Item></Col><Col span={12}><Form.Item name="length" {...labelProps('Chiều dài (m)')} style={{ marginBottom: spaceFormField }}><InputNumber min={0.01} max={500} step={0.01} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="width" {...labelProps('Chiều rộng (m)')} style={{ marginBottom: spaceFormField }}><InputNumber min={0.01} max={500} step={0.01} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col><Col span={12}><Form.Item name="currentWaterDepth" {...labelProps('Độ sâu khu nước hiện tại (theo TBHH gần nhất) (m)')} style={{ marginBottom: spaceFormField }}><InputNumber min={0} max={100} step={0.1} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="designBedElevation" {...labelProps('Cao độ đáy bến thiết kế')} style={{ marginBottom: spaceFormField }}><InputNumber min={0} max={100} step={0.1} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col><Col span={12}><Form.Item name="publishedVesselDWT" {...labelProps('Cỡ tàu khai thác theo công bố (DWT)')} style={{ marginBottom: spaceFormField }}><InputNumber min={0} max={999999} placeholder="0" style={numberStyle} /></Form.Item></Col></Row>
      ...
      <Row gutter={16}><Col span={12}><Form.Item name="cargoThroughput" {...labelProps('Sản lượng hàng thông qua')} style={{ marginBottom: spaceFormField }}><InputNumber min={0} max={999999999} step={0.01} placeholder="0" style={numberStyle} formatter={fmtInputNumber} /></Form.Item></Col><Col span={12}><Form.Item name="operationalStatus" {...labelProps('Tình trạng')} style={{ marginBottom: spaceFormField }}><Select placeholder="Chọn tình trạng" options={OPERATIONAL_STATUS_OPTIONS} style={selectStyle} /></Form.Item></Col></Row>
