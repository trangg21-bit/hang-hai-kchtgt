...
export default function DryPortList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsError(false);
    try {
      const res = await dryPortCRUD.search({
        page,
        pageSize,
        dryPortCode: search || undefined,
        dryPortName: search || undefined,
        province: filterTinh,
  ...
  const handleFilterSearch = useCallback((values: Record<string, any>) => {
    ...
    setFilterTinh(values.province || undefined);
  ...
  const handleDelete = useCallback(
    async (record: DryPort) => {
      try {
        await dryPortCRUD.delete(record.id);
  ...
  const handleSubmitApproval = useCallback(
    async (record: DryPort) => {
      try {
        await dryPortApproval.submitForApproval?.(record.id);
  ...
  const handleApproveL1 = useCallback(
    async (record: DryPort) => {
      try {
        await dryPortApproval.approveL1?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleApproveL2 = useCallback(
    async (record: DryPort) => {
      try {
        await dryPortApproval.approveL2?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleReject = useCallback(
    async (record: DryPort) => {
      ...
      try {
        await dryPortApproval.reject(record.id, reason, localStorage.getItem('user_id') || '1');
  ...
  const filterFields = useMemo(() => [
    ...
    { key: 'province', type: 'select' as const, label: 'Tỉnh/thành phố', placeholder: 'Chọn tỉnh/thành', options: DryPort_TINH_OPTIONS },
  ], []);

  const headerActions = useMemo(() => [
    { key: 'create', label: 'Tạo cảng căn cứ', variant: 'primary' as const, icon: <PlusOutlined />, onClick: () => navigate('/DryPort/create') },
  ...
  const rowActions = useCallback((record: DryPort) => {
    ...
    actions.push({ key: 'view', label: 'Xem chi tiết', icon: <EyeOutlined />, onClick: () => navigate(`/DryPort/${record.id}`) });
    actions.push({ key: 'edit', label: 'Sửa', icon: <EditOutlined />, onClick: () => navigate(`/DryPort/${record.id}`) });
