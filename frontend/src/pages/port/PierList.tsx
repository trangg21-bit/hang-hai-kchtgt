...
export default function PierList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await pierCRUD.search({
        page,
        pageSize,
        pierCode: search || undefined,
        pierName: search || undefined,
        loaiCau: filterLoai,
  ...
  const handleDelete = useCallback(
    async (record: Pier) => {
      try {
        await pierCRUD.delete(record.id);
  ...
  const handleSubmitApproval = useCallback(
    async (record: Pier) => {
      try {
        await pierApproval.submitForApproval?.(record.id);
  ...
  const handleApproveL1 = useCallback(
    async (record: Pier) => {
      try {
        await pierApproval.approveL1?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleApproveL2 = useCallback(
    async (record: Pier) => {
      try {
        await pierApproval.approveL2?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleReject = useCallback(
    async (record: Pier) => {
      ...
      try {
        await pierApproval.reject(record.id, reason, localStorage.getItem('user_id') || '1');
  ...
  const filterFields = useMemo(() => [
    ...
    { key: 'loaiCau', type: 'select' as const, label: 'Loại cầu', placeholder: 'Chọn loại', options: Pier_LOAI_OPTIONS },
  ], []);

  const headerActions = useMemo(() => [
    { key: 'create', label: 'Tạo cầu cảng', variant: 'primary' as const, icon: <PlusOutlined />, onClick: () => navigate('/Pier/create') },
  ...
  const columns = useMemo(() => [
    ...
    { key: 'length', label: 'Chiều dài', dataIndex: 'length', width: 110, render: (v: number) => <span style={{ color: textSecondary }}>{v?.toFixed(1) || '—'}</span> },
    { key: 'taiTrong', label: 'Tải trọng', dataIndex: 'taiTrong', width: 120, render: (v: number) => <span style={{ color: textSecondary }}>{v?.toFixed(1) || '—'}</span> },
    {
      ...
      render: (loaiCau: string) => {
        const m = Pier_LOAI_MAP[loaiCau as keyof typeof Pier_LOAI_MAP];
        return m ? <Tag color={m.color}>{Pier_LOAI_OPTIONS.find((o) => o.value === loaiCau)?.label || loaiCau}</Tag> : <Tag>{loaiCau}</Tag>;
      },
    },
    {
  ...
  const rowActions = useCallback((record: Pier) => {
    ...
    actions.push({ key: 'view', label: 'Xem chi tiết', icon: <EyeOutlined />, onClick: () => navigate(`/Pier/${record.id}`) });
    actions.push({ key: 'edit', label: 'Sửa', icon: <EditOutlined />, onClick: () => navigate(`/Pier/${record.id}`) });
