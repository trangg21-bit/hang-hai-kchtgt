...
export function useVtsAssistHistory({ orgName, transmissionMap }: UseVtsAssistHistoryOptions) {
  ...
  const openHistory = useCallback(async (r: VtsAssistAsset) => {
    setTarget(r); setOpen(true); setLoading(true); setRecords([]); setFilters({ keyword: '' });
    try {
      const d = (await fetchVtsAssistAssetHistory(r.id)) as
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((r) => {
      ...
      if (q) {
        const label = VTS_ASSIST_ASSET_FIELD_LABELS[fn] || fn;
...
export interface VtsAssistAssetHistoryProps {
  open: boolean;
