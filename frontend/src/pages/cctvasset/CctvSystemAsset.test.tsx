...
import CctvSystemAssetList from './CctvSystemAssetList';
...
vi.mock('../../services/cctvasset/api', () => ({
  fetchCctvSystemAssets: vi.fn().mockResolvedValue({
    content: [],
    totalElements: 0,
    totalPages: 0,
    size: 20,
  ...
  fetchCctvDeviceOptions: vi.fn().mockResolvedValue([]),
}));
...
describe('CctvSystemAsset UI Components', () => {
  it('renders CctvSystemAssetList without crashing and without duplicate imports', () => {
    const html = renderToStaticMarkup(<CctvSystemAssetList />);
    expect(html).toContain('Tài sản HT CCTV');
  });
});
