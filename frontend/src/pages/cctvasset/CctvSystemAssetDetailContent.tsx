...
export interface CctvSystemAssetDetailContentProps {
  open: boolean;
  selectedRecord?: CctvSystemAsset;
  onClose: () => void;
  orgName: Map<string, string>;
  cctvDeviceMap: Map<string, { deviceCode: string; deviceName: string }>;
...
export default function CctvSystemAssetDetailContent({
  open,
  selectedRecord,
  onClose,
  orgName,
  ...
  decreaseRows,
  attachments,
}: CctvSystemAssetDetailContentProps) {
  ...
  useEffect(() => {
    ...
    if (attachments && attachments.length > 0) {
      ...
      initialList = selectedRecord.attachmentName
        ...
        .filter(Boolean)
        .map((name, i) => ({
          id: `cctv-att-${selectedRecord.id}-${i + 1}`,
          fileName: name,
      ...
      initialList = [
        {
          id: 'cctv-sample-att-1',
          fileName: 'Quyet_dinh_dau_tu_he_thong_cctv.pdf',
          fileSize: 2450000,
          uploadedAt: selectedRecord?.createdAt || new Date().toISOString(),
          uploadedByName: selectedRecord?.updatedByName || 'Cán bộ quản lý',
        },
        {
          id: 'cctv-sample-att-2',
          fileName: 'Bien_ban_kiem_dinh_thiet_bi_cctv.pdf',
          fileSize: 1820000,
          uploadedAt: selectedRecord?.createdAt || new Date().toISOString(),
          uploadedByName: selectedRecord?.updatedByName || 'Cán bộ quản lý',
        },
        {
          id: 'cctv-sample-att-3',
          fileName: 'So_do_lap_dat_camera_cctv.png',
          fileSize: 3100000,
          uploadedAt: selectedRecord?.createdAt || new Date().toISOString(),
          uploadedByName: selectedRecord?.updatedByName || 'Cán bộ quản lý',
        },
  ...
  const tabs = useMemo<ViewTabConfig<CctvSystemAsset>[]>(() => {
    if (!selectedRecord) return [];

    const device = selectedRecord.cctvId
      ? cctvDeviceMap.get(selectedRecord.cctvId)
      : undefined;
    const deviceDisplay = device
      ? `${device.deviceCode} - ${device.deviceName}`
      : selectedRecord.cctvCode
      ? `${selectedRecord.cctvCode} - ${selectedRecord.cctvName || ''}`
      : '—';
  ...
  return (
    <DynamicViewSidebar<CctvSystemAsset>
      open={open}
      title={
        <span style={{ color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: 16 }}>
          Chi tiết tài sản HT CCTV — {selectedRecord?.assetName || ''}
        </span>
      }
