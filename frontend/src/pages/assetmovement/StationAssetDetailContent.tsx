...
export interface StationAssetDetailContentProps {
  open: boolean;
  selectedRecord?: StationAsset;
  onClose: () => void;
...
export default function StationAssetDetailContent({
  open,
  selectedRecord: r,
  onClose,
  orgName,
  ...
  increaseRows,
  decreaseRows,
}: StationAssetDetailContentProps) {
  ...
  const viewTabs = useMemo<ViewTabConfig<StationAsset>[]>(() => {
    const approvalInfo = APPROVAL_MAP[r?.approvalStatus || ''] || {
      color: statusDraft,
      label: r?.approvalStatus || 'Lưu tạm',
    };

    const stationValue = (r as unknown as Record<string, unknown>)?.[config.stationFieldName] as string | undefined || r?.stationId;
    const stationItem = stationValue ? stationMap.get(stationValue) : undefined;
    const stationLabelText = stationItem
      ? `${stationItem.name}${stationItem.code ? ` (${stationItem.code})` : ''}`
      : stationValue || '—';
        ...
        sections: [
          {
            ...
            fields: [
              ...
              {
                name: config.stationFieldName as unknown as keyof StationAsset,
                label: config.stationLabel,
                value: () => stationLabelText,
              },
              {
                name: 'assetType',
                label: 'Loại tài sản',
  ...
  return (
    <>
      <DynamicViewSidebar<StationAsset>
        open={open}
        onClose={onClose}
        record={r}
