...
import {
  ...
  createAssetDecrease,
  createAssetIncrease,
  createBuoyBerthAsset,
  createKhaiThac,
  deleteBuoyBerthAsset,
...
import { buoyBerthCRUD } from '../../services/portService';
...
import BuoyBerthAssetOperationForm, {
  type OperationMode,
  type OperationValues,
} from './BuoyBerthAssetOperationForm';
...
export default function BuoyBerthAssetList() {
  const perms = useAssetPermissions(['buoyberth', 'buoyberthasset']);
  const [data, setData] = useState<BuoyBerthAsset[]>([]);
  const [organizations, setOrganizations] = useState<Organization[]>([]);
  const [buoyBerths, setBuoyBerths] = useState<BuoyBerth[]>([]);
  ...
  const [filters, setFilters] = useState<BuoyBerthAssetFilters>({});
  const [draftFilters, setDraftFilters] = useState<BuoyBerthAssetFilters & { updatedRange?: [Dayjs | null, Dayjs | null] }>({});
  const [drawerMode, setDrawerMode] = useState<DrawerMode>();
  const [selected, setSelected] = useState<BuoyBerthAsset>();
  const [deleteTarget, setDeleteTarget] = useState<BuoyBerthAsset>();
  ...
  const buoyBerthMap = useMemo(() => new Map(buoyBerths.map(item => [item.id, item])), [buoyBerths]);
  ...
  const openHistory = useCallback(async (r: BuoyBerthAsset) => {
    setHistoryTarget(r);
    setHistoryOpen(true);
    setHistoryRecords([]);
    setHistorySearch('');
  ...
  const historyUpdateCount = useMemo(() => {
    return countStandardHistoryCards({
      ...
      fieldLabels: BUOY_BERTH_ASSET_FIELD_LABELS,
      ...
      formatValue: (fn, raw) => {
        ...
        if (fn === 'buoyBerthId') {
          const item = buoyBerthMap.get(raw!);
          return item ? `${item.buoyBerthCode} - ${item.buoyBerthName}` : raw;
        }
        if (
          fn === 'originalValue' ||
          fn === 'remainingValue' ||
  ...
  }, [filteredHistoryRecords, historyTarget, orgName, buoyBerthMap]);

  const loadData = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const response = await fetchBuoyBerthAssets({ ...filters, page: page - 1, size: pageSize });
      ...
      const [all, ...statusPages] = await Promise.all([
        fetchBuoyBerthAssets(baseFilters),
        ...STATUS_COUNT_KEYS.map(approvalStatus => fetchBuoyBerthAssets({ ...baseFilters, approvalStatus })),
      ]);
      setStatusCounts({
      ...
      setError(getErrorMessage(cause, 'Không thể tải danh sách tài sản bến phao.'));
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    void Promise.all([organizationService.getAll(), buoyBerthCRUD.findAll({ page: 1, size: 5000 })])
      .then(([orgs, buoyBerthPage]) => {
        setOrganizations(orgs);
        setBuoyBerths(buoyBerthPage.data);
      })
      .catch(() => toast.error('Không thể tải danh mục đơn vị hoặc bến phao.'));
  }, []);

  // ── Approval state & handlers ──────────────────────────────────────
  const [approvingRecord, setApprovingRecord] = useState<BuoyBerthAsset | null>(null);
  const [approveLevel, setApproveLevel] = useState<'c1' | 'c2'>('c1');
  const [approveModalOpen, setApproveModalOpen] = useState(false);
  const [approveLoading, setApproveLoading] = useState(false);

  const [rejectingRecord, setRejectingRecord] = useState<BuoyBerthAsset | null>(null);
  const [rejectLevel, setRejectLevel] = useState<'c1' | 'c2'>('c1');
  const [rejectModalOpen, setRejectModalOpen] = useState(false);
  const [rejectLoading, setRejectLoading] = useState(false);

  const handleOpenApproveModal = useCallback((record: BuoyBerthAsset, level: 'c1' | 'c2') => {
  ...
  const handleOpenRejectModal = useCallback((record: BuoyBerthAsset, level: 'c1' | 'c2') => {
  ...
  const handleSubmitApproval = useCallback(async (record: BuoyBerthAsset) => {
    try {
  ...
  const openEdit = useCallback(async (record: BuoyBerthAsset) => {
  ...
  const saveAsset = async (targetAction: 'DRAFT' | 'PENDING_APPROVAL' | 'APPROVED') => {
    try {
      ...
      toast.success(
        targetAction === 'DRAFT'
          ? 'Đã lưu tạm tài sản bến phao.'
          : targetAction === 'PENDING_APPROVAL'
            ? 'Đã lưu và gửi phê duyệt tài sản bến phao.'
            : 'Đã lưu và phê duyệt tài sản bến phao.',
      );
      setDrawerMode(undefined);
      await loadData();
    } catch (cause: unknown) {
      if (!isValidationError(cause)) toast.error(getErrorMessage(cause, 'Không thể lưu tài sản bến phao.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const tableOptions = useMemo<TableOption<BuoyBerthAsset>>(() => ({
    dataKey: 'id',
    mainColumns: [
      {
      ...
      {
        title: 'MÃ BẾN PHAO',
        dataIndex: 'buoyBerthId',
        type: TableColumnType.Text,
        width: 190,
        allowSort: true,
        valueRef: (r) => r.buoyBerthId ? buoyBerthMap.get(r.buoyBerthId)?.buoyBerthCode : undefined,
      },
      {
        title: 'LOẠI TÀI SẢN',
        dataIndex: 'assetType',
    ...
    <ThemeTokenProvider tokens={customTokens}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 12 }}>
        ...
        <DeleteConfirmModal
          ...
          loading={saving}
          itemType="tài sản bến phao"
          itemName={deleteTarget?.assetName}
          itemCode={deleteTarget?.assetCode}
          onConfirm={() => {
            if (!deleteTarget) return;
            setSaving(true);
            void deleteBuoyBerthAsset(deleteTarget.id)
              .then(() => {
                toast.success('Đã xóa tài sản bến phao.');
