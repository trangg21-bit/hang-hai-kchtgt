...
import BuoyAssetList from './BuoyAssetList';
...
vi.mock('../../services/buoy/api', () => ({
  fetchAllBuoys: vi.fn().mockResolvedValue([]),
}));

vi.mock('../../services/buoy-station/api', () => ({
  fetchBuoyStationList: vi.fn().mockResolvedValue([]),
}));

vi.mock('../../services/assetmovement/api', () => ({
  fetchBuoyAssets: vi.fn().mockResolvedValue({
    content: [],
    totalElements: 0,
    totalPages: 0,
    size: 20,
...
describe('BuoyAsset UI Components', () => {
  it('renders BuoyAssetList without crashing and resolves countHistoryUpdates', () => {
    const html = renderToStaticMarkup(<BuoyAssetList />);
    expect(html).toContain('Tài sản phao, tiêu');
  });
});
