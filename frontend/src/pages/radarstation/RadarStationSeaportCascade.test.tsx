...
describe('RadarStation Seaport Cascade, Layout & Code Generation (/radar-station)', () => {
  const mockOrgUnits = [
  ...
  it('TC-RADAR-01: Disables and returns empty seaports when no Đơn vị quản lý is selected', () => {
  ...
  it('TC-RADAR-02: Returns only seaports belonging to selected Đơn vị quản lý and its descendants', () => {
  ...
  it('TC-RADAR-03: Resets seaport if currently selected seaport does not belong to new Đơn vị quản lý', () => {
  ...
  it('TC-RADAR-04: Keeps seaport if currently selected seaport belongs to new Đơn vị quản lý', () => {
  ...
  it('TC-RADAR-05: Formats seaport placeholder without trailing ellipsis', () => {
  ...
  it('TC-RADAR-06: Formats code placeholder matching /berth standard', () => {
  ...
  it('TC-RADAR-07: Resets code when seaport is cleared in create mode and preserves in edit mode', () => {
    let code: string | undefined = 'RADAR-000030';
    const isEdit = false;
    ...
    let editCode: string | undefined = 'RADAR-000030';
    ...
    expect(editCode).toBe('RADAR-000030');
  });

  it('TC-RADAR-08: Field ordering in Section 1 follows /berth standard: Row 1 (orgUnitId + seaportId), Row 2 (code + stationName)', () => {
    const row1Fields = ['orgUnitId', 'seaportId'];
    const row2Fields = ['code', 'stationName'];

    expect(row1Fields[0]).toBe('orgUnitId');
    expect(row1Fields[1]).toBe('seaportId');
    expect(row2Fields[0]).toBe('code');
    expect(row2Fields[1]).toBe('stationName');
  });
});
