...
export interface NavigationChannelChkFormProps {
  open?: boolean;
  editId?: string | null;
...
export default function NavigationChannelChkForm({ open, editId, mode, onCancel, onSuccess }: NavigationChannelChkFormProps = {}) {
  return (
    <ThemeTokenProvider tokens={themeTokenChk}>
      <NavigationChannelChkFormInner open={open} editId={editId} mode={mode} onCancel={onCancel} onSuccess={onSuccess} />
    </ThemeTokenProvider>
  );
}

function NavigationChannelChkFormInner({ open, editId, mode, onCancel, onSuccess }: NavigationChannelChkFormProps = {}) {
    ...
    try {
      ...
      if (isCreateMode) {
        ...
        if (isModalMode) {
          ...
          navigate('/navigation-channel-chk');
        }
  ...
  const breadcrumbs = [
    ...
    { title: 'Luồng hàng hải CHK', onClick: () => navigate('/navigation-channel-chk') },
        ...
        <Spin spinning={isLoading}>
          {formError ? (
            <Card>
              ...
              <Button onClick={() => (isModalMode ? onCancel?.() : navigate('/navigation-channel-chk'))} style={{ marginTop: spaceLg, ...outlineButtonStyle }}>
                Quay lại
              </Button>
            </Card>
          ) : record ? (
    ...
    <Form form={form} layout="vertical" onFinish={handleSubmitForm} style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: spaceSm, marginBottom: spaceMd }}>
        ...
        <Button
          onClick={isIframe
            ? () => window.parent.postMessage({ type: 'CLOSE_KCHT_MODAL' }, '*')
            : isModalMode ? onCancel : () => navigate('/navigation-channel-chk')}
      ...
      <Modal
        ...
        title={
          <span style={{ color: textPrimary, fontWeight: fontWeightBold, fontSize: fontSizeLg }}>
            {isCreateMode ? 'Tạo mới Luồng hàng hải CHK' : isEditMode ? 'Chỉnh sửa Luồng hàng hải CHK' : 'Chi tiết Luồng hàng hải CHK'}
          </span>
        }
      >
        {formContent}
