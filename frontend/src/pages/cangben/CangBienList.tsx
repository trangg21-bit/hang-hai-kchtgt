...
export default function CangBienList() {
  ...
  const headerActions = useMemo(() => [
    { key: 'create', label: 'Tạo cảng biển', variant: 'primary' as const, icon: <PlusOutlined />, onClick: () => navigate('/cangbien/create') },
  ...
  const columns = useMemo(() => [
    { key: 'stt', label: 'STT', width: 60, type: 'mono' as const, align: 'center' as const, render: (_: unknown, __: CangBien, idx: number) => <span style={{ fontSize: fontSizeMd }}>{(page - 1) * pageSize + idx + 1}</span> },
    { key: 'maCang', label: 'Mã', dataIndex: 'maCang', width: 140, render: (maCang: string) => <Tag color="cyan">{maCang}</Tag> },
    { key: 'tenCang', label: 'Tên', dataIndex: 'tenCang', ellipsis: true },
    { key: 'tinhThanhPho', label: 'Tỉnh/thành phố', dataIndex: 'tinhThanhPho', ellipsis: true },
    ...
    { key: 'dienTich', label: 'Diện tích', dataIndex: 'dienTich', width: 110, render: (v: number) => <span style={{ color: textSecondary }}>{v?.toFixed(2) || '—'}</span> },
    {
  ...
  const rowActions = useCallback((record: CangBien) => {
    const actions: { key: string; label: string; icon?: React.ReactNode; onClick: () => void; danger?: boolean }[] = [];
    actions.push({ key: 'view', label: 'Xem chi tiết', icon: <EyeOutlined />, onClick: () => navigate(`/cangbien/${record.id}`) });
    actions.push({ key: 'edit', label: 'Sửa', icon: <EditOutlined />, onClick: () => navigate(`/cangbien/${record.id}`) });
