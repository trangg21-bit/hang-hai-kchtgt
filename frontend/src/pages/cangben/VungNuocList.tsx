...
import {
  vungNuocCRUD as vungnuocCRUD,
  vungNuocApproval as vungnuocApproval,
...
export default function VungNuocList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await vungnuocCRUD.search({
        page,
        pageSize,
        maVung: filterMa || undefined,
        tenVung: filterTen || undefined,
        loaiVung: filterLoai,
        status: filterStatus,
      });
  ...
  const handleApproveL1 = useCallback(
    async (record: VungNuoc) => {
      ...
      try {
        await vungnuocApproval.approveL1(record.id, approverId);
  ...
  const handleApproveL2 = useCallback(
    async (record: VungNuoc) => {
      const approverId = localStorage.getItem('user_id') || '1';
      try {
        await vungnuocApproval.approveL2(record.id, approverId);
  ...
  const handleReject = useCallback(
    async (record: VungNuoc) => {
      ...
      try {
        await vungnuocApproval.reject(record.id, reason, approverId);
  ...
  const columns = [
    { title: '#', width: 60, render: (_: unknown, __: VungNuoc, idx: number) => (page - 1) * pageSize + idx + 1 },
    {
      title: 'Mã',
        ...
        <Space size="small">
          ...
          {record.status === 'DRAFT' && (
            <Tooltip title="Gửi duyệt">
              <Popconfirm
                title="Gửi duyệt vùng nước?"
                description="Sau khi gửi, vùng nước sẽ chuyển sang trạng thái chờ phê duyệt cấp 1."
                okText="Gửi"
                cancelText="Hủy"
            ...
            <>
              <Tooltip title="Phê duyệt cấp 1">
                <Popconfirm
                  ...
                  description="Sau khi phê duyệt, vùng nước sẽ chuyển sang trạng thái chờ phê duyệt cấp 2."
                  okText="Phê duyệt"
                  cancelText="Hủy"
            ...
            <>
              <Tooltip title="Phê duyệt cấp 2">
                <Popconfirm
                  ...
                  description="Sau khi phê duyệt, vùng nước sẽ được công bố chính thức."
                  okText="Phê duyệt"
                  cancelText="Hủy"
          ...
          <Col xs={24} md={16}>
            <Space wrap>
              ...
              <Select
                ...
                options={VUNGNUOOC_LOAI_OPTIONS}
              />
              <Select
        ...
        {!isLoading && !isError && dataSource.length > 0 && (
          <DataTable<VungNuoc>
            ...
            pagination={{
              ...
              showTotal: (t: number) => `Tổng ${t} vùng nước`,
