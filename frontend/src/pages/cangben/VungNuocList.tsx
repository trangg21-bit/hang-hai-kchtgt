...
import type { VungNuoc } from '../../types/cangben';
import {
  BECBANG_STATUS_MAP,
...
export default function VungNuocList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsError(false);
    try {
      const res = await vungNuocCRUD.search({
        page,
        pageSize,
        maVungNuoc: search || undefined,
        tenVungNuoc: search || undefined,
  ...
  const handleDelete = useCallback(
    async (record: VungNuoc) => {
      try {
        await vungNuocCRUD.delete(record.id);
  ...
  const handleSubmitApproval = useCallback(
    async (record: VungNuoc) => {
      try {
        await vungNuocApproval.submitForApproval?.(record.id);
  ...
  const handleApproveL1 = useCallback(
    async (record: VungNuoc) => {
      try {
        await vungNuocApproval.approveL1?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleApproveL2 = useCallback(
    async (record: VungNuoc) => {
      try {
        await vungNuocApproval.approveL2?.(record.id, localStorage.getItem('user_id') || '1');
  ...
  const handleReject = useCallback(
    async (record: VungNuoc) => {
      ...
      try {
        await vungNuocApproval.reject(record.id, reason, localStorage.getItem('user_id') || '1');
  ...
  const headerActions = useMemo(() => [
    { key: 'create', label: 'Tạo vùng nước', variant: 'primary' as const, icon: <PlusOutlined />, onClick: () => navigate('/vungnuoc/create') },
  ...
  const columns = useMemo(() => [
    { key: 'stt', label: 'STT', width: 60, type: 'mono' as const, align: 'center' as const, render: (_: unknown, __: VungNuoc, idx: number) => <span style={{ fontSize: fontSizeMd }}>{(page - 1) * pageSize + idx + 1}</span> },
    { key: 'maVungNuoc', label: 'Mã', dataIndex: 'maVungNuoc', width: 140, render: (maVungNuoc: string) => <Tag color="cyan">{maVungNuoc}</Tag> },
    { key: 'tenVungNuoc', label: 'Tên', dataIndex: 'tenVungNuoc', ellipsis: true },
  ...
  const rowActions = useCallback((record: VungNuoc) => {
    const actions: { key: string; label: string; icon?: React.ReactNode; onClick: () => void; danger?: boolean }[] = [];
    actions.push({ key: 'view', label: 'Xem chi tiết', icon: <EyeOutlined />, onClick: () => navigate(`/vungnuoc/${record.id}`) });
    actions.push({ key: 'edit', label: 'Sửa', icon: <EditOutlined />, onClick: () => navigate(`/vungnuoc/${record.id}`) });
