...
import LineObjectForm, { type LineObjectFormRef } from './LineObjectForm';
import LineObjectDetailContent from './LineObjectDetailContent';
...
export default function LineObjectList() {
  ...
  const createFormRef = useRef<LineObjectFormRef>(null);
  const editFormRef = useRef<LineObjectFormRef>(null);
  ...
  const fetchCounts = useCallback(async () => {
    try {
      const [resAll, resActive, resLocked, resDeleted] = await Promise.all([
        ...
        spatialObjectCategoryService.list({ page: 1, pageSize: 1, geometryType: 2, status: 1, isDeleted: false }),
        spatialObjectCategoryService.list({ page: 1, pageSize: 1, geometryType: 2, status: 0, isDeleted: false }),
        spatialObjectCategoryService.list({ page: 1, pageSize: 1, geometryType: 2, isDeleted: true }),
      ]);
    ...
    try {
      ...
      if (record.updatedAt && record.updatedAt !== record.createdAt) {
        entries.push({
          ...
          description: `Cập nhật thông tin danh mục đối tượng đường "${record.name}"`,
          changes: [
      ...
      if (record.createdAt) {
        entries.push({
          ...
          description: `Khởi tạo danh mục đối tượng đường "${record.name}"`,
          changes: [
            { field: 'code', oldValue: null, newValue: record.code },
            { field: 'name', oldValue: null, newValue: record.name },
            { field: 'geometryType', oldValue: null, newValue: 'Đối tượng đường' },
      ...
      if (entries.length === 0) {
        entries.push({
          ...
          description: `Thông tin danh mục đối tượng đường "${record.name}"`,
          changes: [
            { field: 'code', oldValue: '—', newValue: record.code },
            { field: 'name', oldValue: '—', newValue: record.name },
            { field: 'geometryType', oldValue: '—', newValue: 'Đối tượng đường' },
            { field: 'iconId', oldValue: '—', newValue: sym ? `${sym.name} (${sym.code})` : record.iconId || '(Không có)' },
            { field: 'status', oldValue: '—', newValue: record.status === 1 ? 'Sử dụng' : 'Khóa' },
          ],
        });
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Đã xóa danh mục đối tượng đường thành công');
    ...
    <ThemeTokenProvider tokens={themeTokenChk}>
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100% - 32px)' }}>
        ...
        <AppDrawer
          ...
          title={
            <span style={{ ...drawerTitleStyle, fontSize: 16 }}>
              Chỉnh sửa danh mục đối tượng đường — {editingRecord?.name || ''}
            </span>
          }
          open={editDrawerOpen}
          destroyOnHidden
        ...
        <CommonHistoryDrawer
          ...
          entityName={historyTarget?.name || 'đối tượng đường'}
