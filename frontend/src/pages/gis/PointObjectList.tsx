    ...
    try {
      ...
      if (record.createdAt) {
        entries.push({
          ...
          changes: [
            ...
            { field: 'geometryType', oldValue: null, newValue: 'Đối tượng điểm' },
      ...
      if (entries.length === 0) {
        entries.push({
          ...
          description: `Thông tin danh mục đối tượng điểm "${record.name}"`,
          changes: [
            { field: 'code', oldValue: '—', newValue: record.code },
            { field: 'name', oldValue: '—', newValue: record.name },
            { field: 'geometryType', oldValue: '—', newValue: 'Đối tượng điểm' },
            { field: 'iconId', oldValue: '—', newValue: sym ? `${sym.name} (${sym.code})` : record.iconId || '(Không có)' },
            { field: 'status', oldValue: '—', newValue: record.status === 1 ? 'Sử dụng' : 'Khóa' },
          ],
        });
