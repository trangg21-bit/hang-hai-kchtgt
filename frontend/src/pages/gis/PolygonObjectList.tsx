...
export default function PolygonObjectList() {
  ...
  return (
    <div style={{ padding: spaceLg }}>
      <ScreenHeader
        title="Quản lý danh mục đối tượng vùng"
        breadcrumbs={[
      ...
      {isLoading ? (
        ...
        <EmptyState description="Chưa có danh mục đối tượng vùng nào" />
      ) : (
        <>
          <DataTable
            columns={columns}
      ...
      <Modal
        title={<span style={{ color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeLg }}>{editingRecord ? 'Chỉnh sửa đối tượng vùng' : 'Thêm mới đối tượng vùng'}</span>}
        open={isModalOpen}
        ...
        <Form form={form} layout="vertical" style={MODAL_FORM_STYLE}>
          <Row gutter={spaceMd}>
            <Col span={12}>
              ...
              >
                <Input placeholder="VD: BEN_CANG" style={INPUT_STYLE} disabled={!!editingRecord} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
                ...
                style={{ marginBottom: spaceFormField }}
              >
                <Input placeholder="VD: Bến cảng, cầu cảng" style={INPUT_STYLE} />
              </Form.Item>
            </Col>
          </Row>
