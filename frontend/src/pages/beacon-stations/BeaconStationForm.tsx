...
import type { CreateBeaconStationRequest, UpdateBeaconStationRequest } from '../../types/beacon';
import {
...
export default function BeaconStationForm() {
    ...
    if (isEdit) {
      (async () => {
        try {
          ...
          const data = cached || await beaconStationCRUD.findById(id!);
          ...
          form.setFieldsValue({
            ...
            towerColor: data.towerColor,
            location: data.location,
            unitId: data.unitId,
          });
        } catch {
          toast.error('Không thể tải thông tin đèn biển');
          navigate('/beacon-stations');
        }
      })();
    }
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      if (isEdit) {
        const payload: UpdateBeaconStationRequest = {
          name: values.name,
          type: values.type,
        ...
        const res = await beaconStationCRUD.update(id!, payload);
        if (window.parent && (window.parent as any).kchtDetailCache) {
          (window.parent as any).kchtDetailCache[id!] = res;
        }
        toast.success('Đã cập nhật đèn biển');
      } else {
        const payload: CreateBeaconStationRequest = {
          name: values.name,
          code: values.code,
          type: values.type,
          longitude: values.longitude,
        ...
        await beaconStationCRUD.create(payload);
  ...
  const handleDelete = useCallback(async () => {
    ...
    if (!confirmed) return;
    try {
      await beaconStationCRUD.delete(id);
  ...
  return (
    <>
      <Card style={{ marginBottom: 16 }}>
        <Space>
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/beacon-stations')}>
            Quay lại
          </Button>
        ...
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item style={{ marginTop: 24 }}>
            <Space>
              ...
              <Button onClick={() => navigate('/beacon-stations')} style={{ borderRadius: radiusPill, height: 40, fontSize: fontSizeMd, borderColor: borderDefault, color: textSecondary }}>Hủy</Button>
            </Space>
          </Form.Item>
        </Form>
      </Card>
