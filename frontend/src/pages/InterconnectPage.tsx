...
export default function InterconnectPage() {
  ...
  const transactionColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (val: string) =>
          val || (
            <span style={{ color: textTertiary, fontSize: fontSizeMd }}>&mdash;</span>
          ),
      },
      {
        key: 'sentAt',
  ...
  const renderHistoryDrawer = () => (
    <Drawer
      {...drawerProps}
      title={
        <span style={{ ...drawerTitleStyle, fontSize: fontSizeXl }}>
    ...
    >
      ...
      <DataTable columns={transactionColumns} dataSource={transactionsError ? [] : paginatedTransactions} rowKey="id"
        rowActions={transactionsError ? undefined : transactionRowActions} loading={transactionsLoading} scroll={{ x: 1400 }} />
    ...
    <ThemeTokenProvider tokens={themeTokenChk}>
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100% - 32px)' }}>
        ...
        <Drawer
          ...
          title={
            <span style={{ ...drawerTitleStyle, fontSize: fontSizeXl }}>
              Sửa thông tin kết nối liên thông chia sẻ dữ liệu — {editingConnection?.connectionName}
            </span>
          }
          open={editDrawerOpen}
