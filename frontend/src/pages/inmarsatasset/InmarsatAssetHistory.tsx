...
export const INMARSAT_ASSET_FIELD_LABELS: Record<string, string> = {
  assetCode: 'Mã tài sản',
  assetName: 'Tên tài sản',
  orgUnitId: 'Đơn vị quản lý',
  usingOrgUnitId: 'Đơn vị sử dụng',
  parentOrgUnitId: 'Đơn vị quản lý cấp trên',
  stationId: 'Đài thông tin vệ tinh Inmarsat',
  inmarsatId: 'Đài Inmarsat',
...
const HISTORY_FIELD_ORDER = [
  'assetCode', 'assetName', 'orgUnitId', 'usingOrgUnitId', 'stationId', 'inmarsatId',
...
// Extend histVal to handle stationId / inmarsatId
function histVal(
  fn: string,
  val: unknown,
  orgMap?: Map<string, string>,
  stationMap?: Map<string, { code: string; name: string }>,
): string {
  if (fn === 'stationId' || fn === 'inmarsatId') {
    if (!val || val === '(null)' || val === 'null') return '';
    const v = String(val).trim();
    if (stationMap) {
      const t = stationMap.get(v);
      return t ? (t.code ? `${t.code} - ${t.name}` : t.name) : v;
    }
    return v;
  }
  return baseHistVal(fn, val, orgMap, stationMap);
}

// === Hook ====================================================================

export interface UseInmarsatHistoryOptions {
  orgName: Map<string, string>;
  inmarsatMap: Map<string, { code: string; name: string }>;
}

export function useInmarsatHistory({ orgName, inmarsatMap }: UseInmarsatHistoryOptions) {
  const [open, setOpen] = useState(false);
  const [target, setTarget] = useState<InmarsatAsset | null>(null);
  const [records, setRecords] = useState<Record<string, unknown>[]>([]);
  const [loading, setLoading] = useState(false);
  const [filters, setFilters] = useState<{ keyword: string; fromDate?: string; toDate?: string }>({ keyword: '' });

  const openHistory = useCallback(async (r: InmarsatAsset) => {
    setTarget(r); setOpen(true); setLoading(true); setRecords([]); setFilters({ keyword: '' });
    try {
      const d = (await fetchInmarsatAssetHistory(r.id)) as
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((r) => {
      ...
      if (q) {
        const label = INMARSAT_ASSET_FIELD_LABELS[fn] || fn;
        const rawHits = [fn, label, r.oldValue, r.newValue, r.previousValue, r.value, r.reason, r.note]
          .filter((v) => v !== null && v !== undefined)
          .map((v) => String(v).toLowerCase());
        const ro = histVal(fn, r.oldValue, orgName, inmarsatMap);
        const rn = histVal(fn, r.newValue, orgName, inmarsatMap);
  ...
  }, [records, filters, orgName, inmarsatMap]);
...
export interface InmarsatAssetHistoryProps {
  open: boolean;
  target: InmarsatAsset | null;
...
export default function InmarsatAssetHistory({
  open, target, records, loading, filters, filteredRecords, hasActiveFilter,
  orgName, inmarsatMap, onClose, onFiltersChange,
}: InmarsatAssetHistoryProps) {
