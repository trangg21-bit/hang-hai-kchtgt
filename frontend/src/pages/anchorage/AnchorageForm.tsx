...
const labelProps = (label: string) => ({
  label: (
    <span style={{ color: sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd, lineHeight: '18px' }}>
      {label}
    </span>
  ),
});
...
const AnchorageForm = forwardRef<AnchorageFormHandle, AnchorageFormProps>(({
  ...
  // Accordion section collapse states (mặc định mở sẵn đồng bộ PierForm)
      ...
      if (createdId && uploadedFiles.length > 0) {
        ...
        if (newFiles.length > 0) {
          try {
            ...
            await api.post(`/v1/anchorage/${createdId}/attachments`, fd, {
          ...
          <div style={sectionBoxStyle}>
            ...
            {technicalOpen && (
              <div>
                ...
                <Row gutter={[24, 0]}>
                  <Col span={12}>
                    <Form.Item name="activeAnchorageCount" {...labelProps('Số lượng khu neo đậu đang khai thác')} style={{ marginBottom: spaceFormField }} getValueFromEvent={getValueFromEvent5}>
                      <NumberInputWithCount min={0} step={1} precision={0} maxLength={5} placeholder="0" style={numberStyle} parser={parseNumber5} />
                    </Form.Item>
                  </Col>
                  <Col span={12}>
                    <Form.Item name="publishedAnchorageCount" {...labelProps('Số lượng khu neo đậu đã công bố')} style={{ marginBottom: spaceFormField }} getValueFromEvent={getValueFromEvent5}>
                      <NumberInputWithCount min={0} step={1} precision={0} maxLength={5} placeholder="0" style={numberStyle} parser={parseNumber5} />
                    </Form.Item>
                  </Col>
                </Row>
  ...
  return (
    <>
      ...
      {/* Drawer xem chi tiết khu nước neo buộc tàu (không đẩy Drawer cha) */}
      <Drawer
        {...drawerProps}
        rootClassName="anchorage-drawer-scope"
        className="anchorage-drawer-scope"
