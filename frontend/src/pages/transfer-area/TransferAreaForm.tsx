  ...
  const inputs = [
    {
      ...
      radius: '999px 0 0 999px', unit: '°', unitStyle: dmsUnitStyle, basis: '1 0 70px', width: 70,
    ...
    },
    {
      ...
      radius: '0', unit: '"', unitStyle: dmsUnitEndStyle, basis: '1.2 0 84px', width: 84,
...
const TransferAreaForm = forwardRef<TransferAreaFormHandle, TransferAreaFormProps>(({
  ...
  // Normalize and match symbol after symbols list loads
  useEffect(() => {
    if (symbols.length > 0) {
      const currentSym = form.getFieldValue('mapSymbolId');
      if (currentSym) {
        const found = symbols.find(s => s.id === currentSym || s.id.toLowerCase() === String(currentSym).toLowerCase());
        if (found && found.id !== currentSym) {
          form.setFieldsValue({ mapSymbolId: found.id });
        }
      }
    }
  }, [symbols, form]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rawSymbolId = d.mapSymbolId || (d as any).bieuTuongId || (d as any).symbolId;
        ...
        form.setFieldsValue({
          ...
          mapSymbolId: rawSymbolId || undefined,
    ...
    try {
      ...
      if (wasApproved) {
        if (saveAction === 'SUBMIT') {
          ...
          targetApprovalStatus = (isCuc && hasPerm('transfer-area:approvec2')) ? 'APPROVED' : 'APPROVED_LEVEL1';
        }
      } else if (saveAction === 'APPROVED') {
        targetApprovalStatus = (isCuc && hasPerm('transfer-area:approvec2')) ? 'APPROVED' : 'APPROVED_LEVEL1';
      }
      ...
      if (createdId && pendingDeletedAttachmentIds.length > 0) {
        for (const attId of pendingDeletedAttachmentIds) {
          await api.delete(`/v1/transfer-area/${createdId}/attachments/${attId}`, {
          ...
          <div style={sectionBoxStyle}>
            ...
            {technicalOpen && (
              <div>
                ...
                <Row gutter={[24, 0]}>
                  <Col span={12}>
                    <Form.Item name="activeTransferCount" {...labelProps('Số lượng khu chuyển tải đang khai thác')} style={{ marginBottom: spaceFormField }} getValueFromEvent={getValueFromEvent5}>
                      <NumberInputWithCount min={0} step={1} precision={0} maxLength={5} placeholder="0" style={numberStyle} parser={parseNumber5} />
                    </Form.Item>
                  </Col>
                  <Col span={12}>
                    <Form.Item name="publishedTransferCount" {...labelProps('Số lượng khu chuyển tải đã công bố')} style={{ marginBottom: spaceFormField }} getValueFromEvent={getValueFromEvent5}>
                      <NumberInputWithCount min={0} step={1} precision={0} maxLength={5} placeholder="0" style={numberStyle} parser={parseNumber5} />
                    </Form.Item>
                  </Col>
                </Row>
