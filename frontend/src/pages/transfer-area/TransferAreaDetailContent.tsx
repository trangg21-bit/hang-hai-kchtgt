...
import {
  ...
  drawerTitleStyle, drawerCloseBtnStyle, statusBadgeStyle, outlineButtonStyle, primaryButtonStyle,
} from '../../themetokenchk';
...
export default function TransferAreaDetailContent({
  ...
  const fmtDms = (dd: number | null | undefined, axis: 'N' | 'E'): string => {
    if (dd == null || isNaN(Number(dd))) return '—';
    const d = ddToDms(Number(dd));
    return `${d.d}° ${d.m}' ${d.s}" ${axis}`;
  };
  ...
  return (
      ...
      items={[
        {
          ...
          children: (
            <div style={{ paddingTop: 3, overflowY: 'auto', maxHeight: 'calc(100vh - 290px)' }}>
              ...
              {/* ── Toggle: Thông tin thời gian hoạt động ── */}
              ...
              {/* ── Toggle: Thông tin khu nước neo buộc tàu ── */}
              ...
              {waterAreaOpen && (
                <div style={{ marginTop: 4 }}>
                  ...
                  <DetailTable
                    ...
                    rowKey={(rec: any) => rec.key ?? rec.id ?? rec.description}
                    columns={[
                      { title: 'STT', width: 50 },
                ...
                {(() => {
                  ...
                  return (
                    <DetailTable
                      ...
                      columns={[
                        { title: 'STT', width: 50 },
                        { title: 'Vĩ độ (Latitude - N)', key: 'lat', render: (_v: any, rec: any) => fmtDms(rec.lat, 'N') },
                        { title: 'Kinh độ (Longitude - E)', key: 'lng', render: (_v: any, rec: any) => fmtDms(rec.lng, 'E') },
                      ]}
                    />
                  );
                })()}
        ...
        },
        {
          ...
          children: (
            <div style={{ paddingTop: 3 }}>
              ...
              <DetailTable
                dataSource={(Array.isArray(detailFiles) ? detailFiles : []).map((f) => ({ ...f }))}
                emptyText="Chưa có tài liệu đính kèm"
                rowKey={(rec: any) => rec.id || rec.fileName || rec.uid}
                columns={[
                  { title: 'STT', width: 50 },
    ...
    {/* GIS Location Selector Modal — xem vị trí trên bản đồ chuyên dụng (chuẩn VTS CHK, disabled = chế độ XEM) */}
    <Modal
      title={
            ...
            <div style={{ paddingTop: 3 }}>
              ...
              <div style={{ marginTop: spaceSm }}>
                ...
                <DetailTable
                  ...
                  rowKey={(rec: any) => rec.key ?? rec.name ?? rec.id}
                  columns={[
                    { title: 'STT', width: 50 },
                    { title: 'Tên điểm neo', dataIndex: 'name', key: 'name', render: (name?: string) => <span style={{ fontSize: fontSizeMd, color: textPrimary, fontWeight: fontWeightBold, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }} title={name || '—'}>{name || '—'}</span> },
                    { title: 'Vĩ độ (N)', key: 'lat', render: (_: any, rec: any) => fmtDms(rec.latitude, 'N') },
                    { title: 'Kinh độ (E)', key: 'lng', render: (_: any, rec: any) => fmtDms(rec.longitude, 'E') },
                  ]}
                />
              </div>
            </div>
