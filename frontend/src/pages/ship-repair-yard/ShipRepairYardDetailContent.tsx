    ...
    <Tabs defaultActiveKey="general" tabBarStyle={{ marginBottom: 0, paddingTop: 0, position: 'sticky', top: 0, zIndex: 1, background: surfaceCard }}
      items={[
            ...
            <div style={{ paddingTop: 3 }}>
              ...
              <div className="detail-grid">
                {[
                  ...
                  ['Mã cơ sở sửa chữa, đóng tàu', <span style={{ display: 'inline-flex', padding: '2px 10px', borderRadius: 999, fontSize: fontSizeMd, fontWeight: fontWeightMedium, background: `${actionPrimary}15`, color: actionPrimary }}>{r.shipRepairYardCode || '—'}</span>],
              ...
              <div className="detail-grid">
                {[
                  ...
                  <div key={i} className="detail-row" style={label === 'Ghi chú' ? { gridColumn: '1 / -1' } : undefined}>
                    <span className="detail-label">{label}</span>
                    <span className="detail-value">{value}</span>
                  </div>
                ))}
        ...
        },
        {
          ...
          children: (
            <div style={{ paddingTop: 3 }}>
              <span style={{ ...detailLabelStyle, marginBottom: spaceSm, display: 'inline-block' }}>Danh sách file đính kèm</span>
              <PagedTable dataSource={detailFiles.map((f) => ({ ...f }))}
                emptyText={<div style={{ padding: '32px 0', textAlign: 'center' }}><div style={{ fontSize: 48, color: textTertiary, marginBottom: 12 }}><FileOutlined /></div><span style={{ color: textTertiary, fontSize: fontSizeLg }}>Chưa có file đính kèm</span></div>}>
                <Table.Column title="Tên file" key="name" dataIndex="fileName" align="center"
                  render={(v: string, rec: any) => (
                    <span style={{ fontSize: fontSizeMd, color: actionPrimary }}>
                      <a href={`/api/v1/ship-repair-yard/${r.id}/attachments/${rec.id}/download`} onClick={(e) => e.preventDefault()} style={{ color: actionPrimary }} title={v}>
                        <FileOutlined style={{ marginRight: spaceSm }} />{v || '—'}
                      </a>
                    </span>
                  )}
                  onHeaderCell={() => ({ style: { background: colors.bodyBg, color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd, textTransform: 'uppercase' as const, padding: '12px 12px' } })} />
                <Table.Column title="Kích thước" key="size" dataIndex="fileSize" align="center"
                  render={(v: number) => <span style={{ fontSize: fontSizeMd, color: textPrimary }}>{v != null ? `${(v / 1024).toFixed(1)} KB` : '—'}</span>}
                  onHeaderCell={() => ({ style: { background: colors.bodyBg, color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd, textTransform: 'uppercase' as const, padding: '12px 12px' } })} />
                <Table.Column title="Ngày tải lên" key="uploadedAt" dataIndex="uploadedAt" align="center"
                  render={(v: string) => <span style={{ fontSize: fontSizeMd, color: textPrimary }}>{fmtDateTime(v)}</span>}
                  onHeaderCell={() => ({ style: { background: colors.bodyBg, color: colors.sidebarBg, fontWeight: fontWeightBold, fontSize: fontSizeMd, textTransform: 'uppercase' as const, padding: '12px 12px' } })} />
              </PagedTable>
            </div>
          ),
