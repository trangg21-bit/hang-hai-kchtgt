...
import type { DryPort, CreateCangCanPayload, UpdateCangCanPayload } from './types';
...
): Promise<PaginatedResult<DryPort>> {
...
export async function fetchCangCanById(id: string): Promise<DryPort> {
...
export async function createCangCan(payload: CreateCangCanPayload): Promise<DryPort> {
...
export async function updateCangCan(payload: UpdateCangCanPayload): Promise<DryPort> {
...
export async function rejectCangCan(id: string, reason: string): Promise<void> {
  ...
  await api.post(`/v1/dry-ports/${id}/reject?${sp}`);
}

// ── History ─────────────────────────────────────────────────────────────

export async function fetchdryPortHistory(id: string): Promise<dryPortHistoryRecord[]> {
  const res = await api.get(`/v1/dry-ports/${id}/history`);
  return res.data.data ?? [];
}
