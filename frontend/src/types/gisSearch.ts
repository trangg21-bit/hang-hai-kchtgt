 ...
 */
export const KCHT_GIS_CATEGORY_ID_BY_TYPE: Record<KchtGisType, number> = {
  SEAPORT: 1,
...
export const getKchtGisCategoryId = (value?: string | null): number | undefined => {
  const normalized = normalizeKchtGisType(value);
  return normalized ? KCHT_GIS_CATEGORY_ID_BY_TYPE[normalized] : undefined;
};

export const getKchtGisTypeByCategoryId = (categoryId?: number | null): KchtGisType | undefined =>
...
export const getKchtGisTypeLabelByCategoryId = (categoryId?: number | null): string | undefined => {
  ...
  return type ? KCHT_GIS_TYPE_OPTIONS.find((option) => option.value === type)?.label : undefined;
};
