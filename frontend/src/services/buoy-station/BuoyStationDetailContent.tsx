      ...
      items={[
        ...
        },
        {
          ...
          children: (
            <div style={{ paddingTop: 3 }}>
              ...
              {coords.length === 0 ? (
                <div style={{ color: textTertiary, fontSize: fontSizeMd, marginTop: spaceXs, paddingLeft: 12 }}>Không có tọa độ</div>
              ) : (
                <Table className="list-view-table" dataSource={coords.map((c, i) => ({ ...c, key: i, _idx: i }))} pagination={false} size="middle" bordered style={{ marginTop: spaceXs, marginLeft: 12, marginRight: 12 }}>
        ...
        },
        {
          ...
          children: (
            <div style={{ paddingTop: 3 }}>
              <div style={{ marginBottom: spaceSm, padding: '10px 12px 0 12px' }}>
                <span style={detailLabelStyle}>Danh sách phao tiêu</span>
              </div>
              <Table className="list-view-table" dataSource={detailBuoys.map((b, i) => ({ ...b, key: b.id, _idx: i }))} pagination={false} size="middle" bordered style={{ marginLeft: 12, marginRight: 12 }}
                locale={{ emptyText: (
                  <div style={{ padding: '32px 0', textAlign: 'center' }}>
                    <EnvironmentOutlined style={{ fontSize: 48, color: textTertiary, marginBottom: 12, display: 'block' }} />
                    <span style={{ color: textTertiary, fontSize: fontSizeLg }}>Không tìm thấy Phao, tiêu nào phù hợp</span>
                  </div>
                ) }}
              >
                  <Table.Column title="STT" key="stt" width={60} align="center"
