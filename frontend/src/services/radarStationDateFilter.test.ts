...
describe('RadarStation Date Range Filters (/radar-station)', () => {
  ...
  describe('Ngày cập nhật formatting', () => {
    it('formats same-day range with 00:00:00.000 and 23:59:59.999', () => {
      const range: [dayjs.Dayjs, dayjs.Dayjs] = [dayjs('2026-09-24'), dayjs('2026-09-24')];
      const { from, to } = formatRange(range);
      expect(from).toBe('2026-09-24 00:00:00.000');
      expect(to).toBe('2026-09-24 23:59:59.999');
    });

    it('formats multi-day range with 00:00:00.000 and 23:59:59.999', () => {
      const range: [dayjs.Dayjs, dayjs.Dayjs] = [dayjs('2026-01-01'), dayjs('2026-09-24')];
      const { from, to } = formatRange(range);
  ...
  describe('rangeValue helper round-trip parsing', () => {
    it('parses formatted string back to Dayjs range correctly for DatePicker display', () => {
      const from = '2026-09-24 00:00:00.000';
      const to = '2026-09-24 23:59:59.999';
      const rv = rangeValue(from, to);
      expect(rv).not.toBeNull();
      expect(rv![0]?.format('YYYY-MM-DD')).toBe('2026-09-24');
      expect(rv![1]?.format('YYYY-MM-DD')).toBe('2026-09-24');
    });
