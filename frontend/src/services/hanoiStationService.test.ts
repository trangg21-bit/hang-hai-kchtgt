...
describe('hanoiStationService Unit Tests', () => {
  ...
  describe('search / list', () => {
    it('should call one list endpoint that includes status counts', async () => {
      const mockListRes = {
        data: {
          data: {
            ...
            number: 0,
            size: 10,
            statusCounts: { all: 1, approved: 1 },
          },
        },
      };
      vi.mocked(api.get).mockResolvedValueOnce(mockListRes);
  ...
  describe('CRUD & Approval', () => {
    it('should call GET /v1/stations/haiphong/:id', async () => {
      vi.mocked(api.get).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', name: 'Đài TTXLTT Hà Nội' } } });
    ...
    it('should call POST /v1/stations/haiphong?action=DRAFT', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'new-id', name: 'Đài TTXLTT Mới' } } });
    ...
    it('should call POST /v1/stations/haiphong/:id/submit', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', approvalStatus: 2 } } });
    ...
    it('should call POST /v1/stations/haiphong/:id/approve-c1', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', approvalStatus: 3 } } });
      const res = await hanoiStationService.approveC1('uuid-1');
      expect(api.post).toHaveBeenCalledWith('/v1/stations/haiphong/uuid-1/approve-c1', { content: 'Đã phê duyệt' });
      expect(res.approvalStatus).toBe(3);
    });

    it('should call POST /v1/stations/haiphong/:id/approve-c2', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', approvalStatus: 5 } } });
      const res = await hanoiStationService.approveC2('uuid-1');
      expect(api.post).toHaveBeenCalledWith('/v1/stations/haiphong/uuid-1/approve-c2', { content: 'Đã phê duyệt' });
      expect(res.approvalStatus).toBe(5);
    });

    it('should call POST /v1/stations/haiphong/:id/reject with reason', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', rejectionReason: 'Thiếu đường truyền' } } });
