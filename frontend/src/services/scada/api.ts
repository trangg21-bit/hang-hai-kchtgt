...
export async function fetchScadaList(params: {
  ...
  sortBy?: string;
  sortOrder?: string;
}): Promise<PageResponse<ScadaResponse>> {
...
export async function fetchScadaById(id: string): Promise<ScadaResponse> {
  const res = await api.get(`${BASE}/${id}`);
  return res.data.data;
}

export async function createScada(payload: CreateScadaRequest): Promise<ScadaResponse> {
  const res = await api.post(BASE, payload);
  return res.data.data;
}

export async function updateScada(payload: UpdateScadaRequest): Promise<ScadaResponse> {
  const res = await api.put(BASE, payload);
  return res.data.data;
}

export async function deleteScada(id: string): Promise<void> {
  await api.delete(`${BASE}/${id}`);
}

// ── Code generation ─────────────────────────────────────────────────

export async function generateScadaCode(): Promise<string> {
...
export async function fetchScadaOptions(): Promise<ScadaOptionResponse[]> {
...
export async function approveScadaC1(id: string, data: ApprovalRequest): Promise<ApprovalResult> {
  const res = await api.post(`${BASE}/${id}/approve/c1`, data);
  return res.data;
}

export async function approveScadaC2(id: string, data: ApprovalRequest): Promise<ApprovalResult> {
...
export async function restoreScada(id: string): Promise<ScadaResponse> {
...
export async function fetchScadaAttachments(id: string): Promise<any[]> {
  const res = await api.get(`${BASE}/${id}/attachments`);
  return res.data.data || [];
}

export async function uploadScadaAttachment(id: string, file: File): Promise<any> {
...
export async function deleteScadaAttachment(id: string, attachmentId: string): Promise<any> {
