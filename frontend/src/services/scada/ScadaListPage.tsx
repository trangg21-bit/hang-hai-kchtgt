  ...
  return (
    <ThemeTokenProvider tokens={{ ...themeTokenChk, fontSizeMd: 13.5, filterLabelStyle: CHK_FILTER_LABEL }}>
      ...
      <style>{`
        ...
        .scada-page-wrapper div:has(> button[aria-pressed]) > button {
          white-space: nowrap !important;
          flex-shrink: 0 !important;
          cursor: pointer !important;
          padding: 4px 2px !important;
        }
        .scada-page-wrapper div:has(> button[aria-pressed])::-webkit-scrollbar { height: 4px !important; display: block !important; }
        ...
        footer={
          <div style={drawerFooterStyle}>
            <KchtFormFooter
              ...
              onSubmit={(action) => {
                const act = action === 'draft' ? 'DRAFT' : action === 'approve' ? 'APPROVED' : 'SUBMIT';
                actionTypeRef.current = act;
                setActionType(act);
                scadaFormRef.current?.submit(act);
              }}
            />
          </div>
        }
          ...
          <div style={drawerFooterStyle}>
            <KchtFormFooter
              ...
              onSubmit={(action) => {
                ...
                actionTypeRef.current = act;
                setActionType(act);
                editScadaFormRef.current?.submit(act);
              }}
            />
          </div>
        }
