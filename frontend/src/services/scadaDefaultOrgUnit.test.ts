...
vi.mock('./scada/api', () => ({
  fetchScadaList: vi.fn().mockResolvedValue({ content: [], totalElements: 0 }),
  fetchScadaById: vi.fn().mockResolvedValue({}),
  createScada: vi.fn().mockResolvedValue({}),
  updateScada: vi.fn().mockResolvedValue({}),
  deleteScada: vi.fn().mockResolvedValue({}),
  generateScadaCode: vi.fn().mockResolvedValue('SCADA-TEST-001'),
  fetchOperatingOrganizations: vi.fn().mockResolvedValue([]),
  fetchScadaAttachments: vi.fn().mockResolvedValue([]),
}));
...
describe('SCADA Default Managing Unit (/scada)', () => {
