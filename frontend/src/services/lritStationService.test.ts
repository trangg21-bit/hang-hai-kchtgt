...
describe('lritStationService Unit Tests', () => {
  ...
  describe('CRUD & Approval', () => {
    it('should call GET /v1/stations/lrit/:id', async () => {
      vi.mocked(api.get).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', name: 'Đài LRIT' } } });
    ...
    it('should call POST /v1/stations/lrit?action=DRAFT', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'new-id', name: 'Đài LRIT Mới' } } });
    ...
    it('should call POST /v1/stations/lrit/:id/reject with reason', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({ data: { data: { id: 'uuid-1', rejectionReason: 'Sai vị trí' } } });
