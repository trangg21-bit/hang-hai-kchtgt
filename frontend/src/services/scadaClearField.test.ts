...
import api from './api';
import { updateScada } from './scada/api';
import type { UpdateScadaRequest } from './scada/types';

describe('SCADA Clear Field & Null Handling (/scada)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('TC-SCADA-CLEAR-01: sends null instead of undefined when user clears fields', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/v1/scada', expect.objectContaining({
      id: '11111111-1111-1111-1111-111111111111',
      detailedLocation: null,
      model: null,
      manufacturer: null,
