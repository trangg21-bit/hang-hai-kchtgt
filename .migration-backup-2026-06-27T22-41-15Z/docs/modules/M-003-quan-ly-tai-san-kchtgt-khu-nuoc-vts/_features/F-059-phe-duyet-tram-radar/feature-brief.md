...
consumed_by_modules: []
---
# Feature: Phê duyệt Trạm radar

## Description
Quy trình phê duyệt hai cấp cho các bản ghi trạm radar: trưởng phòng phê duyệt cấp 1 và cục trưởng phê duyệt cấp 2. Sau khi hoàn tất cả hai cấp phê duyệt, bản ghi trạm radar được đưa vào hệ thống chính thức và xuất hiện trong các báo cáo tổng hợp về hạ tầng giám sát hàng hải.

## Business Intent
Đảm bảo chất lượng và tính chính xác của dữ liệu trạm radar trước khi đưa vào hệ thống thống kê chính thức. Cơ chế phê duyệt hai cấp giúp kiểm soát thông tin, giảm thiểu sai sót và đảm bảo trách nhiệm giải trình trong quản lý tài sản hạ tầng hàng hải khu nước VTS.

## Flow Summary
Khi chuyên viên tạo mới hoặc cập nhật bản ghi trạm radar, bản ghi ở trạng thái "chờ phê duyệt cấp 1". Trưởng phòng xem danh sách bản ghi chờ phê duyệt, kiểm tra thông tin và quyết định phê duyệt hoặc từ chối kèm lý do. Nếu được phê duyệt cấp 1, bản ghi chuyển sang "chờ phê duyệt cấp 2". Cục trưởng thực hiện phê duyệt cấp 2 và khi hoàn tất, bản ghi chuyển sang trạng thái "đã phê duyệt" và chính thức ghi nhận trong hệ thống.

## Acceptance Criteria
- Bản ghi trạm radar cần phải trải qua 2 cấp phê duyệt (trưởng phòng → cục trưởng)
...
## In Scope
- Danh sách bản ghi trạm radar chờ phê duyệt theo từng cấp
...
## Out of Scope
- Quy trình tạo mới bản ghi (thuộc tính năng F-056)
- Cập nhật bản ghi sau phê duyệt (thuộc tính năng F-057)
...
## Entities
- **TramRadar**: id, tenTram, viTri, loaiRadar, phamViHoatDong, trangThaiKyThuat, ngayVaoVanhHan, heSoTruyenDan, nguoiQuanLy, ghiChu, trangThai, pheDuyetC1, nguoiPheDuyetC1, ngayPheDuyetC1, pheDuyetC2, nguoiPheDuyetC2, ngayPheDuyetC2, lyDoTuChoi
- **PheDuyetLichSu**: id, tramRadarId, capPheDuyet, trangThai, nguoiPheDuyet, ngayPheDuyet, lyDo
