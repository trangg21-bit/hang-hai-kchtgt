...
Chia sẻ thông tin hạ tầng giao thông vận tải đường thủy về khu vực neo đậu tàu thuyền (địa điểm, tọa độ, diện tích, loại tàu cho phép, sâu nước tối thiểu, điều kiện thời tiết) qua Trục liên thông dữ liệu quốc gia LGSP.

## Business Intent

Cho phép các cơ quan quản lý nhà nước, đơn vị khai thác cảng và tổ chức liên quan tiếp cận nhanh chóng thông tin khu vực neo đậu phục vụ công tác điều hành giao thông đường thủy, quy hoạch hạ tầng và quản lý an toàn hàng hải. Dữ liệu được chuẩn hóa để đảm bảo tính nhất quán và khả năng trao đổi liên ngành giữa Cục Hàng hải, Chi cục Hàng hải, cảng vụ và các đơn vị kiểm tra chuyên ngành.

## Flow Summary

Dữ liệu khu vực neo đậu được trích xuất từ hệ thống quản lý hạ tầng giao thông vận tải đường thủy của đơn vị chủ quản, sau đó chuyển đổi sang định dạng chuẩn theo schema quy định của Trục LGSP. Hệ thống dịch vụ API sẽ publish dữ liệu lên cổng chia sẻ, trong đó bao gồm thông tin mã khu neo, tên khu, tọa độ địa lý, diện tích, loại tàu cho phép, sâu nước tối thiểu, điều kiện thời tiết, sức chứa và tình trạng hoạt động. Các đơn vị tiêu thụ (cảng vụ, kiểm tra chuyên ngành, cơ quan quản lý vùng biển) có thể gọi API hoặc subscribe qua cơ chế publish/subscribe của LGSP để nhận dữ liệu theo chu kỳ định kỳ hoặc theo yêu cầu. Toàn bộ luồng dữ liệu được bảo mật bằng HTTPS, xác thực JWT và kiểm soát truy cập theo danh sách IP whitelist.

## Acceptance Criteria

- Hệ thống API publish dữ liệu khu neo đậu lên Trục LGSP thành công, phản hồi HTTP 200 với payload JSON đúng schema
- Dữ liệu hiển thị chính xác các trường: mã khu, tên khu, tọa độ, diện tích, loại tàu, sâu nước, điều kiện thời tiết, sức chứa, tình trạng hoạt động
...
- API endpoint publish dữ liệu khu neo đậu lên Trục LGSP
...
| Role | Permissions |
|------|-------------|
| Khách truy cập (Public) | Xem thông tin khu neo đậu công khai |
| Nhân viên quản lý hạ tầng | Tạo, cập nhật, xóa thông tin khu neo đậu |
...
- **KhuNeoDau**: id, maKhu, tenKhu, tinhThanh, quanHuyen, xaPhuong, toaDo, dienTich, loaiTauChoPhep, sauNuocToiThieu, dieuKienThietKe, sucChua, tinhTrangHoatDong, createdAt, updatedAt

## Business Rules

1. Dữ liệu khu neo đậu chỉ được chia sẻ sau khi đã được kiểm tra tính hợp lệ (validation) về định dạng và nội dung
...
- Unit test: kiểm tra các hàm chuyển đổi dữ liệu khu neo đậu sang schema LGSP
