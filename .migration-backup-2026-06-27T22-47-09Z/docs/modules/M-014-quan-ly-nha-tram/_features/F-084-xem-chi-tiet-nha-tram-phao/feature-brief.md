...
## Description
Tính năng cho phép tất cả các vai trò trong hệ thống tra cứu và xem chi tiết thông tin nhà trạm phao, bao gồm toàn bộ dữ liệu mô tả, tọa độ trên bản đồ, tình trạng, trạng thái phê duyệt và các văn bản đính kèm liên quan. Dữ liệu hiển thị ở chế độ đọc-only, không cho phép chỉnh sửa trực tiếp từ màn hình chi tiết.

## Business Intent
Cho phép mọi người dùng có quyền truy cập tra cứu, xem thông tin chi tiết và văn bản đính kèm của nhà trạm phao, hỗ trợ công tác quản lý, giám sát và ra quyết định dựa trên dữ liệu chính xác và cập nhật.

## Flow Summary
Người dùng bất kỳ có quyền truy cập vào danh sách nhà trạm phao, tìm kiếm hoặc lọc bản ghi cần xem, sau đó nhấn vào tên hoặc nút "Xem chi tiết" để mở màn hình chi tiết. Màn hình hiển thị đầy đủ thông tin: mã, tên, tọa độ (hiển thị trên bản đồ), loại phao, tình trạng, trạng thái phê duyệt, thông tin người tạo/cập nhật, ngày tạo/cập nhật và danh sách các văn bản đính kèm (có thể xem trước hoặc tải về).

## Acceptance Criteria
- Mọi role có quyền truy cập đều có thể xem chi tiết nhà trạm phao.
- Màn hình chi tiết hiển thị đầy đủ các trường thông tin: mã, tên, tọa độ, loại, tình trạng, trạng thái phê duyệt.
...
## In Scope
- Màn hình hiển thị chi tiết nhà trạm phao ở chế độ đọc-only
...
- Cập nhật thông tin (thuộc F-081)
- Xóa nhà trạm phao (thuộc F-082)
- Phê duyệt (thuộc F-083)
...
- Backend: GET `/api/v1/buoys/{id}` trả về full detail bao gồm metadata phê duyệt và danh sách attachment files.
...
## Entities
- **BuoyStationDetail**: id, name, code, longitude, latitude, buoyType, status, description, approvalStatus, createdBy, updatedBy, createdAt, updatedAt, attachments ([])

## Business Rules
1. Tất cả các role đã đăng nhập đều có quyền xem chi tiết nhà trạm phao.
...
- Integration test: Gọi GET `/api/v1/buoys/{id}` với các role khác nhau, xác nhận phản hồi 200 + dữ liệu đầy đủ.
- E2E test: Truy cập chi tiết nhà trạm phao từ danh sách, xác nhận hiển thị đúng thông tin, bản đồ tọa độ và danh sách tài liệu.
