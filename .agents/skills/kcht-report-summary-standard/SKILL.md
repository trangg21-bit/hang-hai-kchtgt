...
1. **Flexbox 2 đầu (Space-between)**:
   ...
   - Vùng bên trái: Thẻ Tag xanh hiển thị Tổng số dòng: {count.toLocaleString('vi-VN')} dạng viên thuốc (orderRadius: 999), tách biệt hoàn toàn.
...
|---|---|---|
| ReportViewer.tsx | rontend/src/pages/reports/ReportViewer.tsx | Màn hình chính: Sidebar bộ lọc + Thanh chọn chỉ tiêu + Master Summary Table |
| ReportColumnSelector.tsx | rontend/src/components/reports/ReportColumnSelector.tsx | Popover ẩn/hiện cột và sắp xếp thứ tự chỉ tiêu |
| ReportPreviewModal.tsx | rontend/src/components/reports/ReportPreviewModal.tsx | Modal xem trước chi tiết số liệu, hỗ trợ xuống dòng tiêu đề và footer space-between |
| 
eports.ts | rontend/src/config/reports.ts | Cấu hình metadata 46 báo cáo (mã VMD, tên biểu chuẩn hóa) |
...
5. ❌ **CẤM căn trái hoặc căn phải tiêu đề cột**:
   - Toàn bộ tiêu đề cột (	head th) bắt buộc phải căn giữa (	extAlign: 'center'). Căn lề số liệu (căn phải cho số, căn trái cho chữ) chỉ áp dụng cho nội dung ô trong thân bảng (	body td), tuyệt đối không áp dụng cho tiêu đề cột.
6. ❌ **CẤM hardcode mã báo cáo dạng nội bộ (F-152) khi hiển thị**:
   - Ưu tiên hiển thị mdCode (vd: BCKCHT_167) trên cột Mã báo cáo của bảng thông tin chung theo đúng ảnh chuẩn nghiệp vụ.
...
8. ❌ **CẤM để xảy ra lỗi cảnh báo IDE / ESLint**:
   - Tuân thủ 100% quy tắc zero warnings, zero errors (không unused imports, không explicit ny).
...
- [ ] 8. Chạy 
pm test -- --run và 
px tsc --noEmit đạt 100% pass với 0 errors, 0 warnings.
