...
## Description
Tính năng cho phép Lãnh đạo (Trưởng phòng và Lãnh đạo Cục) xem xét, phê duyệt hoặc từ chối các nhà trạm phao mới tạo hoặc cập nhật. Quy trình phê duyệt gồm 2 cấp: Trưởng phòng phê duyệt tầng 1, sau đó Lãnh đạo Cục phê duyệt tầng 2. Nếu một cấp từ chối, bản ghi quay lại trạng thái chờ chỉnh sửa cho Chuyên viên.

## Business Intent
Quy trình phê duyệt 2 cấp đảm bảo tính chính xác và đầy đủ của dữ liệu nhà trạm phao trước khi đưa vào hệ thống chính thức. Cấp phòng kiểm tra thông tin chi tiết, cấp Cục xác nhận và cho phép đưa vào vận hành, tạo cơ chế kiểm soát chéo.

## Flow Summary
Trưởng phòng đăng nhập hệ thống, truy cập mục "Chờ phê duyệt" để xem danh sách nhà trạm phao cần duyệt. Trưởng phòng mở chi tiết, kiểm tra thông tin, chọn "Phê duyệt" hoặc "Từ chối" kèm lý do (nếu từ chối). Nếu phê duyệt, bản ghi chuyển sang trạng thái "chờ duyệt cấp 2" và thông báo cho Lãnh đạo Cục. Lãnh đạo Cục xem lại và thực hiện phê duyệt cấp 2. Nếu cả 2 cấp đều phê duyệt, bản ghi chuyển sang trạng thái "đã duyệt" và thông báo cho Chuyên viên tạo/cập nhật.

## Acceptance Criteria
- Trưởng phòng và Lãnh đạo Cục có thể truy cập danh sách nhà trạm phao chờ phê duyệt.
...
## In Scope
- Danh sách nhà trạm phao chờ phê duyệt (cho từng cấp)
...
- Cập nhật nhà trạm phao (thuộc F-081)
- Xóa nhà trạm phao (thuộc F-082)
- Xem lịch sử thay đổi chi tiết (thuộc F-085)
...
- API: POST `/api/v1/buoys/{id}/approve` và POST `/api/v1/buoys/{id}/reject` với body chứa `reason`.
...
## Entities
- **BuoyStation**: id, approvalLevel, currentApprover, approvalStatus, approvedByLevel1, approvedByLevel2, approvedAt
- **ApprovalAuditLog**: id, buoyStationId, approverId, action (approve/reject), reason, level, timestamp
...
- E2E test: Tạo nhà trạm → phê duyệt cấp 1 (Trưởng phòng) → phê duyệt cấp 2 (Lãnh đạo Cục) → xác nhận trạng thái "đã duyệt" và thông báo đến Chuyên viên. Test trường hợp từ chối ở từng cấp.
