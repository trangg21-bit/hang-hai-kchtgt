...
Feature F-003 (Quản lý đơn vị / Unit Management) implements hierarchical organizational unit management using a **Materialized Path** pattern on the existing `org_units` table. The feature covers CRUD operations, tree traversal, soft-delete, and audit trail via `unit_history`.
...
- Enum values mismatch: `OrgUnitType` uses `{DEPARTMENT, DIVISION, TEAM, STATION}` instead of BA spec `{CUC, CHI_CUC, CANG_VU, TCT}`; the approval-status enum `OrgUnitStatus` (+ `OrgUnitStatusConverter`) is removed per TRI-1786950754582-5a51
...
|---|---|---|
| `entity/OrgUnit.java` | MODIFY | +`coefficient` (BigDecimal), +`level` (Integer), +`path` (String), +`scopeId` (Long), +`sortOrder` (Integer); update `@Where` clause |
| `entity/OrgUnitType.java` | MODIFY | Change enum to: `CUC, CHI_CUC, CANG_VU, TCT` |
| `entity/OrgUnitStatus.java` | **DELETE** | Approval-status enum removed per TRI-1786950754582-5a51 |
| `entity/OrgUnitStatusConverter.java` | **DELETE** | Converter removed with the enum |
...
| `dto/UpdateOrgUnitRequest.java` | MODIFY | Add `coefficient`, `type`, `description` |
| `dto/OrgUnitResponse.java` | MODIFY | Add `coefficient`, `level`, `path`, `scopeId`, `sortOrder`, `description` |
...
| `service/OrganizationService.java` | MODIFY | Integrate MaterializedPathService, subtree move, history recording |
| `service/OrgUnitService.java` | MODIFY | Delegate to OrganizationService or mark for deprecation |
| `controller/OrgUnitController.java` | MODIFY | Add pagination, search/filter, update @PreAuthorize annotations |

### In Scope — Flyway Migrations (New)
| Migration File | Purpose |
|---|---|
| `V18__add_f003_materialized_path_fields.sql` | ADD COLUMN path, level, scopeId, sortOrder, coefficient to org_units; add indexes |
| `V20260817150000__drop_org_unit_approval.sql` | DROP COLUMN status, approved_at; DROP INDEX idx_org_units_status (TRI-1786950754582-5a51) |
...
|---|---|---|
| Database schema (org_units table) | **HIGH** — DROP 2 columns (`status`, `approved_at`) + 1 index (`idx_org_units_status`) | ✅ **DevOps review required**: Migration V20260817150000 changes schema; requires DBA approval for production rollout |
...
|---|---|---|---|---|---|---|
| T1.1 | **OrgUnit entity update** | Add fields: `coefficient` (BigDecimal), `level` (Integer), `path` (String), `scopeId` (Long), `sortOrder` (Integer); update `@Size` on name to 200 per BA spec; add `description` field; add `@NotNull` on path; add `@DecimalMin` on coefficient | None | `engineering-backend-developer` | ✅ | Low: Straightforward JPA field additions |
| T1.2 | **Enum updates** | `OrgUnitType` → `{CUC, CHI_CUC, CANG_VU, TCT}`; delete `OrgUnitStatus` + `OrgUnitStatusConverter` per TRI-1786950754582-5a51 | None | `engineering-backend-developer` | ✅ (parallel with T1.1) | Medium: all references must be removed in the same wave |
...
| T2.3 | **OrganizationService update** | Integrate MaterializedPathService; Add subtree move with cascade path rebuild; Update tree building using path-based queries; Record UnitHistory on all mutations | Wave 1 complete; depends on T2.1 | `engineering-backend-developer` | ✅ (parallel with T2.2) | High: Most complex service logic — affects all CRUD + tree operations |

### Wave 3: API Endpoints & Testing (2 days) — depends on Wave 2

| # | Task | Description | Dependency | Owner Type | Parallelizable | Risk |
|---|---|---|---|---|---|---|
| T3.1 | **Controller update** | Add pagination (`Pageable`, `Page<T>`) to list endpoint; Add search/filter query params (name, code, type, level); Update `@PreAuthorize` per role matrix (system-admin, admin, Lanh dao, Can bo, user); Add search endpoint: `GET /search?query=...` | Wave 2 complete | `engineering-backend-developer` | ✅ | Medium: RBAC annotation changes must match role matrix exactly |
| T3.2 | **Integration tests** | Test circular reference detection (UT); Test path computation (UT); Test soft-delete with children (IT); Test tree traversal consistency (IT); Test coefficient validation (UT); Test unique code constraint (IT) | Wave 2 complete | `engineering-qa-engineer` | ✅ (parallel with T3.1) | Low: Well-defined test cases from BA spec |
...
|---|---|---|---|
| **Enum deletion breaks existing code** | High | `OrgUnitStatus` + `OrgUnitStatusConverter` deleted; any code referencing them fails to compile | T1.2 coordinated with T2.3/T3.1 — all references removed in the same wave; compiler errors surface immediately |
...
|---|---|---|
| **Wave 1** | Data model integrity | - New columns present in `org_units` table after migration V18<br>- Root unit seeded correctly (path=`/1/`, level=1, scopeId=0)<br>- Enum values match BA spec: `CUC`, `CHI_CUC`, `CANG_VU`, `TCT`<br>- New DTO fields compile and serialize correctly |
| **Wave 2** | Core service logic | - **Circular reference**: setting a unit as its own parent is rejected<br>- **Circular reference**: setting a unit as descendant's parent is rejected<br>- **Path computation**: root unit path=`/1/`; child of root path=`/1/{childId}/`<br>- **Level calculation**: root=1, child=2, grandchild=3<br>- **Subtree move**: moving node X to new parent updates path for X AND all descendants<br>- **Soft-delete**: deleted unit excluded from tree/list queries (WHERE deletedAt IS NULL)<br>- **Audit trail**: every mutation creates UnitHistory record |
| **Wave 3** | API endpoints | - **Pagination**: GET /units?page=0&size=10 returns Page<T> with correct total elements<br>- **Search**: GET /units/search?q=abc returns matching units by name or code<br>- **Filter**: GET /units?type=CUC returns correctly filtered results<br>- **Create**: POST /units creates unit with code uniqueness check; rejects duplicate code<br>- **Update**: PUT /units/{id} updates fields; rejects duplicate code on update<br>- **Delete**: DELETE /units/{id} rejects if child units exist; succeeds if leaf node<br>- **Tree**: GET /units/tree returns nested tree with children arrays<br>- **RBAC**: unauthenticated → 401; non-admin → 403 on write endpoints |
...
3. **V20260817150000__drop_org_unit_approval.sql** (TRI-1786950754582-5a51):
...
- ✅ **Schema change**: DROP 2 columns (`status`, `approved_at`) + 1 index (`idx_org_units_status`) from `org_units` (V20260817150000, TRI-1786950754582-5a51)
