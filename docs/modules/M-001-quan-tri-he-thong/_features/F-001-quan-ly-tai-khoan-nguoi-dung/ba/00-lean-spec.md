...
**Business intent:** Hệ thống cần cơ chế quản lý tài khoản người dùng an toàn và linh hoạt, cho phép tài khoản có quyền quản trị hệ thống, lãnh đạo và chuyên viên (theo phân quyền động) thực hiện đầy đủ các thao tác tạo, sửa, xóa, khóa/mở khóa tài khoản theo quy trình nghiệp vụ được thiết định.

**Complexity:** Complex (19 business rules total: 7 high-level + 12 granular, 4 actors, phân quyền động (dynamic permission model) + soft-delete + registration approval workflow).
...
| 2 | Chỉnh sửa thông tin tài khoản | Họ tên, email, quyền gán, đơn vị |
...
| 6 | Phân quyền động (quyền gán riêng + quyền nhóm) | Quyền `<resource>:<action>` gán qua nhóm/tài khoản (BR-005; mô hình cũ: Admin, Lãnh đạo, Cán bộ, Cá nhân — đã thay thế) |
| 7 | Tìm kiếm & lọc người dùng | Theo tên, email, quyền, trạng thái (AC-008) |
...
| Quyền (resource:action) | Thao tác | Actor ID |
|---|---|---|
| `user:manage` | Tạo, sửa, khóa/mở khóa, reset mật khẩu, gán quyền cho tài khoản | A-001 (Quản trị hệ thống) |
| `user:read` | Xem danh sách/chi tiết người dùng (read-only); phạm vi theo đơn vị trực thuộc | A-002 (Lãnh đạo) |
| `user:approve` | Phê duyệt/từ chối tài khoản tự đăng ký (F-271) | A-002 (Lãnh đạo) |
| `user:manage` (phạm vi đơn vị) | Xem danh sách, chỉnh sửa thông tin, khóa/mở khóa trong đơn vị mình | A-003 (Chuyên viên) |
| Tự quản (bản thân) | Chỉ xem và sửa thông tin cá nhân của chính mình | A-005 (Cá nhân/Tổ chức bên ngoài) |

> Bảng vai trò cũ (mô hình cũ — đã thay thế bởi mô hình phân quyền động): Admin = Full access (tạo, sửa, xóa, khóa/mở khóa, reset mật khẩu, phân quyền); Lanh dao = View only; Can bo = View + Edit; Ca nhan = Self only. Quyền của một tài khoản = quyền gán riêng + quyền của các nhóm đang thuộc; ROLE_SYSTEM_ADMIN / ROLE_SUPER_ADMIN vượt qua mọi kiểm tra; Admin Cục full quyền + xem thêm metadata (tài liệu nền mục 3.2/3.8).
...
flowchart TD
    ...
    B -->|Tạo mới| C[Nhập thông tin: tên, email,<br/>mật khẩu, quyền gán, đơn vị]
...
|---|---|---|---|
| US-001 | Là **Admin**, tôi muốn tạo tài khoản người dùng mới với đầy đủ thông tin (tên, email, mật khẩu, quyền gán, đơn vị) để quản lý người dùng mới vào hệ thống. | Must | Feature-brief AC-1 |
| US-002 | Là **Admin**, tôi muốn chỉnh sửa thông tin tài khoản (tên, email, quyền, đơn vị) của người dùng để cập nhật thông tin. | Must | Feature-brief In Scope #2 |
...
| US-006 | Là **tài khoản có quyền quản lý người dùng** (`user:manage`), tôi muốn phân quyền động (quyền gán riêng + quyền nhóm) để kiểm soát truy cập. | Must | Feature-brief In Scope #6 |
...
| US-010 | Là **người dùng hệ thống**, tôi muốn tìm kiếm và lọc danh sách người dùng theo tên, email, quyền, trạng thái với phân trang chính xác. | Must | Feature-brief AC-4 |
...
|---|---|---|---|---|---|---|
| AC-001 | Tạo tài khoản thành công | Người dùng đã đăng nhập với quyền quản lý tài khoản (`user:manage`) | Nhập đầy đủ thông tin (tên, email, mật khẩu mạnh, quyền gán, đơn vị) và nhấn Tạo | Tài khoản được tạo, email unique được xác nhận, mật khẩu được hash, toast "Tạo thành công" hiển thị | Critical | BR-001, BR-002 |
| AC-002 | Tạo tài khoản thất bại — email trùng | Người dùng đã đăng nhập với quyền quản lý tài khoản (`user:manage`) | Nhập email đã tồn tại trong hệ thống và nhấn Tạo | Hệ thống hiển thị lỗi "Email đã tồn tại", tài khoản không được tạo | Critical | BR-001 |
| AC-003 | Tạo tài khoản thất bại — mật khẩu yếu | Người dùng đã đăng nhập với quyền quản lý tài khoản (`user:manage`) | Nhập mật khẩu không đáp ứng yêu cầu (dưới 8 ký tự, thiếu chữ hoa, chữ thường, số) và nhấn Tạo | Hệ thống hiển thị lỗi validation mật khẩu, tài khoản không được tạo | Critical | BR-002 |
| AC-004 | Khóa tài khoản thành công | Người dùng đã đăng nhập với quyền `user:manage` (quản trị hoặc cán bộ) | Chọn tài khoản cần khóa, nhập lý do, nhấn Khóa | Tài khoản chuyển sang trạng thái blocked, không thể đăng nhập, toast "Khóa thành công" hiển thị | Critical | BR-004 |
| AC-005 | Mở khóa tài khoản thành công | Người dùng đã đăng nhập với quyền `user:manage` (quản trị hoặc cán bộ) | Chọn tài khoản blocked, nhập lý do, nhấn Mở khóa | Tài khoản chuyển sang active, có thể đăng nhập lại, toast "Mở khóa thành công" hiển thị | Critical | BR-004 |

| AC-008 | Tìm kiếm & lọc danh sách người dùng | Người dùng đã đăng nhập | Nhập từ khóa tìm kiếm và chọn bộ lọc (quyền, trạng thái) | Bảng hiển thị kết quả đúng, phân trang hoạt động, tổng số record hiển thị | Major | — |
| AC-009 | Reset mật khẩu thành công | Người dùng đã đăng nhập với quyền `user:manage` | Chọn tài khoản, nhập mật khẩu mới đáp ứng policy, nhấn Reset | Mật khẩu được hash và lưu, token cũ bị invalidate, toast "Reset thành công" hiển thị | Major | BR-006 |
...
| AC-012 | Chỉ tài khoản có quyền gán quyền mới phân quyền được | Người dùng đã đăng nhập nhưng không có quyền gán quyền (mô hình cũ: vai trò Cán bộ) | Cố gắng thay đổi quyền của người dùng khác thành quyền quản trị | Hệ thống từ chối (403 Forbidden), không có thay đổi | Critical | BR-005 |
| AC-013 | Cá nhân chỉ sửa thông tin của chính mình | Người dùng đã đăng nhập với tài khoản cá nhân (chỉ được cấp quyền tự quản; mô hình cũ: vai trò Cá nhân) | Cố gắng sửa thông tin tài khoản của người dùng khác | Hệ thống từ chối (403 Forbidden), chỉ cho phép sửa thông tin cá nhân | Critical | BR-005 |
...
| AC-015 | Lãnh đạo chỉ xem (read-only), không sửa | Người dùng đã đăng nhập với tài khoản chỉ được cấp quyền xem (mô hình cũ: vai trò Lãnh đạo) | Cố gắng chỉnh sửa thông tin tài khoản hoặc khóa/mở khóa | Hệ thống từ chối (403 Forbidden), chỉ cho phép xem | Major | Feature-brief Roles table |
...
| **BR-005** | Chỉ tài khoản có quyền gán quyền mới được phân quyền cho tài khoản khác; tài khoản cán bộ chỉ sửa thông tin và khóa/mở khóa; tài khoản cá nhân chỉ sửa thông tin cá nhân | Phân quyền động (mô hình cũ: Role Assignment — đã thay thế) | Feature-brief BR-005 | — |
...
| **BR-001-04** | Chỉ tài khoản có quyền đặc biệt ROLE_SYSTEM_ADMIN mới được tạo hoặc xóa tài khoản có quyền đặc biệt ROLE_SYSTEM_ADMIN | Phân quyền động (mô hình cũ: Role assignment — đã thay thế) | Phân quyền hệ thống |
...
| **BR-001-08** | Tài khoản không thể tự thay đổi quyền của chính mình; chỉ tài khoản có quyền gán quyền (gồm quyền đặc biệt ROLE_SYSTEM_ADMIN) được phép gán/hủy quyền | Phân quyền động (mô hình cũ: Role assignment — đã thay thế) | Phân quyền |
...
| **BR-001-10** | Tài khoản được phân quyền truy cập module cụ thể qua cây quyền động (tài khoản có quyền đặc biệt ROLE_SYSTEM_ADMIN: tất cả; tài khoản khác: theo quyền được gán; không có quyền: không truy cập) | Phân quyền động | URD III.3.2 |
| **BR-001-11** | Khi tài khoản có quyền phê duyệt (`user:approve`) phê duyệt tài khoản đăng ký, user account được kích hoạt và phân quyền theo yêu cầu đăng ký (requestedRoleCode — ánh xạ sang mô hình quyền động do SA chốt) | Phê duyệt | URD III.3.2 |
...
| BR-005 (Phân quyền động — dynamic permission constraints) | BR-001-04, BR-001-08, BR-001-10 | BR-001-04: ROLE_SYSTEM_ADMIN only; BR-001-08: no self-permission-change; BR-001-10: module-level access |
...
|---|---|---|---|
| **UserAccount** | id (BIGINT PK), username (VARCHAR 50 UNIQUE NOT NULL), email (VARCHAR 100 UNIQUE NOT NULL), passwordHash (VARCHAR 255 NOT NULL), displayName, phone, status (VARCHAR 20), ~~roleId (BIGINT FK→Role — Role đã loại bỏ)~~, organizationId (BIGINT FK→Organization), createdAt, updatedAt, deletedAt (TIMESTAMP NULL), lastLoginAt | Organization → organizationId | Soft delete via deletedAt; status: active, blocked; quyền = quyền gán riêng + quyền từ nhóm (mô hình động) |
| **~~Role~~** (đã loại bỏ) | ~~id, name, code, description, permissions (JSON), isSystem~~ | — | Mô hình cũ — đã thay thế: không còn bảng Role; quyền `<resource>:<action>` gán cho nhóm/tài khoản (PermissionSeeder.java) |
...
| **~~UserRole~~** (đã loại bỏ) | ~~id, userId, roleId, assignedBy, assignedAt, expiresAt~~ | — | Mô hình cũ — đã thay thế: gán quyền trực tiếp cho tài khoản (API `/users/{id}/permissions`) và cho nhóm (F-002) |
...
| Method | Endpoint | Description | Auth / Quyền |
|---|---|---|---|
| GET | `/api/v1/users` | Danh sách người dùng (phân trang) | JWT (`user:read`) |
| GET | `/api/v1/users/{id}` | Chi tiết người dùng | JWT (`user:read`) |
| POST | `/api/v1/users` | Tạo người dùng mới | `user:manage` (mô hình cũ: Admin) |
| PUT | `/api/v1/users/{id}` | Chỉnh sửa người dùng | `user:manage` (mô hình cũ: Admin, Can bo) |
| PUT | `/api/v1/users/{id}/lock` | Khóa/mở khóa tài khoản | `user:manage` (mô hình cũ: Admin, Can bo) |
| POST | `/api/v1/users/{id}/reset-password` | Reset mật khẩu | `user:manage` (mô hình cũ: Admin) |
| ~~GET/POST/PUT/DELETE~~ | ~~`/api/v1/roles`, `/api/v1/roles/{id}`~~ | ~~Danh sách/tạo/sửa/xóa vai trò — không còn~~ | ~~—~~ (bảng Role đã bị loại bỏ trong mô hình phân quyền động) |
| GET | `/api/v1/organizations` | Danh sách đơn vị | JWT (`orgunit:read`) |
| POST | `/api/v1/organizations` | Tạo đơn vị mới | `orgunit:manage` (mô hình cũ: Admin) |
| GET | `/api/v1/groups` | Danh sách nhóm | JWT (`group:read`) |
| POST | `/api/v1/groups/{id}/members` | Thêm thành viên vào nhóm | `groupmember:manage` (mô hình cũ: Admin) |
| DELETE | `/api/v1/groups/{id}/members/{userId}` | Xóa thành viên khỏi nhóm | `groupmember:manage` (mô hình cũ: Admin) |
| GET | `/api/v1/admins` | Danh sách tài khoản admin | `admin:manage` (mô hình cũ: Super Admin) |
| POST | `/api/v1/admins` | Tạo tài khoản admin | `admin:manage` (mô hình cũ: Super Admin) |
| GET | `/api/v1/logs` | Danh sách log truy cập | `log:manage` (mô hình cũ: Admin, Security) |
| GET | `/api/v1/logs/export` | Xuất log CSV | `log:manage` (mô hình cũ: Admin) — ⚠️ nếu cần tách quyền xuất riêng `log:export`: chưa đăng ký seed — SA/Dev xác nhận |
...
| **Scalability** | Hỗ trợ tối thiểu 500 concurrent users; pagination default 20 items/page, max 100; database indexing trên email, username, status (~~roleId~~ — cột không còn trong mô hình động); connection pooling (HikariCP) | 1000+ users trong tương lai |
...
| TS-010 | Chỉ tài khoản có quyền gán quyền mới phân quyền cho tài khoản khác | Integration | Critical | AC-012 |
...
|---|---|---|
| **Q1: Creates new domain elements?** | **Yes** | Entities: UserAccount, ~~Role, UserRole~~ (đã loại bỏ — mô hình quyền động), PasswordResetToken, UserGroup, GroupMember, AdminAccount — all new domain aggregates for account management |
| **Q2: Affects system architecture?** | **Yes** | Introduces Spring Security + JWT auth layer, phân quyền động (permission matrix: quyền gán riêng + quyền nhóm), soft-delete pattern — impacts cross-cutting architecture |
| **Q3: Approach clear from existing architecture?** | **No** | Requires system architect to define: JWT filter chain, cấu trúc permission động (user/group permissions), session invalidation strategy, soft-delete implementation pattern |
...
- **Phân quyền động:** quyền `<resource>:<action>` gán cho nhóm/tài khoản (PermissionSeeder.java); không còn bảng Role (mô hình cũ: Role-based với permission matrix JSON trong Role — đã thay thế)
...
| Permission UI | Ẩn/hiện nút hành động theo quyền (vd `user:manage`, `user:read`); disabled + tooltip khi không có quyền |
| Specific | Phân quyền động (gán quyền cho nhóm/tài khoản qua cây quyền checkbox), Lock/Unlock modal với lý do, Password reset với strength indicator |
...
| Layer | Scope |
|---|---|
| **Unit (Backend)** | Password policy validation (BR-002), lockout logic (BR-007), email unique (BR-001), authorization per permission (BR-005), token expiry (BR-006) |
...
```mermaid
flowchart TD
    A[Tài khoản có quyền `user:manage` nhập form: tên, email, mật khẩu, quyền gán, đơn vị] --> B{Frontend validation}
    ...
    G --> H[Tạo UserAccount với status = ACTIVE + quyền gán ban đầu]
...
flowchart TD
    ...
    B -->|Không có quyền user:manage| B1[403 Forbidden]
...
### 16.3 Permission Check Flow (mô hình phân quyền động)

```mermaid
flowchart TD
    A[Request đến API endpoint] --> B[JWT Filter: xác thực token]
    B -->|Token không hợp lệ| B1[401 Unauthorized]
    B -->|Hợp lệ| C[Extract userId + các quyền hiện có: quyền gán riêng + quyền từ nhóm]
    C --> D[PermissionMiddleware / @PreAuthorize / PermissionAuthorizationManager: load quyền của tài khoản]
    D --> E{Endpoint yêu cầu quyền <resource:action> — tài khoản có quyền?}
    E -->|Không| E1[403 Forbidden — Không có quyền]
    E -->|Có| F{BR-005: Kiểm tra ràng buộc quyền}
    F -->|Tài khoản không có quyền gán quyền cố phân quyền| F1[403 Forbidden — Chỉ tài khoản có quyền gán quyền mới phân quyền]
...
> Tài khoản có quyền đặc biệt ROLE_SYSTEM_ADMIN / ROLE_SUPER_ADMIN vượt qua mọi kiểm tra quyền; Admin Cục full quyền + xem thêm metadata (tài liệu nền mục 3.2/3.8).
...
|---|---|---|---|
| **F-002 (Quản lý nhóm người dùng & phân quyền nhóm)** | Entity shared | UserGroup, GroupMember, quyền của nhóm | F-001 tạo user với quyền gán; F-002 quản lý nhóm + gán quyền cho nhóm (mô hình cũ: F-002 = Quản lý vai trò với Role/UserRole — đã thay thế) |
...
| [AMBIGUITY-003] | AdminAccount — tách biệt hay tích hợp với UserAccount? | Thấp | Có bảng AdminAccount riêng — Admin có phải là một UserAccount đặc biệt hay là tài khoản độc lập? | (A) AdminAccount độc lập; (B) AdminAccount là UserAccount với quyền quản trị (mô hình cũ: roleId = Admin — đã thay thế); (C) Không rõ, cần kiến trúc quyết định |
| [AMBIGUITY-004] | Không có yêu cầu về phân quyền chi tiết (permission granularity) | Thấp | Quyền `<resource>:<action>` trong cây quyền động (PermissionSeeder.java) — định nghĩa cụ thể như thế nào? | (A) Định nghĩa trong Phase 2 (SA); (B) Sử dụng permission matrix từ actor-registry.json; (C) Không rõ |
