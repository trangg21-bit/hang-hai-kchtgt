...
# DESIGN — Đê/Kè (F-044 → F-049)

Module: **M-003 — Quản lý tài sản KCHTGT - Khu nước & VTS**
Nhóm tính năng: **Đê/Kè** (F-044, F-045, F-046, F-047, F-048, F-049)
Stack: **Spring Boot 17+**, **MSSQL Server 2022**, **ReactJS 18+**, **Nginx**, **MinIO**, **GeoServer**
BA Source: `docs/modules/M-003-quan-ly-tai-san-kchtgt-khu-nuoc-vts/ba/00-lean-spec.md`
...
Hệ thống Đê/Kè tuân thủ kiến trúc lớp (layered architecture) chuẩn Spring Boot,
áp dụng cùng pattern phê duyệt 2 cấp (C1: Trưởng phòng → C2: Cục trưởng) như nhóm Lượng hàng hải.

```mermaid
flowchart TD
    subgraph Client["ReactJS Client (F-048, F-049 UI)"]
        U[Người dùng]
    end
    ...
    subgraph Controller["REST Controllers<br/>com.hanghai.kchtg.deke.controller"]
        C[DeKeController<br/>@RestController]
        C1[Auth filter<br/>@PreAuthorize]
    end

    subgraph Service["Business Services<br/>com.hanghai.kchtg.deke.service"]
        S[DeKeService<br/>@Service @Transactional]
        S1[Validation layer]
        S2[Approval workflow engine<br/>(2 cấp: phòng → cục)]
    end

    subgraph Repository["Data Access<br/>com.hanghai.kchtg.deke.repository"]
        R[DeKeRepository<br/>JpaRepository]
        R1[DeKeAttachmentRepository<br/>JpaRepository]
    end
...
**Lưu ý:**
- `PheDuyetLichSu` được lưu cùng repository `DeKeRepository` (FK cùng module M-003).
- MinIO dùng cho `DeKeAttachment` (tài liệu đính kèm).
- JWT auth + `@PreAuthorize` filter tại controller layer.
...
erDiagram
    ...
    DE_KE {
        ...
        TrangThaiPheDuyet trangThai "Enum: PROPOSED/UNDER_REVIEW/APPROVED/REJECTED"
        Boolean pheDuyetC1 "Default: false"
        String nguoiPheDuyetC1
        LocalDateTime ngayPheDuyetC1
        Boolean pheDuyetC2 "Default: false"
...
### Enum: TrangThaiPheDuyet (reuse từ LuongHangHaiApprovalStatus)
...
| 3 | `GET` | `/api/v1/de-ke/{id}` | F-048 | `deke:read` | — | `ApiResponse<DeKeResponse>` | Xem chi tiết (bao gồm thông tin phê duyệt + attachment) |
...
| Field | Type | Required | Validation |
|-------|------|----------|------------|
| `loaiDe` | String | No | max 100 |
| `viTri` | String | No | max 255 |
| `chieuDai` | BigDecimal | No | `@Positive` |
| `chieuRong` | BigDecimal | No | `@Positive` |
| `chieuCao` | BigDecimal | No | `@Positive` |
| `matVatLieu` | String | No | max 100 |
...
```mermaid
stateDiagram-v2
    [*] --> PROPOSED: Tạo mới (F-044)

    PROPOSED --> UNDER_REVIEW: Phê duyệt cấp 1 (F-047, Trưởng phòng)
    PROPOSED --> REJECTED: Từ chối cấp 1 (F-047, lý do bắt buộc)

    UNDER_REVIEW --> APPROVED: Phê duyệt cấp 2 (F-047, Cục trưởng)
    UNDER_REVIEW --> REJECTED: Từ chối cấp 2 (F-047, lý do bắt buộc)
    UNDER_REVIEW --> UNDER_REVIEW: Cập nhật (F-045, đã APPROVED trước đó → RE-REVIEW)

    APPROVED --> [*]: Xóa soft delete (F-046, isDeleted=true)

    REJECTED --> PROPOSED: Cập nhật và gửi lại (F-045)
...
| Java Field (camelCase) | DB Column (snake_case) | Type |
|------------------------|------------------------|------|
| `loaiDe` | `loai_de` | VARCHAR(100) NOT NULL |
| `viTri` | `vi_tri` | VARCHAR(255) NOT NULL |
| `chieuDai` | `chieu_dai` | DECIMAL(10,2) NOT NULL |
| `chieuRong` | `chieu_rong` | DECIMAL(10,2) NOT NULL |
| `chieuCao` | `chieu_cao` | DECIMAL(10,2) NOT NULL |
| `matVatLieu` | `mat_vat_lieu` | VARCHAR(100) |
...
| Java Field | DB Column | Type |
|-----------|-----------|------|
| `deKeId` | `de_ke_id` | BIGINT NOT NULL (FK) |
| `tenTaiLieu` | `ten_tai_lieu` | VARCHAR(255) NOT NULL |
| `duongDan` | `duong_dan` | VARCHAR(500) NOT NULL |
| `kichThuoc` | `kich_thuoc` | BIGINT |
| `loaiTaiLieu` | `loai_tai_lieu` | VARCHAR(50) |
...
- **Java fields:** camelCase (ví dụ: `loaiDe`, `chieuDai`, `viTri`)
- **DB columns:** snake_case (ví dụ: `loai_de`, `chieu_dai`, `vi_tri`)
- **Table names:** snake_case, không dấu (ví dụ: `de_ke`, `de_ke_attachment`, `phe_duyet_lich_su`)
- **Enum values:** UPPER_SNAKE_CASE (ví dụ: `PROPOSED`, `UNDER_REVIEW`, `APPROVED`, `REJECTED`)
- **REST paths:** kebab-case (ví dụ: `/api/v1/de-ke`, `/approve/c1`, `/status-phe-duyet`)
- **DTO names:** `CreateRequest`, `UpdateRequest`, `Response`, `Entry`
- **Entity names:** không có tiền tố `Entity` (ví dụ: `DeKe`, không `DeKeEntity`)
...
| DB Driver | jtds / MSSQL JDBC | Latest | MSSQL Server 2022 connection |
| ORM | Spring Data JPA + Hibernate | Bundled with Spring Boot | Entity mapping, CRUD, pagination |
| Validation | Hibernate Validator | Bundled | `@NotBlank`, `@Positive`, `@Min` |
| Security | Spring Security | Bundled | JWT auth, `@PreAuthorize` |
| File Storage | MinIO Client | Latest | For DeKeAttachment upload/download |
| Utility | Lombok | Latest | `@Data`, `@Builder`, `@NoArgsConstructor`, `@AllArgsConstructor` |
| Logging | SLF4J + Logback | Bundled | Structured logging |

### 8.2. Shared Dependencies (reuse từ LuongHangHai)
...
| ID | Constraint | Description |
|----|------------|-------------|
| C-044-01 | Soft delete bắt buộc | Tất cả xóa phải dùng `isDeleted = true`, không `DELETE` SQL |
| C-044-02 | Approval 2 cấp cứng | Không cho phép skip cấp 1 → cấp 2; không cho phép role sai phê duyệt sai cấp |
| C-044-03 | Transaction integrity | Mọi write operation (create, update, delete, approve) phải `@Transactional` |
| C-044-04 | Audit timestamps | `@PrePersist` và `@PreUpdate` tự động quản lý `ngayTao` và `ngaySuaDoi` |
| C-044-05 | Pagination trong query | `GET /de-ke` và `GET /de-ke/search` phải có `Pageable` |
| C-044-06 | Role-based access control | Mỗi endpoint phải có `@PreAuthorize` annotation với permission tương ứng |
| C-044-07 | API versioning | Tất cả endpoints phải có prefix `/api/v1/` |
| C-044-08 | Response wrapper | Tất cả response phải qua `ApiResponse<T>` wrapper |
| C-044-09 | Chuông dimension > 0 | `chieuDai`, `chieuRong`, `chieuCao` luôn dương (BigDecimal > 0) |
| C-044-10 | Reuse enum TrangThaiPheDuyet | Không tạo enum mới, dùng chung enum với LuongHangHai |

---

*Design document thêm bởi engineering-system-architect cho module M-003, nhóm Đê/Kè (F-044 → F-049).*
