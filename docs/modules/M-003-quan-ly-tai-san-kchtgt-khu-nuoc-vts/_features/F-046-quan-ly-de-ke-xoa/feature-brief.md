...
2. Chọn công trình và nhấn **"Xóa"** (chỉ hiển thị với bản ghi DRAFT/Lưu tạm).
...
- **US-046-02:** Là Cục trưởng, tôi muốn xóa bất kỳ công trình DRAFT (Lưu tạm) nào trong toàn hệ thống để quản lý dữ liệu.
...
**AC-046-01 — Xóa thành công bản ghi DRAFT (Lưu tạm):** Người dùng có quyền, bản ghi DRAFT cùng đơn vị. Nhấn Xóa → xác nhận. Bản ghi được soft delete: `isDeleted = true`, `deletedAt = now`. ApprovalHistory ghi XOA_MEM. Thông báo "Xóa đê kè thành công". Bản ghi không còn hiển thị trong danh sách.

**AC-046-02 — Không cho xóa bản ghi không phải DRAFT:** Người dùng cố xóa bản ghi PENDING_APPROVAL, APPROVED_LEVEL1, APPROVED, REJECTED_LEVEL1 hoặc REJECTED_LEVEL2. Nút "Xóa" bị ẩn. Gọi API trực tiếp → lỗi "Chỉ có thể xóa bản ghi ở trạng thái Lưu tạm".

**AC-046-03 — Điều kiện hiển thị nút Xóa:** Nút "Xóa" chỉ hiển thị khi:
- Bản ghi DRAFT (Lưu tạm) + (Cấp Cục) hoặc (cùng đơn vị quản lý)
...
**AC-046-06 — Xóa Cấp Cục:** Cục trưởng có thể xóa bất kỳ bản ghi DRAFT (Lưu tạm) nào, không giới hạn đơn vị.

---

## 5. Quy tắc nghiệp vụ (Business Rules)

**BR-046-01 — Chỉ xóa DRAFT (Lưu tạm):** Chỉ bản ghi ở trạng thái DRAFT (Lưu tạm, chưa gửi duyệt) mới có thể bị xóa. Bản ghi đã APPROVED cần quy trình hủy riêng, không xóa trực tiếp.
...
|---|---|---|
| **F-044** | Tạo mới | F-046 xóa bản ghi được tạo từ F-044 (chỉ khi DRAFT/Lưu tạm) |
