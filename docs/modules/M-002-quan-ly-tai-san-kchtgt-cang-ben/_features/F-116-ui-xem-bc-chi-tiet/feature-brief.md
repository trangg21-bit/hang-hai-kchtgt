...
- Chỉnh sửa trực tiếp trên màn hình chi tiết — phải chuyển qua màn hình cập nhật (F-118).
- Xóa Bến cảng — phải chuyển qua màn hình xóa (F-120).
- Xem lịch sử thay đổi chi tiết — phải chuyển qua màn hình lịch sử (F-121).
...
|---|---|---|---|
| BR-116-01 | Chỉ hiển thị chi tiết của Bến cảng có deletedAt = NULL (không bị xóa mềm). | Read | Soft-delete filter |
| BR-116-02 | Trạng thái phê duyệt mặc định là CHO_PHE_DUYET khi tạo mới; badge màu tương ứng được hiển thị. | Display | Default status |
| BR-116-03 | GPS viDo nằm trong [-90, 90], kinhDo nằm trong [-180, 180]; hiển thị với định dạng ±XX.XXXXXX. | Display | GPS validation |
| BR-116-04 | Chỉ người dùng có quyền Leader mới được nhìn thấy và thao tác với nút "Phê duyệt" và "Từ chối". | UI logic | RBAC |
| BR-116-05 | Lý do từ chối phải có ít nhất 10 ký tự; không cho phép từ chối nếu lý do < 10 ký tự. | UI validation | Reason length |
| BR-116-06 | Các trường metadata (createdBy, updatedBy) được hiển thị dạng string; createdAt, updatedAt hiển thị dạng datetime định dạng dd/MM/yyyy HH:mm:ss. | Display | Metadata format |

## Testing Strategy

Kiểm thử chấp nhận tập trung vào các kịch bản: (1) fetch chi tiết BenCang thành công hiển thị đúng tất cả các trường dữ liệu; (2) breadcrumb điều hướng chính xác về F-115; (3) link cangBienId mở đúng màn hình chi tiết CangBien; (4) định dạng GPS ±XX.XXXXXX hiển thị đúng 5 chữ số thập phân; (5) badge màu trạng thái đúng cho cả 2 enum; (6) các trường metadata (orgUnitId, createdBy, updatedBy, createdAt, updatedAt) hiển thị đúng định dạng; (7) nút Phê duyệt/Từ chối chỉ hiển thị cho Role có quyền Leader; (8) xác nhận phê duyệt qua hộp thoại → gọi `POST /:id/approve` → trạng thái chuyển thành DUOC_PHE_DUYET → toast thành công; (9) từ chối với lý do < 10 ký tự bị chặn, lý do ≥ 10 ký tự → gọi `POST /:id/reject` → trạng thái TU_CHO → toast thành công; (10) xử lý lỗi API (404 Not Found, 500 Server Error) hiển thị toast phù hợp.
