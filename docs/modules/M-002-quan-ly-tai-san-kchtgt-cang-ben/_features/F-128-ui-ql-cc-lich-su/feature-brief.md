...
3. Cột fieldChanged hiển thị tên trường tiếng Việt dễ hiểu (ví dụ: "Tên cầu cảng", "Chiều dài", "Tải trọng", "Loại cầu cảng").
...
12. Link "Lịch sử" từ danh sách F-122 mở trang lịch sử cho đúng cầu cảng được chọn.
...
| Quan ly tai san | approve | Xem lịch sử phê duyệt từ trang F-128 |
| Linh dao | read | Xem lịch sử thay đổi của cầu cảng |
| Admin | read | Xem lịch sử thay đổi của cầu cảng |

## Entities

- **LichSuThayDoi** (ChangeLog): id (UUID), cauCangId (UUID FK → CauCang), fieldChanged (string — tên trường: tenCau, chieuDai, taiTrọng, loaiCau, trangThaiHoatDong, trangThaiPheDuyet, benCangId), oldValue (text — giá trị cũ của trường), newValue (text — giá trị mới của trường), changedBy (UUID FK → User — người thực hiện thay đổi), changedAt (auto timestamp), actionType (enum: TAO_MOI, CAP_NHAT, PHE_DUYET, TU_CHOI, XOA_MEM)
...
- **CauCang**: id (UUID), maCau (string unique, length≤50), tenCau (string, length≤255), trangThaiPheDuyet (enum: CHO_PHE_DUYET, DUOC_PHE_DUYET, TU_CHOI), deletedAt (nullable)
...
| # | Rule | Description |
|---|---|---|
| 1 | Ghi nhận mọi thay đổi | Mọi thay đổi đối với cầu cảng (tạo mới, cập nhật, phê duyệt, từ chối, xóa mềm) đều tạo một bản ghi LichSuThayDoi — không có thay đổi nào bị bỏ qua. |
| 2 | Sắp xếp thời gian | Các bản ghi được sắp xếp theo changedAt DESC — thay đổi mới nhất luôn hiển thị đầu tiên trong bảng. |
| 3 | Ánh xạ tiếng Việt | Trường fieldChanged được ánh xạ sang tên tiếng Việt để hiển thị cho người dùng (ví dụ: "chieuDai" → "Chiều dài", "taiTrọng" → "Tải trọng", "trangThaiPheDuyet" → "Trạng thái phê duyệt"). |
| 4 | Tên người dùng | Trường changedBy được hiển thị dưới dạng tên người dùng (hoTen) — không chỉ hiển thị UUID. |
| 5 | Phân loại hành động | ActionType phân biệt rõ ràng các loại hành động: TAO_MOI (lần tạo đầu tiên), CAP_NHAT (cập nhật thông tin), PHE_DUYET (phê duyệt), TU_CHOI (từ chối), XOA_MEM (xóa mềm). |
| 6 | Tích hợp PheDuyetLog | PheDuyetLog cũng được tích hợp vào cùng bảng lịch sử với LichSuThayDoi để người dùng có cái nhìn toàn diện về toàn bộ biến động của cầu cảng. |
| 7 | Read-only | Dữ liệu lịch sử là read-only — không cho phép người dùng chỉnh sửa, xóa hoặc hoàn tác từ giao diện này. |
