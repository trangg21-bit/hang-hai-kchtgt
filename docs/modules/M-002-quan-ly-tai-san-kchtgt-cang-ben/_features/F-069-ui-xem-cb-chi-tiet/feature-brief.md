...
last-updated: "2026-07-01T04:08:09Z"
locked-fields: []
consumed_by_modules: []
---

# Feature: Chi tiết Cảng biển
...
Cung cấp một cái nhìn toàn diện về một cảng biển cụ thể, bao gồm tất cả thông tin kỹ thuật, địa lý và trạng thái, cho phép người dùng xác minh dữ liệu trước khi ra quyết định phê duyệt hoặc chỉnh sửa. Đây là nền tảng cho mọi thao tác tiếp theo (sửa, xóa, phê duyệt), đáp ứng yêu cầu UC 10 về đính kèm của TKCT.

## Flow Summary

Người dùng nhấp vào "Xem chi tiết" trên hàng tương ứng trong danh sách (F-068), hệ thống gọi GET /api/v1/cang-bien/:id để tải dữ liệu. Trang hiển thị breadcrumb "Quản lý cảng biển > Chi tiết cảng [maCang]". Các trường được hiển thị trong các nhóm: thông tin cơ bản (maCang, tenCang, tinhThanhPho), địa lý (viDo, kinhDo dưới dạng ±XX.XXXXXX, dienTich), khả năng (khaNangTiepNhan), trạng thái (badge màu). Nếu có đính kèm, danh sách file hiển thị với tên, định lượng, dung lượng và nút Download/Print. Hành động Phê duyệt/Từ chối chỉ hiện cho Lãnh đạo. Nhấp "Chỉnh sửa" chuyển đến F-071, "Xóa" mở hộp thoại xác nhận F-093, "Lịch sử" mở F-094. Breadcrumb cho phép quay lại danh sách.
...
5. Nút "Chỉnh sửa" mở trang F-071 với form được pre-fill từ dữ liệu hiện tại; "Lịch sử" mở F-094; "Xóa" mở hộp thoại xác nhận F-093.
...
- Tạo mới cảng biển (thuộc F-070)
- Chỉnh sửa trực tiếp tại trang chi tiết (điều hướng đến F-071)
- Phê duyệt thực thi (điều hướng đến F-072)
- Xóa mềm (điều hướng đến F-093)
- Xem lịch sử thay đổi (điều hướng đến F-094)
