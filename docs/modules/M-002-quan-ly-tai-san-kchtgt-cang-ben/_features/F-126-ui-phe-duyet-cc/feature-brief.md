...
3. Khi click "Phê duyệt", hộp thoại xác nhận xuất hiện hiển thị thông tin: maCau, tenCau, loại cầu cảng, chiều dài, tải trọng, tên benCang.
...
- Xem lại danh sách các lần phê duyệt/từ chối trước đó từ trang này (chuyển đến F-128 cho lịch sử thay đổi).
...
| # | Rule | Description |
|---|---|---|
| 1 | Phân quyền phê duyệt | Chỉ các role Leader (Linh dao) và Admin mới có quyền phê duyệt hoặc từ chối cầu cảng — các role khác không thấy nút hành động. |
| 2 | Điều kiện trạng thái | Chỉ cầu cảng có trạng thái trangThaiPheDuyet = CHO_PHE_DUYET mới được phép phê duyệt hoặc từ chối — cầu cảng đã DUOC_PHE_DUYET hoặc TU_CHOI không thể thay đổi trạng thái phê duyệt. |
| 3 | Phê duyệt (approve) | Khi phê duyệt (approve): trạng thái chuyển ngay thành DUOC_PHE_DUYET, bản ghi PheDuyetLog được tạo với ketQua = DUOC_PHE_DUYET — không cần nhập lý do. |
| 4 | Từ chối (reject) | Khi từ chối (reject): trạng thái chuyển ngay thành TU_CHOI, bản ghi PheDuyetLog được tạo với ketQua = TU_CHOI và lyDoTuChoi phải có độ dài ≥ 10 ký tự — hệ thống không cho phép từ chối nếu lý do quá ngắn. |
| 5 | Ghi nhận vào lịch sử | Mọi thao tác phê duyệt/từ chối đều được ghi nhận vào LichSuThayDoi của cầu cảng (INT-003). |
| 6 | PheDuyetBy từ đăng nhập | PheDuyetBy được lấy từ đăng nhập hiện tại của người dùng — không cho phép người khác phê duyệt thay. |
