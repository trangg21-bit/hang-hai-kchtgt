...
Thành công được đo bằng khả năng hiển thị chính xác toàn bộ thông tin Cầu cảng trong mọi trạng thái (PENDING, APPROVED, REJECTED, soft-deleted) với thời gian tải ≤ 1 giây, badge màu đúng vòng đời, và kiểm soát field-level RBAC chặt chẽ.

## Scope

| | Items |
|---|---|
| In scope | Hiển thị 11 nhóm thông tin dạng collapsible (A: Thông tin cơ bản, B: Kỹ thuật, C: Trạng thái, D: Thời điểm & kiểm định, E: Số lượng & sản lượng, F: Phương án bảo đảm ATHH, G: Công bố mở/đưa vào sử dụng, H: Thông tin GIS, I: Phạm vi & Tọa độ GIS, J: Giấy tờ đính kèm, K: Hành động); 5 tab dữ liệu liên quan (L: Phê duyệt, M: KCHT thuộc cầu cảng, N: Vận hành khai thác, O: Bảo trì, P: Sự cố); Badge trạng thái hoạt động (OPERATIONAL=xanh lá, SUSPENDED=vàng) và phê duyệt (PENDING=vàng, APPROVED=xanh dương, REJECTED=đỏ); Hyperlink đến Bến cảng cha (Berth) và Cảng biển cha (Port); Danh sách giấy tờ đính kèm với nút Tải xuống/In; Nút hành động động theo trạng thái và vai trò (Chỉnh sửa, Phê duyệt, Từ chối, Lịch sử); Breadcrumb: Trang chủ > Quản lý KCHT > Quản lý Cầu cảng > [pierName]; Kiểm soát hiển thị metadata (createdBy, updatedBy) chỉ cho Admin Cục; Loading skeleton khi đang tải; Cảnh báo trạng thái theo vòng đời; Responsive (≤ 768px) |
...
| AC-024-03 | US-024-01 | Badge trạng thái theo vòng đời | Given trang chi tiết Cầu cảng hiển thị; When xem trường operationalStatus và approvalStatus; Then operationalStatus hiển thị badge: OPERATIONAL=xanh lá, SUSPENDED=vàng; Then approvalStatus hiển thị badge: PENDING=vàng, APPROVED=xanh dương, REJECTED=đỏ; Then DA_XOA hiển thị badge xám | Màu sắc theo design token statusOperational/statusWarning/... từ tokens.ts |
...
| AC-024-05 | US-024-03 | Hành động theo trạng thái và vai trò | Given trang chi tiết Cầu cảng; When approvalStatus = PENDING và người dùng là Leader/Admin; Then hiển thị nút "Phê duyệt" và "Từ chối"; When approvalStatus = APPROVED hoặc REJECTED; Then ẩn nút phê duyệt/từ chối, hiển thị "Chỉnh sửa" (nếu có quyền); If người dùng không có quyền tương ứng; Then nút tương ứng bị ẩn | Nút hành động cố định cuối trang, dạng pill |
...
|---|---|---|---|
| BR-024-01 | Cầu cảng ở bất kỳ trạng thái nào (PENDING, APPROVED, REJECTED, DA_XOA/soft-deleted) đều có thể xem chi tiết. Trang luôn hiển thị trạng thái hiện tại, không phải lịch sử cũ | AC-024-01 | Không có ngoại lệ |
...
| BR-024-03 | Leader/Admin có thể phê duyệt hoặc từ chối Cầu cảng ngay từ trang chi tiết khi approvalStatus = PENDING. Hành động này có thể chuyển hướng đến F-023 để xác nhận trước khi thực hiện | AC-024-05 | Không có ngoại lệ |
...
| BR-024-07 | Nếu approvalStatus = PENDING hoặc REJECTED, trang chi tiết hiển thị cảnh báo "Cầu cảng chưa được phê duyệt, không khả dụng trong các module khác". Nếu approvalStatus = APPROVED, hiển thị "Cầu cảng đã được phê duyệt, đang khả dụng" | AC-024-03 | Không có ngoại lệ |
