...
- Chỉnh sửa thông tin cảng (thuộc F-111)
...
|---|---|---|---|
| BR-001 | LichSuThayDoi được tạo tự động khi tạo mới và cập nhật, ghi nhận tất cả field thay đổi | Lịch sử | F-114, F-013, INT-003 |
| BR-002 | loaiThayDoi = TẠO_MỚI khi tạo mới, CẬP_NHẬT khi sửa — hiển thị indicator trong bảng | Indicator | F-114 |
| BR-003 | changedAt sắp xếp DESC (mới nhất trước), hỗ trợ lọc theo field name | Sắp xếp & Lọc | F-114 |
| BR-004 | oldValue và newValue lưu trữ giá trị text (dạng serialized JSON) | Giá trị | F-114, F-013 |
