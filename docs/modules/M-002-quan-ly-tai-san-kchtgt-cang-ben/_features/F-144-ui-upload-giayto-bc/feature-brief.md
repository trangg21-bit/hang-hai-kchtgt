---
id: F-144
name: "Upload GiayTo Bến cảng"
slug: ui-upload-giayto-bc
module-id: M-002
status: proposed
classification: local
priority: medium
created: "2026-07-01T07:52:21Z"
last-updated: "2026-07-01T07:52:21Z"
locked-fields: []
consumed_by_modules: []
---

# Feature: Upload GiayTo Bến cảng

## Description

Tính năng Upload GiayTo Bến cảng cho phép người dùng đính kèm các văn bản liên quan (giấy tờ, hồ sơ, chứng từ) trực tiếp từ trang chi tiết Bến cảng (F-074). Giao diện bao gồm một nút "Thêm văn bản đính kèm" nằm trên trang thông tin chi tiết của Bến cảng; khi người dùng nhấn nút này, một modal upload file hiện ra, cho phép chọn file từ thiết bị, xem trước thông tin file (tên, kích thước, định dạng MIME) và xác nhận tải lên. Hệ thống cho phép tải lên file với bất kỳ MIME type nào, không giới hạn định dạng file cụ thể. Định danh của file (GiayTo) được lưu trữ cùng tham chiếu thực thể — entityType="ben-cang" (string) và entityId (string) của Bến cảng tương ứng — nhằm đảm bảo mọi tài liệu đính kèm luôn gắn liền với thực thể Bến cảng mà nó mô tả. Dữ liệu binary của file không được lưu thực tế vào MinIO mà sử dụng mock console-stub (đàm phán với W4 để tích hợp Tika kiểm tra magic-byte ở giai đoạn sau). Đây là một tính năng cục bộ, phục vụ nhu cầu quản lý hồ sơ điện tử cho Bến cảng trong hệ thống cảng hải quan một cách thống nhất.

## Business Intent

Cho phép người dùng gắn các giấy tờ chứng từ liên quan trực tiếp vào từng Bến cảng, tạo lập một kho lưu trữ hồ sơ điện tử tập trung, giảm thiểu việc sử dụng giấy tờ và đảm bảo tính truy xuất, kiểm toán của các tài liệu quản lý bến cảng.

## Flow Summary

Người dùng mở trang chi tiết Bến cảng (F-074), nhấn nút "Thêm văn bản đính kèm". Modal upload file hiện ra, người dùng chọn file từ thiết bị và nhấn "Tải lên". Client-side kiểm tra thông tin file (tên, kích thước, MIME type) trước khi gửi lên. File được gửi qua POST /api/v1/giay-to với FormData chứa file, entityType="ben-cang" và entityId (string của Bến cảng tương ứng). Server lưu metadata vào CSDL, và gọi MinIO stub (ghi log key ra console). Sau khi hoàn tất, hệ thống hiển thị toast "Upload thành công" và cập nhật danh sách file đính kèm ngay trên trang chi tiết. Người dùng có thể xem danh sách file đã upload, nhấn nút "Tải xuống" (GET /api/v1/giay-to/:id/download) để xem lại hoặc nhấn "Xóa" (DELETE /api/v1/giay-to/:id) để xóa nếu có quyền.

## Acceptance Criteria

1. Nút "Thêm văn bản đính kèm" hiển thị trên trang chi tiết Bến cảng (F-074) và mở modal upload khi được nhấn.
...
5. Sau khi upload thành công, metadata được lưu vào CSDL với entityType="ben-cang" và entityId (string) đúng của Bến cảng tương ứng; toast "Upload thành công" được hiển thị.
...
9. GET /api/v1/giay-to?entityType=ben-cang&entityId={entityId} trả về đúng danh sách các file đính kèm cho Bến cảng cụ thể.
...
- Giao diện nút "Thêm văn bản đính kèm" trên trang chi tiết Bến cảng (F-074).
...
- GET /api/v1/giay-to?entityType=ben-cang&entityId=string — liệt kê file đính kèm.
...
| Role | Level | Notes |
|---|---|---|
| Admin | upload + delete + view | Toàn quyền quản lý file đính kèm Bến cảng |
...
| Chuyên viên Cảng vụ | upload + view | Có thể upload và xem file đính kèm Bến cảng, không có quyền xóa |
| Doanh nghiệp cảng | upload + view | Có thể upload và xem file đính kèm liên quan đến Bến cảng của mình |
| Người dùng tại cảng | upload + view | Có thể upload và xem file đính kèm trong phạm vi Bến cảng được phân quyền |
...
| BR-03 | GiayTo phải được liên kết với một thực thể Bến cảng xác định thông qua entityType="ben-cang" và entityId (string) — không cho phép upload file không gắn với thực thể | POST | Entity constraint |
...
Kiểm thử sẽ bao gồm test đơn vị cho các service phương thức validateMime (chấp nhận bất kỳ MIME type nào) và validateFileSize (không có giới hạn max size từ BE), integration test cho POST /api/v1/giay-to với các trường hợp MIME hợp lệ (PDF, DOCX, JPEG, PNG, GIF, bất kỳ MIME nào), test RBAC cho từng role với các thao tác upload, delete và view, end-to-end test trên giao diện modal upload: mở trang chi tiết Bến cảng, nhấn nút upload, chọn file, kiểm tra toast thông báo, xác nhận metadata lưu đúng CSDL với entityType="ben-cang" và entityId (string), liệt kê file qua endpoint list, thực hiện download và delete. MinIO stub sẽ được kiểm tra bằng cách xác nhận log console được ghi đúng khi upload. Tất cả các test được thực hiện trên môi trường CI/CD với database in-memory.
