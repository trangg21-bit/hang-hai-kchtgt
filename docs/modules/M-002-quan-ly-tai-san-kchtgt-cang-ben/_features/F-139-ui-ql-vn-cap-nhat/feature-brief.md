...
priority: medium
created: "2026-07-01T07:52:12Z"
last-updated: "2026-07-01T07:52:12Z"
locked-fields: []
consumed_by_modules: []
---
...
Tính năng Cập nhật Vùng nước cung cấp giao diện form để người dùng chỉnh sửa thông tin của một vùng nước đã tồn tại trong hệ thống. Trang mở từ nút "Chỉnh sửa" trên trang Danh sách Vùng nước (F-136) hoặc trang Chi tiết (F-137). Form được pre-filled với toàn bộ dữ liệu hiện tại của vùng nước từ API `GET /api/v1/vung-nuoc/{id}`. Trường maVungNuoc được hiển thị dưới dạng readonly (không cho phép chỉnh sửa) để đảm bảo tính duy nhất của mã. Các trường còn lại (tenVungNuoc, loaiVungNuoc, cangBienId, dienTich, doSauMax, doSauTrungBinh, trangThaiHoatDong) cho phép chỉnh sửa theo cùng quy tắc validation như khi tạo mới (F-138): cangBienId dropdown chỉ hiển thị cảng có `trangThaiHoatDong = HIEN_HANH`, các trường số (dienTich, doSauMax, doSauTrungBinh) phải là giá trị BigDecimal hợp lệ. Khi click nút "Cập nhật", hệ thống gọi API `PUT /api/v1/vung-nuoc/{id}`. Sau khi cập nhật thành công, hệ thống tự động tạo bản ghi LichSuThayDoi (INT-003) ghi nhận các trường bị thay đổi, reset `trangThaiPheDuyet` về `CHỜ_PHÊ_DUYỆT` (vùng nước cần phê duyệt lại), hiển thị toast "Cập nhật thành công — chờ phê duyệt lại", và redirect về trang Danh sách Vùng nước (F-136).
...
7. Sau khi toast hiển thị, hệ thống tự động redirect về trang Danh sách Vùng nước (F-136).
...
- Xem lịch sử thay đổi chi tiết (thuộc F-142)
