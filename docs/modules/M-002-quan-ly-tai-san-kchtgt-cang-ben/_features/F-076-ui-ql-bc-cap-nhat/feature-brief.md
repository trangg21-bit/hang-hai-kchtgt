...
|---|---|---|---|
| BR-076-01 | Trường maBen không được phép chỉnh sửa — luôn hiển thị readonly (không có trong UpdateDTO). | UI logic | Unique constraint |
| BR-076-02 | Bến cảng cha (CangBien) phải có trạng thái HIEN_HANH; không cho phép chọn CangBien TAM_NGUNG. | Update | Parent guard |
| BR-076-03 | viDo phải nằm trong [-90, 90], kinhDo phải nằm trong [-180, 180]; validation client-side và server-side (chỉ áp dụng khi có giá trị). | Update | GPS validation |
| BR-076-04 | Sau khi cập nhật, trạng thái phê duyệt tự động reset về CHO_PHE_DUYET (yêu cầu phê duyệt lại). | Backend | F-015 backend |
| BR-076-05 | Mỗi lần cập nhật tạo bản ghi trong bảng LichSuThayDoi ghi lại fieldChanged, oldValue, newValue, changedBy, changedAt. | Backend | INT-003 |
