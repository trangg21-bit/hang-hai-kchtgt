...
Người dùng truy cập trang Chi tiết Cảng cạn (F-130) và bấm nút "Lịch sử" để mở trang Lịch sử Cảng cạn (F-135) với entityId tương ứng. Hệ thống gọi API GET /api/v1/cang-can/:id/history để tải tất cả LichSuThayDoi record của cảng cạn đó. Dữ liệu được hiển thị dưới dạng bảng phân trang với các cột: actionType (badge "Tạo mới" xanh hoặc "Cập nhật" vàng), field, oldValue, newValue, changedBy (tên người dùng), changedAt (định dạng ngày/giờ). Bảng mặc định sắp xếp theo changedAt DESC. Người dùng có thể lọc theo field bằng cách chọn từ dropdown bộ lọc — hệ thống gọi lại API với tham số filterField tương ứng. Mỗi hàng cho biết chính xác trường nào đã thay đổi, giá trị cũ là gì, giá trị mới là gì, ai là người thay đổi và khi nào thay đổi. Người dùng có thể bấm nút "Quay lại" để trở về trang Chi tiết (F-130).
...
6. Nhấp nút "Quay lại" hoặc breadcrumb "Chi tiết" điều hướng người dùng quay về trang Chi tiết Cảng cạn (F-130) với đúng entityId.
...
| Role | Level | Notes |
|---|---|---|
| NhanVienCangBien (Nhân viên cảng) | Read | Xem lịch sử thay đổi của Cảng cạn |
| QuanTriCangBien (Quản lý cảng biển) | Read | Xem lịch sử thay đổi của Cảng cạn |
| LanhDaoCuc | Read | Xem lịch sử thay đổi của Cảng cạn |
| Admin | Read | Xem lịch sử thay đổi của Cảng cạn |
...
|---|---|---|---|
| BR-135-01 | Mọi hành động Tạo mới và Cập nhật Cảng cạn phải được ghi nhận vào LichSuThayDoi record bởi backend | F-135, F-131, F-132 | Spec |
| BR-135-02 | Lịch sử thay đổi được sắp xếp theo changedAt giảm dần (mới nhất lên đầu) | F-135, INT-003 | Spec |
| BR-135-03 | Badge hành động phân biệt rõ: "Tạo mới" (CREATE, màu xanh) và "Cập nhật" (UPDATE, màu vàng) | F-135 | Spec |
| BR-135-04 | maCangCan phải là duy nhất trong toàn hệ thống; không cho phép tạo mới hoặc sửa có trùng mã | F-135, F-131, F-132 | Spec |
| BR-135-05 | Soft-delete: CangCan không có thực thể con nên khi xóa chỉ cần đặt deletedAt, không cần kiểm tra guard | F-134 | Spec |

## Testing Strategy

Kiểm thử đơn vị (unit test) tập trung vào component bảng Lịch sử: render đúng tất cả các cột (actionType badge, field, oldValue, newValue, changedBy, changedAt), badge màu đúng ("Tạo mới" xanh, "Cập nhật" vàng), component dropdown bộ lọc Field, component phân trang. Mock API response với các bản ghi CREATE và UPDATE khác nhau, xác nhận bảng hiển thị đúng thứ tự changedAt DESC. Kiểm thử tích hợp (integration test): gọi GET /api/v1/cang-can/:id/history, xác nhận danh sách LichSuThayDoi trả về chính xác; gọi với filterField, xác nhận chỉ trả về bản ghi của trường được chọn. Kiểm thử nghiệp vụ: tạo 1 cảng cạn (bản ghi CREATE đầu tiên), chỉnh sửa 2 trường (2 bản ghi UPDATE), xác nhận bảng có 3 bản ghi với đúng oldValue/newValue; lọc theo từng field — mỗi lần chỉ hiển thị bản ghi của field đó. Kiểm thử RBAC: tất cả các vai trò (NhanVienCangBien, QuanTriCangBien, LanhDaoCuc, Admin) đều có thể xem lịch sử thay đổi. Kiểm thử định dạng: changedBy resolve UUID → tên người dùng, oldValue/newValue null hiển thị "—".
