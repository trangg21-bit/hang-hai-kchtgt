...
priority: medium
created: "2026-07-01T07:52:20Z"
last-updated: "2026-07-01T07:52:20Z"
locked-fields: []
consumed_by_modules: []
---
...
5. Sau khi upload thành công, metadata được lưu vào CSDL với entityType="cang-bien" và entityId (string) đúng của Cảng biển; toast "Upload thành công" được hiển thị.
...
9. GET /api/v1/giay-to?entityType=cang-bien&entityId={entityId} trả về đúng danh sách các file đính kèm cho Cảng biển cụ thể.
...
- GET /api/v1/giay-to?entityType=cang-bien&entityId=string — liệt kê file đính kèm.
...
| BR-03 | GiayTo phải được liên kết với một thực thể Cảng biển xác định thông qua entityType="cang-bien" và entityId (string) — không cho phép upload file không gắn với thực thể | POST | Entity constraint |
...
Kiểm thử sẽ bao gồm test đơn vị cho các service phương thức validateMime (chấp nhận bất kỳ MIME type nào) và validateFileSize (không có giới hạn max size từ BE), integration test cho POST /api/v1/giay-to với các trường hợp MIME hợp lệ (PDF, DOCX, JPEG, PNG, GIF, bất kỳ MIME nào), test RBAC cho từng role với các thao tác upload, delete và view, end-to-end test trên giao diện modal upload: mở trang chi tiết Cảng biển, nhấn nút upload, chọn file, kiểm tra toast thông báo, xác nhận metadata lưu đúng CSDL với entityType="cang-bien" và entityId (string), liệt kê file qua endpoint list, thực hiện download và delete. MinIO stub sẽ được kiểm tra bằng cách xác nhận log console được ghi đúng khi upload. Tất cả các test được thực hiện trên môi trường CI/CD với database in-memory.
