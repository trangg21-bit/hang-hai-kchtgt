...
7. Cầu cảng bị xóa mềm biến mất khỏi danh sách F-122 và không còn hiển thị khi tìm kiếm bình thường.
8. Toast "Xóa cầu cảng thành công" hiển thị sau khi xóa thành công.
9. Danh sách F-122 được làm mới sau khi xóa để phản ánh thay đổi.
...
- Xóa cầu cảng từ trang chi tiết F-123 (thực hiện từ danh sách F-122).
...
| # | Rule | Description |
|---|---|---|
| 1 | Phân quyền xóa | Chỉ các role Leader (Linh dao) và Admin mới có quyền xóa cầu cảng — các role khác không thấy nút hành động xóa. |
| 2 | Xóa mềm | Xóa cầu cảng là xóa mềm (soft-delete): set deletedAt = current timestamp — bản ghi không bị xóa vật lý khỏi database, chỉ được ẩn khỏi danh sách hoạt động. |
| 3 | Không cần kiểm tra child | Không cần kiểm tra tồn tại child/entities con trước khi xóa vì CauCang là node lá (leaf node) — không có entity nào tham chiếu FK đến CauCang. |
| 4 | Dữ liệu vẫn tồn tại | Cầu cảng bị xóa mềm vẫn giữ nguyên toàn bộ dữ liệu và có thể được phát hiện qua API history hoặc database query trực tiếp. |
| 5 | Ghi nhận vào lịch sử | Mọi thao tác xóa được ghi nhận vào LichSuThayDoi của cầu cảng (INT-003) để phục vụ kiểm toán. |
| 6 | Xóa trùng lặp | Nếu cầu cảng đã có deletedAt != null (đã bị xóa trước đó), thao tác xóa thêm sẽ trả về lỗi hoặc bỏ qua tùy theo nghiệp vụ. |
