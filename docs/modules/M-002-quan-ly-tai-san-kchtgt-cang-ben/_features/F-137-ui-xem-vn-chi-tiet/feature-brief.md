...
priority: medium
created: "2026-07-01T07:52:10Z"
last-updated: "2026-07-01T07:52:10Z"
locked-fields: []
consumed_by_modules: []
---
...
Người dùng click vào tên vùng nước hoặc nút "Xem chi tiết" từ bảng Danh sách Vùng nước. Hệ thống gọi API `GET /api/v1/vung-nuoc/{id}` để lấy thông tin chi tiết của vùng nước. Đồng thời, hệ thống gọi API để lấy thông tin cảng mẹ (GET /api/v1/cang-bien/{cangBienId}) và hiển thị tên cảng dưới dạng liên kết. Nếu có quyền phê duyệt và trạng thái là CHO_PHE_DUYET, hệ thống hiển thị các nút "Phê duyệt" và "Từ chối". Nếu vùng nước có danh sách attachment, hệ thống gọi API `GET /api/v1/vung-nuoc/{id}/attachments` để lấy danh sách tài liệu, hiển thị tên tệp, định dạng, kích thước (MB) với nút tải xuống (download) và in (print) cho từng tệp. Breadcrumb hiển thị "Danh sách Vùng nước > Chi tiết {tenVungNuoc}". Người dùng click nút "Phê duyệt" hoặc "Từ chối" sẽ chuyển sang trang Phê duyệt Vùng nước (F-140) với ID tương ứng.
...
5. Nếu có danh sách attachment, hệ thống hiển thị tên tệp, định dạng, kích thước (MB), với nút "Tải xuống" và "In" cho từng tệp.
...
- Danh sách attachment với nút download và print
...
- Chỉnh sửa vùng nước trực tiếp trên trang chi tiết (thuộc F-139)
- Xóa vùng nước trực tiếp trên trang chi tiết (thuộc F-141)
- Quản lý attachment upload (thuộc tính năng quản lý tài liệu)
- Xem lịch sử thay đổi trên trang chi tiết (thuộc F-142)
...
| BR-04 | Attachment chỉ hiển thị nếu có | GET, Display | Attachment policy |
