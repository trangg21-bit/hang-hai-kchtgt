...
| | Items |
|---|---|
| In scope | Danh sách Cảng biển chờ phê duyệt (`PENDING_APPROVAL` — tạo mới + cập nhật); Trang chi tiết với lịch sử thay đổi; Giao diện Chấp thuận / Từ chối kèm confirmation dialog; Trường lý do từ chối (bắt buộc, tối thiểu 10 ký tự); Cập nhật `approval_status` của port; Ghi approval_log bất biến; Toast thông báo thành công/lỗi; Thông báo kết quả đến người tạo; Làm mới danh sách sau mỗi hành động |
| Out of scope | Phê duyệt xóa Cảng biển; Phê duyệt hàng loạt; Tự động phê duyệt; Phê duyệt đa cấp; Xuất báo cáo phê duyệt; Chỉnh sửa thông tin cảng (thuộc F-071); Xóa mềm cảng (thuộc F-093); Thông báo email khi phê duyệt/từ chối |
...
| AC-011-05 | US-011-03 | Chấp thuận thành công | Given Cảng biển trạng thái `PENDING_APPROVAL`; When người phê duyệt chọn Chấp thuận → confirmation dialog → xác nhận; Then `approval_status` chuyển `APPROVED`, approval_log ghi nhận, toast "Đã phê duyệt thành công", thông báo gửi người tạo, danh sách được làm mới | |
...
| AC-011-07 | US-011-04 | Từ chối thành công khi có lý do hợp lệ | Given người phê duyệt nhập lý do từ chối (>= 10 ký tự); When chọn Từ chối → confirmation dialog → xác nhận; Then `approval_status` chuyển `REJECTED`, approval_log ghi nhận kèm lý do, toast "Đã từ chối", thông báo kèm lý do gửi người tạo, danh sách được làm mới | |
...
|---|---|---|---|
| BR-011-01 | Cảng biển tạo mới hoặc cập nhật phải có trạng thái `PENDING_APPROVAL` trước khi chuyển sang `APPROVED` (Hiện hành) | Tất cả port | Không có ngoại lệ |
...
| BR-011-03 | Chỉ một cấp phê duyệt duy nhất; không áp dụng multi-level approval | Tất cả luồng phê duyệt | Không có ngoại lệ |
...
|---|---|---|---|
| GET | `/api/v1/ports?approval_status=PENDING` | Danh sách Cảng biển chờ phê duyệt | Lãnh đạo, Admin |
| POST | `/api/v1/ports/:id/approve` | Phê duyệt Cảng biển → `APPROVED`, tạo approval_log | Lãnh đạo, Admin |
| POST | `/api/v1/ports/:id/reject` | Từ chối Cảng biển → `REJECTED`, bắt buộc lý do >= 10 ký tự, tạo approval_log | Lãnh đạo, Admin |
...
| TS-011-04 | AC-011-05, AC-011-09 | Luồng chấp thuận: trạng thái → `APPROVED`, log ghi đúng, toast, thông báo gửi | Integration / E2E |
| TS-011-05 | AC-011-07, AC-011-09 | Luồng từ chối có lý do (>= 10 ký tự): trạng thái → `REJECTED`, toast, thông báo kèm lý do | Integration / E2E |
...
|---|---|---|
| Domain model affected? | Yes | approval_log là entity mới; trạng thái port có transition mới (`PENDING_APPROVAL → APPROVED / REJECTED`); workflow approval là bounded context mới |
