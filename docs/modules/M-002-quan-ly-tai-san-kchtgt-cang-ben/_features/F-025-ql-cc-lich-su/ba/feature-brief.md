...
Hiển thị toàn bộ lịch sử thay đổi của một Cầu cảng, bao gồm mọi thao tác: tạo mới (F-020), cập nhật (F-021), phê duyệt/từ chối (F-023), xóa mềm (F-022). Mỗi lần thay đổi được hiển thị dưới dạng **card box** riêng biệt, dễ đọc và truy vết.

### 1.2. Tại sao cần tính năng này?

Cung cấp khả năng kiểm toán (audit trail) cho mọi thay đổi đối với cầu cảng, cho phép quản lý viên và lãnh đạo xem lại ai đã thay đổi thông tin gì, khi nào, và từ giá trị nào sang giá trị nào — đảm bảo tính minh bạch, trách nhiệm giải trình và khả năng truy vết lỗi dữ liệu. Đây là yêu cầu bắt buộc trong quản lý hạ tầng cảng biển.

### 1.3. Luồng hoạt động chính

1. Người dùng truy cập trang chi tiết Cầu cảng (F-024) → click nút "Lịch sử".
2. Hệ thống gọi GET `/api/v1/cau-cang/:id/history`.
...
- **US-025-01:** Là Quản lý tài sản, tôi muốn xem toàn bộ lịch sử thay đổi của một cầu cảng để biết ai đã thay đổi gì và khi nào.
- **US-025-02:** Là Lãnh đạo, tôi muốn xem lịch sử phê duyệt/từ chối để kiểm tra quy trình đã được thực hiện đúng chưa.
- **US-025-03:** Là Kiểm toán viên, tôi muốn truy vết mọi thay đổi của cầu cảng để phục vụ công tác kiểm toán.

### Mức Should (nên có)

- **US-025-04:** Là Quản lý tài sản, tôi muốn lọc lịch sử theo khoảng thời gian hoặc người thực hiện để nhanh chóng tìm được thay đổi cần xem.

### Mức Could (có thể có sau)

- **US-025-05:** Là Admin, tôi muốn xuất báo cáo lịch sử thay đổi ra file Excel/PDF.

---

## 4. Yêu cầu chức năng (Acceptance Criteria)

**AC-025-01 — Hiển thị danh sách các lần thay đổi:** Danh sách hiển thị theo thứ tự thời gian giảm dần (mới nhất lên đầu). Mỗi lần thay đổi là một card box riêng biệt. Nếu không có lịch sử, hiển thị "Không có lịch sử thay đổi".

**AC-025-02 — Hiển thị metadata của mỗi lần thay đổi:** Mỗi card box hiển thị: thời gian cập nhật (định dạng `HH:mm:ss dd/MM/yyyy`), người cập nhật (họ tên).
...
**AC-025-04 — Lọc và tìm kiếm:** Người dùng có thể lọc theo khoảng thời gian (từ ngày → đến ngày), người thực hiện (dropdown), và loại hành động (TAO_MOI, CAP_NHAT, PHE_DUYET, TU_CHOI, XOA_MEM).

**AC-025-05 — Dữ liệu read-only:** Lịch sử là read-only, không thể chỉnh sửa hoặc xóa.

**AC-025-06 — Badge loại hành động:** Mỗi card box có badge màu phân biệt loại hành động: Tạo mới (xanh lá), Cập nhật (xanh dương), Phê duyệt (xanh dương đậm), Từ chối (đỏ), Xóa mềm (xám).

---

## 5. Quy tắc nghiệp vụ (Business Rules)

**BR-025-01 — Ghi nhận tự động mọi thay đổi:** Mọi thao tác trên Cầu cảng (tạo mới, cập nhật, phê duyệt, từ chối, xóa mềm) đều tự động tạo bản ghi LichSuThayDoi. Không có thay đổi nào bị bỏ qua.

**BR-025-02 — Lịch sử bất biến:** Dữ liệu lịch sử là read-only, không thể chỉnh sửa hoặc xóa bởi bất kỳ ai. Chỉ được bổ sung thêm.

**BR-025-03 — Lưu trữ vĩnh viễn:** Dữ liệu lịch sử được lưu trữ vĩnh viễn, phục vụ mục đích kiểm toán và tham khảo.

**BR-025-04 — Thông tin người thực hiện:** Tên người thực hiện được tự động lấy từ tài khoản đăng nhập, không thể giả mạo.

**BR-025-05 — Thay đổi quan trọng được làm nổi bật:** Các thay đổi quan trọng (phê duyệt, từ chối, thay đổi trạng thái) được đánh dấu nổi bật bằng badge màu riêng.

---

## 6. Mô hình dữ liệu

### 6.1. Bảng LichSuThayDoi — Nhật ký thay đổi

Bảng chính lưu toàn bộ lịch sử thay đổi của Cầu cảng.

| Trường | Kiểu | Mô tả |
|---|---|---|
| `id` | UUID | Định danh bản ghi |
| `cauCangId` | UUID (FK → CauCang) | Cầu cảng bị thay đổi |
...
**Các trường `fieldChanged` có thể xuất hiện** (toàn bộ trường có thể chỉnh sửa của CauCang — tham khảo F-021):

- Mã cầu cảng, Tên cầu cảng, Đơn vị quản lý, Thuộc cảng biển, Thuộc bến cảng, Thuộc luồng HH
- Địa điểm, Địa điểm chi tiết, Phân cấp công trình, Loại kết cấu, Công năng khai thác, Tình trạng
- Chiều dài, Chiều rộng, Độ sâu khu nước, Cao độ đáy bến, Cỡ tàu khai thác
- Thời điểm phê duyệt bảo trì, Thời điểm chấp thuận ATCT, Thời điểm kiểm định
- Số lượng CC đang khai thác/đã công bố/đang thỏa thuận, Sản lượng hàng thông qua
- Tiếp nhận tàu > QĐ công bố, Số văn bản, Ngày văn bản
- Thời điểm công bố mở, Quyết định công bố, Văn bản thỏa thuận đầu tư
- Trạng thái hoạt động, Trạng thái phê duyệt, Ghi chú
...
|---|---|---|---|
| GET | `/api/v1/cau-cang/:id/history` | Lấy danh sách lịch sử thay đổi của cầu cảng | Tất cả người dùng đã đăng nhập |
...
│  │ 17/07/2026       │  │                              │ │
│  │ Người cập nhật:  │  │   Cầu cảng số 1  →  Cầu      │ │
│  │ Nguyễn Văn A    │  │   cảng số 1A                 │ │
│  │                  │  │                              │ │
│  │                  │  │ Chiều dài:                  │ │
│  │                  │  │   150.5m  →  155.0m          │ │
│  └──────────────────┘  │                              │ │
...
- Nếu là thao tác **Tạo mới**: cột phải hiển thị "Tạo mới cầu cảng" + danh sách tất cả giá trị ban đầu.
- Nếu là thao tác **Xóa mềm**: cột phải hiển thị "Cầu cảng đã bị xóa mềm".
...
| Người thực hiện | Dropdown | Chọn từ danh sách người dùng đã từng thay đổi cầu cảng này |
| Loại hành động | Dropdown (multi-select) | Tạo mới / Cập nhật / Phê duyệt / Từ chối / Xóa mềm |
...
| Phê duyệt (PHE_DUYET) | Xanh dương đậm `#1565C0` |
| Từ chối (TU_CHOI) | Đỏ `#F44336` |
| Xóa mềm (XOA_MEM) | Xám `#9E9E9E` |

### 10.5. Màn hình Lịch sử Cầu cảng

1. **ScreenHeader:** breadcrumb "Quản lý KCHT Hàng Hải > Quản lý cầu cảng > [tên cầu cảng] > Lịch sử".
