# UI Specification: CangBien — Approve Page

## Page: Approve/Reject (Phê duyệt/Từ chối Cảng biển)

### 1. Component Structure
```
CangBienApprovalModal
├── ModalHeader (title: "Phê duyệt Cảng biển", subtitle: "[maCang] — [tenCang]")
├── ApprovalSummaryCard
│   ├── Thông tin: maCang, tenCang, tinhThanhPho
│   ├── GPS: viDo, kinhDo (if present)
│   ├── Diện tích, Khả năng tiếp nhận
...
- **ApprovalSummaryCard** (top, read-only): 2-column grid showing all key fields of the CangBien entity. Badge for current approval status.
...
|---|---|---|---|
| Approve | /api/v1/cang-bien/{id}/approve | POST | Body empty; userId from session auth |
| Reject | /api/v1/cang-bien/{id}/reject | POST | Query param: `?reason=...` (min 10 chars) |
...
### 7. Error Handling
- **403 Forbidden**: "Bạn không có quyền phê duyệt cảng biển này."
- **404 Not Found**: "Không tìm thấy cảng biển để phê duyệt."
- **422 Validation** (reason too short on reject): Inline error "Lý do từ chối tối thiểu 10 ký tự".
- **Concurrent modification** (record already approved/rejected): Toast "Cảng biển này đã được phê duyệt/từ chối trước đó." + close modal.
- **Success Approve**: Toast "Phê duyệt cảng biển thành công" + close modal + refresh list.
- **Success Reject**: Toast "Từ chối cảng biển thành công" + close modal + refresh list.
