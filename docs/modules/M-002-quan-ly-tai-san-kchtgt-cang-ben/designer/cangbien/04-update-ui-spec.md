# UI Specification: CangBien — Update Page

## Page: Update (Cập nhật Cảng biển)

### 1. Component Structure
```
CangBienUpdatePage
├── PageHeader (title, breadcrumbs, backBtn)
└── CangBienForm (React Hook Form + Zod — same layout as Create)
    ├── FormSection (info fields)
    │   ├── maCang (Input, readonly) ← immutable
    │   ├── tenCang (Input)
    │   └── tinhThanhPho (Input)
    ...
    │   └── trangThaiPheDuyet (Select, readonly—cannot change approval state)
...
| maCang | string (length 50) | Input (readonly) | **Immutable** — loaded from GET detail, disabled, not submitted |
| tenCang | string (length 255) | Input (text) | Optional edit, @Size(max=255) |
...
- **Key differences**:
  - `maCang`: Readonly text input (background light gray, shows original value).
  - `trangThaiPheDuyet`: Readonly text (Badge displayed, not editable).
  - Form pre-populated with GET /api/v1/cang-bien/{id} data on mount.
  - If GET fails (404): Show error screen with back button.
  - Only changed fields need to be sent to BE (UpdateCangBienRequest is partial — all fields optional except id).

### 5. API Integration
| Operation | Endpoint | Method | Request Shape |
|---|---|---|---|
| Get by ID (pre-populate) | /api/v1/cang-bien/{id} | GET | - |
| Update | /api/v1/cang-bien | PUT | { id, tenCang?, tinhThanhPho?, viDo?, kinhDo?, dienTich?, khaNangTiepNhan?, trangThaiHoatDong? } |
...
### 7. Error Handling
- **404**: "Không tìm thấy cảng biển để cập nhật." with back button.
- **409 Conflict** (if maCang is submitted despite readonly — should not happen, but defensive): Toast + inline error.
- **422 Validation** (BE): Same mapping as Create page. Note: `isGpsPaired` validation applies on update too.
- **No changes submitted**: Toast "Không có thay đổi nào được thực hiện." (preventive check on frontend before submit).
- **Success**: Toast "Cập nhật cảng biển thành công" + redirect to Detail page.

### 8. Accessibility
- Keyboard: Identical tab order to Create page, but maCang is skipped (readonly, focus=false).
- Readonly fields: `aria-readonly="true"` on maCang input and trangThaiPheDuyet display.
