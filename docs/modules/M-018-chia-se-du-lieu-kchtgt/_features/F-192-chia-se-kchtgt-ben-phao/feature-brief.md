---
id: F-192
name: Chia se KCHTGT Ben phao
slug: chia-se-kchtgt-ben-phao
module-id: M-018
status: proposed
...
# Feature: Chia se KCHTGT Ben phao

## Description

Chia sẻ thông tin hạ tầng giao thông vận tải đường thủy về bến phao (địa điểm, tọa độ, loại phao, kích thước, khả năng tiếp nhận tàu) qua Trục liên thông dữ liệu quốc gia LGSP.

## Business Intent

Cho phép các cơ quan quản lý nhà nước, đơn vị khai thác cảng và tổ chức liên quan tiếp cận nhanh chóng thông tin bến phao phục vụ công tác điều hành giao thông đường thủy, quy hoạch hạ tầng và ứng phó sự cố. Dữ liệu được chuẩn hóa để đảm bảo tính nhất quán và khả năng trao đổi liên ngành giữa Cục Hàng hải, Chi cục Hàng hải, cảng vụ và các đơn vị kiểm tra chuyên ngành.

## Flow Summary

Dữ liệu bến phao được trích xuất từ hệ thống quản lý hạ tầng giao thông vận tải đường thủy của đơn vị chủ quản, sau đó chuyển đổi sang định dạng chuẩn theo schema quy định của Trục LGSP. Hệ thống dịch vụ API sẽ publish dữ liệu lên cổng chia sẻ, trong đó bao gồm thông tin mã phao, tên phao, tọa độ địa lý, loại phao (phao neo đậu, phao tiêu, phao dẫn đường), kích thước, khả năng tiếp nhận tàu và tình trạng hoạt động. Các đơn vị tiêu thụ (cảng vụ, kiểm tra chuyên ngành, cơ quan quản lý vùng biển) có thể gọi API hoặc subscribe qua cơ chế publish/subscribe của LGSP để nhận dữ liệu theo chu kỳ định kỳ hoặc theo yêu cầu. Toàn bộ luồng dữ liệu được bảo mật bằng HTTPS, xác thực JWT và kiểm soát truy cập theo danh sách IP whitelist.

## Acceptance Criteria

- Hệ thống API publish dữ liệu bến phao lên Trục LGSP thành công, phản hồi HTTP 200 với payload JSON đúng schema
- Dữ liệu hiển thị chính xác các trường: mã phao, tên phao, tọa độ, loại phao, kích thước, khả năng tiếp nhận tàu, tình trạng hoạt động
...
- API endpoint publish dữ liệu bến phao lên Trục LGSP
...
| Role | Permissions |
|------|-------------|
| Khách truy cập (Public) | Xem thông tin bến phao công khai |
| Nhân viên quản lý hạ tầng | Tạo, cập nhật, xóa thông tin bến phao |
...
- **BenPhao**: id, maPhao, tenPhao, tinhThanh, quanHuyen, xaPhuong, toaDo, loaiPhao, kichThuoc, khaNangTiepNhan, tinhTrangHoatDong, createdAt, updatedAt

## Business Rules

1. Dữ liệu bến phao chỉ được chia sẻ sau khi đã được kiểm tra tính hợp lệ (validation) về định dạng và nội dung
...
- Unit test: kiểm tra các hàm chuyển đổi dữ liệu bến phao sang schema LGSP
