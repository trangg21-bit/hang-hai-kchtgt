...
**Role tag helper:** `getRoleTagClass(roleId)` maps role IDs to `.role-tag--*` CSS classes (admin, org-admin, manager, viewer, user).
...
|---|---|---|---|
| `UsersPage` | **Modified** | Loading, Error, Empty (empty-data + no-results), Success | N/A — unit tests outside scope per task brief |
...
No new test files — unit tests are outside the defined scope of this refactor task. The modal form validation logic was NOT modified (kept identical to original).
...
1. **FilterBar field types** — The `FilterBar` component expects `type: 'search' | 'select'` field configs. The actual `FilterBar.tsx` must support these types for the search field (type `'search'`) and dropdowns (type `'select'`) to render correctly.

2. **StatusTabs color tokens** — `StatusTabs` receives `color` props (`statusOperational`, `statusCritical`, `statusDraft`). The component must accept and render these semantic color tokens.
...
5. **`handleTableChange` callback** — Defined in the spec but the current `DataTable` uses `onSort` internally for sorting, making `handleTableChange` effectively a no-op. This is dead-code-safe and does not affect functionality.

6. **Out of scope** — No new unit tests written for `UsersPage`; test infrastructure is outside the defined scope of this refactor task.
