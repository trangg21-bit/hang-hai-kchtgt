...
**Tài liệu:** Tài liệu chức năng — phần riêng (theo mẫu `docs/feature-brief-template.md`) — **DRAFT chờ duyệt, chưa scaffold** (feature id `F-XXX` tạm thời; sẽ cấp khi tạo folder trong `docs/modules/M-017-thong-ke-chuyen-de/_features/`)
**Chức năng:** F-XXX (dự kiến)
...
- Tài liệu nền `ba/01-base-pattern.md` của M-017: **chưa tồn tại** (M-017 hiện chỉ có module-brief + các feature-brief biểu cũ) — phần CHUNG của module sẽ bổ sung khi scaffold; brief này chỉ ghi phần RIÊNG theo Excel #33.

> ⚠️ **Trạng thái draft:** toàn bộ mục 6–7 là đề xuất của BA, SA chốt khi scaffold. Các ô trong Excel không đọc/đối chiếu chắc tới từng ô → ghi rõ **UNRESOLVED**, không bịa.
...
- Sau khi được duyệt (ban hành), số liệu là **đầu vào nguồn** cho các báo cáo khối lượng hàng hóa của M-017 (xem `overlap-analysis.md`: F-165/166/168/169).
