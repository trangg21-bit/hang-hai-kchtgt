...
## Description
Chức năng cho phép Chuyên viên viên chức hàng hải xem và quản lý lịch sử thay đổi của Đèn biển qua từng phiên bản. Tính năng hiển thị bảng chronological các lần cập nhật với thông tin chi tiết về người thực hiện, thời gian, trường đã thay đổi, giá trị cũ và giá trị mới, hỗ trợ so sánh side-by-side giữa các phiên bản và khôi phục về phiên bản trước đó khi cần thiết.

## Business Intent
Hệ thống cần cung cấp cơ chế theo dõi và kiểm soát thay đổi đầy đủ cho mỗi Đèn biển, cho phép回溯 mọi lần sửa đổi về thông tin kỹ thuật, đảm bảo tính minh bạch, trách nhiệm giải trình và khả năng khôi phục dữ liệu khi có sai sót, phục vụ công tác thanh tra, kiểm toán và phân tích diễn biến thông tin Đèn biển theo thời gian.

## Flow Summary
Chuyên viên truy cập trang chi tiết Đèn biển, chuyển sang tab "Lịch sử thay đổi". Hệ thống hiển thị bảng chronological liệt kê tất cả các lần cập nhật từ khi tạo đến hiện tại, mỗi dòng hiển thị: số thứ tự phiên bản, ngày giờ thay đổi, tên người thực hiện, danh sách các trường đã thay đổi (badge màu). Người dùng có thể click vào từng phiên bản để xem chi tiết side-by-side (trước/sau) các giá trị đã thay đổi. Tính năng tìm kiếm và lọc theo ngày, người thực hiện, hoặc trường thay đổi cũng được cung cấp.
...
## In Scope
- Bảng lịch sử thay đổi chronological của Đèn biển
...
## Architecture Notes
Lịch sử thay đổi được lưu trữ trong bảng `BeaconChangeLog` với trigger sau mỗi lần UPDATE trên bảng `beacons`. Endpoint REST API `GET /api/v1/beacons/{beaconId}/changelog` trả về danh sách chronological. Tính năng so sánh side-by-side sử dụng endpoint `GET /api/v1/beacons/{beaconId}/changelog/{changeId}/diff`. Frontend hiển thị timeline component với khả năng click để expand chi tiết.

## Entities
- **BeaconChangeLog**: id, beaconId, versionNumber, changedBy, changedAt, fieldName, oldValue, newValue, reason, diffJson
- **BeaconVersion**: id, beaconId, versionNumber, snapshotData, createdAt, createdBy

## Business Rules
1. Mọi lần cập nhật Đèn biển đều tự động tạo bản ghi trong bảng BeaconChangeLog (không thể bỏ qua)
2. Version number tăng dần từ 1, không có số trùng lặp cho cùng một Đèn biển
...
## Testing Strategy
- Unit test cho hàm tự động tạo bản ghi change log mỗi khi update entity Beacon
- Integration test cho API endpoint `GET /api/v1/beacons/{beaconId}/changelog` kiểm tra chronological order, filtering, pagination
- Integration test cho API endpoint diff kiểm tra hiển thị đúng giá trị trước/sau
- End-to-end test kiểm tra toàn bộ quy trình: tạo Đèn biển → cập nhật → xem lịch sử → click vào phiên bản để xem chi tiết so sánh
