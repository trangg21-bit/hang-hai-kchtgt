...
|---|---|---|---|---|---|
| 1 | _(empty)_ | `loai` | 150px | plain text | KCHT type name |
| 2 | Tổng số lượng | `tongSL` | 80px | `fontWeight=600, fontFamily=fontMono` | Total count, mono font |
| 3 | Chưa khai thác / vận hành | `chuaKhaiThac` | 70px | `pillBadge(v, sea0, sea0+18, surface)` | Not-yet-exploited count |
| 4 | Đang khai thác / vận hành | `dangKhaiThac` | 70px | `pillBadge(v, surface, sea0, surface)` | Active exploitation count |
| 5 | Dừng khai thác / vận hành | `dungKhaiThac` | 70px | `pillBadge(v, sea0, sea3, surface)` | Stopped exploitation count |
| 6 | (eye icon) | — | 40px | `<EyeOutlined />` | Action column, layout placeholder |

**BA Decision (2026-07-13):** Colored dot indicators removed from column headers. Column titles expanded to full text with line break for clarity.
...
| AC-3 | Cột trạng thái có badge màu: xanh (đang vận hành), vàng (chưa khai thác), đỏ (dừng) | Three exploitation status columns with pill badges using token colors | ✅ Pass — colors confirmed as sea-blue palette per token system. Feature brief green/yellow/red is obsolete. | `pillBadge()` function in `Home.tsx` |
