...
| Null handling | Months 08–12 render as 0 when current year data only covers 7 months. For in-progress month, value = cumulative total up to current date within that month. |
...
> **BA Decision (2026-07-13):** Polar bar is the correct implementation. Feature brief's line chart specification is obsolete — update brief, not code.
...
flowchart TD
    ...
    D -->|No| F[Return 0 for that month (not MOCK_DATA fallback)]
...
|---|---|---|---|---|
| **G-001** | Cargo stacked bar | CargoAggregate entity has NO cargo-type field. The stacked bar needs 6 cargo types (Nội địa, Xuất khẩu, Nhập khẩu, Chuyển tải, Quá cảnh bốc dỡ, Quá cảnh K bốc dỡ) but the entity only stores `totalTons` without type breakdown. Currently 100% mock data. | 🔴 Blocking | **Unresolved — backend must add cargoType field. Until resolved: use mock ratios.** |
| **G-002** | Passenger polar bar | No passenger direction field (arrival vs departure) in CargoAggregate. The polar bar needs 2 series but only gets one aggregate total. | 🟡 High | **Unresolved — backend must add direction field. Until resolved: use mock split.** |

### 7.2 Additional F-282-Specific Gaps

| Gap ID | Chart | Issue | Severity | Detail |
|---|---|---|---|---|
| **G-010** | Cargo stacked bar | Transform pipeline produces **4-series** output but Home.tsx renders **6-series** inline constant. The 2 extra series (`Quá cảnh bốc dỡ`, `Quá cảnh K bốc dỡ`) have no API pipeline path and no mock ratio assignment. | 🔴 Blocking | **RESOLVED by BA decision — stacked bar now has 6 series by design. Transform pipeline should be updated to produce 6 series when backend G-001 is resolved. For now: 6 series rendered from inline CARGO_SERIES constant.** |
| **G-011** | Both charts | Home.tsx does NOT use `TrendChartCard` wrapper. Loading/empty/error states are not implemented for the chart blocks. Currently only mock fallback at API level; no skeleton or empty state UI. | 🟡 High | **Deferred to Phase 2 — TrendChartCard exists but not wired.** |
| **G-012** | Passenger polar bar | Feature brief specifies a **line chart** but implementation is a **polar bar chart**. These are different visualizations with different data interpretation. | 🟡 Medium | **RESOLVED by BA decision — polar bar is correct. Update feature brief.** |
| **G-013** | Cargo stacked bar | Feature brief specifies **4 cargo types** with hex colors; implementation has **6 cargo types** via `cargoSeriesColors` tokens. Feature brief AC #1 says "Stacked bar 4 chuỗi đúng màu" — this AC is obsolete. | 🟢 Low | **RESOLVED by BA decision — 6 cargo types confirmed. Update feature brief.** |
| **G-014** | Cargo stacked bar | Feature brief claims "khe 2px giữa các phân đoạn" (2px gap between segments). Actual code uses ECharts `stack: 'total'` with `barWidth: '58%'` and no gap configuration. No `barGap` or `barCategoryGap` is set. | 🟢 Low | **RESOLVED by BA decision — 2px gap is optional. Not blocking.** |
| **G-015** | Both charts | The 10+ cargo/passenger entities with `periodType=MONTHLY` returned by E2 may not be filtered by year — the `filter(c.periodStart.startsWith(String(year)))` in `fetchCargoMonthly()` is client-side. Backend may return all years. | 🟡 Low | **Observation — verify backend pagination during dev testing.** |
...
|---|---|---|---|---|
| AC-01 | Stacked bar 6 chuỗi đúng màu, bo góc, khe 2px | §2.3 Cargo option | ✅ Pass | 6 series confirmed, 2px gap waived |
| AC-02 | Polar bar 2 stacked bars đúng màu, bo mượt | §2.4 Passenger option | ✅ Pass | polar bar confirmed as correct |
...
| AC-05 | Empty: tháng chưa có dữ liệu → không vẽ nửa biểu đồ trắng | §5.1 transform pipeline | ✅ Pass — null months now render as 0 per BA decision | Null months in CARGO_SERIES (08-12 = null); ECharts skips null data points |
