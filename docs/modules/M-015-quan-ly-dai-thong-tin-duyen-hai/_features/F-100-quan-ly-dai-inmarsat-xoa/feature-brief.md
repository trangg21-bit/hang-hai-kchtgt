...
consumed_by_modules: []
---
# Feature: Quản lý Đại Inmarsat - Xóa

## Description
Cho phép Chuyên viên xóa (hoặc vô hiệu hóa) một Đại Inmarsat (trạm vệ tinh Inmarsat) đã được phê duyệt trong hệ thống. Việc xóa chỉ được thực hiện trên các bản ghi đã hoàn tất quy trình phê duyệt, với cơ chế soft delete để bảo toàn dữ liệu lịch sử cho mục đích kiểm toán.

## Business Intent
Việc xóa Đại Inmarsat cần phải được kiểm soát nghiêm ngặt, chỉ thực hiện trên dữ liệu đã được phê duyệt để đảm bảo không có thông tin nào bị xóa trái phép. Sử dụng soft delete để duy trì tính toàn vẹn của dữ liệu lịch sử, hỗ trợ hoạt động kiểm toán và phân tích sau này.

## Flow Summary
Chuyên viên truy cập danh sách Đại Inmarsat, chọn bản ghi cần xóa, hệ thống kiểm tra điều kiện (bản ghi phải ở trạng thái "Đã phê duyệt"), xác nhận xóa bằng popup xác nhận, hệ thống chuyển trạng thái bản ghi thành "Đã xóa" (soft delete) và ghi nhận vào lịch sử kiểm toán.

## Acceptance Criteria
- Chỉ có thể xóa bản ghi Đại Inmarsat ở trạng thái "Đã phê duyệt"
...
## In Scope
- Giao diện chọn và xóa Đại Inmarsat
...
## Architecture Notes
Xóa mềm (soft delete) bằng cách set flag `is_deleted = true` và `deleted_at = timestamp` trên bản ghi `coastal_station_inmarsat`. Query mặc định thêm điều kiện `WHERE is_deleted = false`. Bảng `coastal_station_inmarsat_history` vẫn ghi nhận sự kiện xóa để phục vụ kiểm toán.

## Entities
- **CoastalStationInmarsat**: id, device_code, station_name, modem_type, frequency, coverage_zone, sar_code, location_address, contact_person, contact_phone, status, is_deleted, deleted_by, deleted_at
