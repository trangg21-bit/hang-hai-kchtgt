...
consumed_by_modules: []
---
# Feature: Phê duyệt Đại TTDH

## Description
Cho phép lãnh đạo cấp Phòng và cấp Cục thực hiện quy trình phê duyệt hai cấp cho các yêu cầu tạo mới, cập nhật hoặc xóa Đại Thông tin Duyên Hải (VTS station). Hệ thống quản lý luồng phê duyệt tuần tự từ Phòng lên Cục, với đầy đủ chức năng xem chi tiết, bình luận và ghi chú phê duyệt.

## Business Intent
Quy trình phê duyệt hai cấp (Phòng → Cục) đảm bảo mọi thay đổi về thông tin Đại TTDH đều được kiểm tra và chấp thuận bởi các cấp quản lý có thẩm quyền, đảm bảo tính chính xác và an toàn của dữ liệu hệ thống thông tin duyên hải.
...
## Entities
- **CoastalStationVTS**: id, station_code, station_name, status (pending/approved/rejected), version
