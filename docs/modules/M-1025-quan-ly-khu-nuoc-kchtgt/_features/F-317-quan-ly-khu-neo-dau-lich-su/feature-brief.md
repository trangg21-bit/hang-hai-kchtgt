---
id: F-317
name: "Lịch sử Khu neo đậu"
slug: quan-ly-khu-neo-dau-lich-su
module-id: M-1025
status: proposed
...
# Đặc tả nghiệp vụ: Lịch sử Khu neo đậu

**Tài liệu:** Tài liệu chức năng — phần riêng (theo mẫu 7 section).
**Chức năng:** F-317 — Lịch sử Khu neo đậu (Anchorage).
...
Chức năng cho phép người dùng có `anchorage:history` xem lịch sử thay đổi và phê duyệt của hồ sơ Khu neo đậu. Lịch sử chỉ hiển thị thay đổi ĐÃ duyệt (approved-only): bản ghi Nháp/Lưu tạm không xuất hiện; chỉ khi hồ sơ đã duyệt và sửa thành công ("Lưu và phê duyệt") mới ghi bản cũ vào nhật ký.
...
| ID | Quy tắc | Áp dụng |
|---|---|---|
| BR-317-01 | Lịch sử chỉ hiển thị thay đổi ĐÃ duyệt. | History |
| BR-317-02 | Nháp/Lưu tạm không tạo/không hiển thị bản ghi. | History |
| BR-317-03 | Data scope theo đơn vị. | History |

### 4.2. Acceptance Criteria

| AC-ID | Given | When | Then | Oracle |
|---|---|---|---|---|
| AC-317-01 | Hồ sơ đã duyệt + sửa "Lưu và phê duyệt" | Mở Lịch sử | Hiển thị bản cũ + người + thời điểm | `infrastructure_history` có bản ghi |
| AC-317-02 | Hồ sơ Nháp/Lưu tạm | Mở Lịch sử | Không hiển thị bản ghi | Không có history |
| AC-317-03 | Thiếu `anchorage:history` | GET | 403; UI ẩn nút | Permission khớp |

### 4.3. User Stories

- **US-317-01:** Là Chuyên viên, tôi muốn xem lịch sử thay đổi đã duyệt của hồ sơ để truy vết.
- **US-317-02:** Là Admin Cục, tôi muốn xem lịch sử đầy đủ.

### 4.4. Phân quyền riêng

| Thao tác | Quyền (`<resource>:<action>`) |
|---|---|
| Xem lịch sử | `anchorage:history` |

| Vai trò | Xem lịch sử | Ghi chú |
|---|---|---|
| Chuyên viên thuộc đơn vị | Có nếu gán `anchorage:history`, theo scope | — |
...
**Admin Cục:** xem lịch sử toàn phạm vi Cục khi có `anchorage:history` hoặc `admin:all`/`*`.
...
| 5 | Quyền riêng | `anchorage:history`. |
...
|---|---|---|---|
| GET | `/api/v1/anchorage/{id}/history` | Lịch sử hồ sơ | `anchorage:history` |
| GET | `/api/v1/anchorage/history/all` | Lịch sử toàn module | `anchorage:history` |
