# UI Specification: CangBien — History Page

## Page: History (Lịch sử thay đổi Cảng biển)

### 1. Component Structure
```
CangBienHistoryPage
├── PageHeader (title: "Lịch sử thay đổi", breadcrumbs, backBtn)
├── EntitySummaryCard (read-only: maCang, tenCang, tinhThanhPho)
└── HistoryTimeline
...
### 4. Form/Table Layout
- **PageHeader**: Title "Lịch sử thay đổi — [maCang] | [tenCang]", breadcrumbs (Trang chủ > Quản lý tài sản > Cảng biển > Lịch sử), Back button.
- **EntitySummaryCard** (top, read-only): Compact display of maCang, tenCang, tinhThanhPho with badge for current status.
...
|---|---|---|---|
| Get History | /api/v1/cang-bien/{id}/history | GET | - |
...
### 7. Error Handling
- **404 Not Found**: "Không tìm thấy cảng biển." with back button.
