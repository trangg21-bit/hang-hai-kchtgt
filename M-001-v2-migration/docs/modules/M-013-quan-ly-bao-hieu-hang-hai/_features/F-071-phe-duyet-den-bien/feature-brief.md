...
## Description
Chức năng cho phép Lãnh đạo cấp Phòng và cấp Cục thực hiện phê duyệt hồ sơ Đèn biển theo quy trình 2 cấp. Trưởng phòng xem xét và phê duyệt hồ sơ ở cấp độ cơ sở, sau đó Chuyển hồ sơ lên Lãnh đạo Cục để phê duyệt cấp cao hơn. Mỗi cấp có bảng danh sách công việc riêng và khả năng chấp nhận hoặc từ chối hồ sơ kèm lý do chi tiết.

## Business Intent
Hệ thống cần đảm bảo mọi Đèn biển mới tạo, cập nhật hoặc đề nghị xóa đều phải trải qua quy trình kiểm tra độc lập bởi ít nhất 2 cấp quản lý, đảm bảo thông tin kỹ thuật chính xác, vị trí hợp lý, và tuân thủ quy chuẩn kỹ thuật quốc gia về hệ thống báo hiệu hàng hải trước khi được đưa vào hoạt động chính thức trên hệ thống quản lý.

## Flow Summary
Trưởng phòng truy cập danh sách công việc "hồ sơ chờ phê duyệt", chọn hồ sơ Đèn biển cần xem xét, hệ thống hiển thị chi tiết thông tin kỹ thuật của Đèn biển kèm thông tin người nộp và thời gian nộp. Trưởng phòng kiểm tra và chọn "Phê duyệt cấp 1" hoặc "Từ chối" kèm lý do. Nếu phê duyệt, hồ sơ chuyển đến hàng đợi phê duyệt của Lãnh đạo Cục. Lãnh đạo Cục thực hiện tương tự ở cấp 2. Sau khi cả 2 cấp phê duyệt, Đèn biển chuyển sang trạng thái "approved" và xuất hiện trong danh sách hoạt động. Nếu từ chối ở bất kỳ cấp nào, hồ sơ quay lại trạng thái "rejected" và gửi thông báo cho người nộp.

## Acceptance Criteria
- Trưởng phòng có thể xem danh sách tất cả hồ sơ Đèn biển chờ phê duyệt cấp 1
...
- Hiển thị chi tiết hồ sơ Đèn biển kèm thông tin kỹ thuật đầy đủ
...
## Architecture Notes
Workflow phê duyệt được implement bằng State Machine Pattern với enum `ApprovalStage { PENDING, APPROVED_LEVEL_1, APPROVED, REJECTED }`. Endpoint REST API `POST /api/v1/beacons/{beaconId}/approve` và `POST /api/v1/beacons/{beaconId}/reject`. Notification service gửi email và thông báo trong hệ thống khi trạng thái thay đổi. Audit trail ghi lại mọi hành động phê duyệt/từ chối ở cả 2 cấp.

## Entities
- **Beacon**: id, beaconCode, name, latitude, longitude, beaconType, lightCharacteristic, color, period, luminousRange, powerSource, status, approvalStage, approvedByLevel1, approvedByLevel2, approvedAt, rejectedBy, rejectedReason, rejectedAt, createdAt, updatedAt
- **ApprovalAuditLog**: id, beaconId, stage, action (approve/reject), performedBy, performedAt, comment
...
- Integration test cho API endpoints `POST /api/v1/beacons/{beaconId}/approve` và `reject` kiểm tra workflow 2 cấp
