...
- Quy chuẩn màu sắc StatusTabs & Badge (chuẩn Quản lý khu neo đậu): `Tất cả` (`#0E6FD6`), `Lưu tạm` (`#93A3B3`), `Chờ phê duyệt cấp Cảng vụ/Chi cục` (`#204E9C`), `Chờ phê duyệt cấp Cục` (`#EDA100`), `Đã phê duyệt` (`#1BAF7A`), `Từ chối cấp Cảng vụ/Chi cục` (`#E34948`), `Từ chối cấp Cục` (`#E34948`), `Đã xóa` (`#E34948`). Số lượng trên tab Tất cả bắt buộc bằng tổng số các tab con (`Tất cả = Lưu tạm + Chờ phê duyệt cấp Cảng vụ/Chi cục + Chờ phê duyệt cấp Cục + Đã phê duyệt + Từ chối C1 + Từ chối C2 + Đã xóa`).
- **Quy chuẩn hiển thị Tab "Tất cả" bao gồm bản ghi Đã xóa (MANDATORY)**: Khi đứng ở tab "Tất cả", bảng dữ liệu **BẮT BUỘC hiển thị toàn bộ bản ghi của mọi trạng thái, bao gồm cả các bản ghi có trạng thái Đã xóa (`ARCHIVED` / soft-deleted)**, khớp chính xác với số lượng tổng trên badge của tab Tất cả (TUYỆT ĐỐI KHÔNG để bảng rỗng khi có bản ghi đã xóa). Ở Backend JPQL, nhánh `:approvalStatus IS NULL` bắt buộc là `(:approvalStatus IS NULL)` trần, cấm nối thêm `AND deletedAt IS NULL` hay `AND approvalStatus != ARCHIVED`. Ở Frontend, khi `activeTab === 'all'` truyền `approvalStatus: undefined`, `isDeleted: undefined` và cấm client-side filter loại bỏ bản ghi đã xóa.
...
- **Quy chuẩn ô văn bản 2000 / 4000 ký tự (Phạm vi áp dụng, Thông báo hàng hải, Vùng phủ sóng, Ghi chú...)**: **BẮT BUỘC** thiết kế dưới dạng **ô `Input.TextArea` nhiều dòng kéo dài xuống dưới** (`rows={3}`, `maxLength={2000}` hoặc `4000`, `showCount`, `style={textAreaStyle}`), **kéo dài toàn bộ chiều rộng form (`<Col span={24}>`)** để người dùng thuận tiện nhập và xem trọn vẹn văn bản dài (TUYỆT ĐỐI KHÔNG để dồn 1 bên `<Col span={12}>` làm trống nửa màn hình).
...
1. **Nguyên tắc chung**:
   ...
   - **GIT — QUY TRÌNH COMMIT THEO STAGE (SỬA ĐỔI 2026-09-06)**: Mặc định AI **KHÔNG tự ý** `git push`/`merge` — mọi thay đổi giữ local. Tuy nhiên, vì gate `ai-kit-verify --as-gate` chấm theo baseline đã commit (git HEAD) chứ không phải working tree, **pmo-software-project-manager (lead module) ĐƯỢC phép tự `git add` + `git commit` local theo từng stage** trước khi chạy gate, theo đúng ràng buộc: (a) chỉ stage đúng file thuộc phạm vi stage/module đang xử lý (không `git add .`), (b) chạy `git diff --cached --check`, (c) commit message ghi rõ module + stage, (d) tuyệt đối không push/merge/amend/tag. Build seat (receptionist) chỉ commit khi user ra lệnh trực tiếp. User vẫn kiểm soát cuối bằng việc quyết định push lên remote.
