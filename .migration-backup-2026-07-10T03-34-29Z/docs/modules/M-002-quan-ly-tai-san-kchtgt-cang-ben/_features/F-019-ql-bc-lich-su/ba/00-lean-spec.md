...
---
# Quản lý Bến cảng - Lịch sử

## Summary

Hệ thống cần ghi nhận và cho phép truy xuất toàn bộ lịch sử thay đổi của một Bến cảng (tạo mới, cập nhật, phê duyệt, xóa) nhằm đảm bảo tính minh bạch và phục vụ kiểm toán. Tính năng tổng hợp các sự kiện từ các tính năng F-014, F-015, F-016, F-017 vào một dòng thời gian thống nhất, với thông tin chi tiết giá trị cũ/mới cho từng trường thay đổi. Thành công đo bằng: mọi thay đổi Bến cảng đều được ghi nhận đầy đủ, không có bản ghi bị thiếu hay bị sửa/xóa.

## Scope

| | Items |
|---|---|
| In scope | Trang hiển thị lịch sử chronologically; chi tiết sự kiện (loại, trường thay đổi, giá trị cũ/mới, người thực hiện, thời gian); lọc theo loại sự kiện / người thực hiện / khoảng thời gian; tích hợp sự kiện từ F-014 F-015 F-016 F-017 |
...
| Assumptions | Bảng `lich_su_thay_doi` đã được thiết kế với cấu trúc phù hợp; các tính năng nguồn (F-014–F-017) ghi sự kiện vào bảng này tại thời điểm thực hiện; người dùng đã xác thực |

## User Stories

| US-ID | Actor | Goal | Value | Priority |
|---|---|---|---|---|
| US-001 | Quản trị viên / Quản lý cảng | Xem danh sách lịch sử thay đổi của một Bến cảng sắp xếp mới nhất lên đầu | Truy xuất nguồn gốc dữ liệu nhanh, phục vụ kiểm toán | Must Have |
...
|---|---|---|---|---|
| AC-001 | US-001 | Truy cập trang lịch sử thành công | Given người dùng có role Quản trị viên hoặc Quản lý cảng đã đăng nhập; When truy cập trang lịch sử của Bến cảng bất kỳ; Then danh sách sự kiện hiển thị đầy đủ sắp xếp thời gian giảm dần | Tất cả 4 loại sự kiện (tao_moi, cap_nhat, phe_duyet, xoa) phải xuất hiện nếu đã xảy ra |
...
| AC-008 | US-004 | Nhân viên vận hành xem lịch sử với trường kỹ thuật bị ẩn | Given người dùng role Nhân viên vận hành; When xem danh sách lịch sử; Then các trường kỹ thuật (tọa độ GPS, thông số kỹ thuật) bị ẩn; các trường hành chính vẫn hiển thị | Danh sách trường kỹ thuật bị ẩn cần được xác nhận với domain expert |
| AC-009 | US-001 | Lịch sử Bến cảng chưa có thay đổi | Given Bến cảng mới tạo chưa có sự kiện nào ngoài tao_moi; When xem trang lịch sử; Then ít nhất 1 sự kiện tao_moi hiển thị | Không hiển thị trang trắng khi chỉ có 1 sự kiện |
| AC-010 | US-001 | Lịch sử không bị thiếu sự kiện từ các tính năng nguồn | Given đã thực hiện tao_moi (F-014), cap_nhat (F-015), xoa (F-016), phe_duyet (F-017) trên cùng một Bến cảng; When xem lịch sử; Then tất cả 4 loại sự kiện đều xuất hiện theo thứ tự thời gian | Kiểm thử tích hợp chéo F-014/F-015/F-016/F-017/F-019 |

## Business Rules

| BR-ID | Rule | Applies to | Exception |
|---|---|---|---|
| BR-001 | Mọi thay đổi về Bến cảng đều phải được ghi nhận vào bảng lịch sử — không cho phép bỏ qua hoặc vô hiệu hóa | Tất cả sự kiện trên Bến cảng | Không có ngoại lệ |
...
| BR-003 | Sự kiện từ F-014, F-015, F-016, F-017 được tích hợp vào cùng một dòng thời gian thống nhất theo benCangId | US-001 | |
| BR-004 | Giá trị cũ/mới được lưu dạng văn bản hóa; giá trị JSON (tọa độ GPS) phải chuyển sang định dạng text dễ đọc trước khi lưu | US-002 | |
...
|---|---|---|
| Performance | Truy vấn danh sách lịch sử (có phân trang 20 bản ghi/trang) phải trả về trong ngưỡng hợp lý | ≤ 2 giây cho Bến cảng có ≤ 10,000 sự kiện |
...
| Audit/Logging | Mọi truy cập trang lịch sử được ghi vào nhật ký hệ thống (ai xem, khi nào, Bến nào) | Lưu tối thiểu: userId, benCangId, timestamp, action=VIEW_HISTORY |
| Operability | Phân trang, bộ lọc hoạt động ổn định; không timeout khi Bến cảng có lịch sử lớn | N/A (phân trang bắt buộc) |

## Test Scenarios

| TS-ID | AC-ref | Scenario | Type |
|---|---|---|---|
| TS-001 | AC-001 | Quản trị viên xem lịch sử Bến cảng có đầy đủ 4 loại sự kiện | Integration |
...
| TS-009 | AC-009 | Bến cảng mới tạo có ít nhất 1 sự kiện tao_moi | Integration |
| TS-010 | AC-010 | Tích hợp chéo F-014/F-015/F-016/F-017 → F-019 ghi đầy đủ | Cross-feature Integration |

## Pipeline Triage

| Question | Answer | Rationale |
|---|---|---|
| Domain model affected? | No | Thực thể LichSuThayDoi đã định nghĩa trong feature-brief; feature này là read + filter trên dữ liệu đã tồn tại từ các tính năng nguồn |
