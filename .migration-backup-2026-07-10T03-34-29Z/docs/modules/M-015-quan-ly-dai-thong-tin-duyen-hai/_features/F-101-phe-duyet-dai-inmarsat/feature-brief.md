...
consumed_by_modules: []
---
# Feature: Phê duyệt Đại Inmarsat

## Description
Cho phép lãnh đạo cấp Phòng và cấp Cục thực hiện quy trình phê duyệt hai cấp cho các yêu cầu tạo mới, cập nhật hoặc xóa Đại Inmarsat (trạm vệ tinh Inmarsat). Hệ thống quản lý luồng phê duyệt tuần tự từ Phòng lên Cục, với đầy đủ chức năng xem chi tiết, bình luận và ghi chú phê duyệt.

## Business Intent
Quy trình phê duyệt hai cấp (Phòng → Cục) đảm bảo mọi thay đổi về thông tin Đại Inmarsat đều được kiểm tra và chấp thuận bởi các cấp quản lý có thẩm quyền, đảm bảo tính chính xác và an toàn của dữ liệu hệ thống thông tin vệ tinh Inmarsat phục vụ công tác cứu nạn cứu hộ trên biển.

## Flow Summary
Lãnh đạo cấp Phòng nhận được thông báo yêu cầu phê duyệt, truy cập hệ thống để xem chi tiết yêu cầu (tạo mới/cập nhật/xóa), kiểm tra thông tin kỹ thuật (loại modem, tần số, vùng phủ sóng), thêm ghi chú nếu cần, sau đó chọn Phê duyệt hoặc Từ chối. Nếu được Phòng phê duyệt, yêu cầu chuyển lên Cấp Cục để xem xét và phê duyệt cấp 2. Sau khi cả hai cấp đều phê duyệt, bản ghi được cập nhật chính thức.
...
## In Scope
- Giao diện xem chi tiết yêu cầu phê duyệt (thông tin kỹ thuật Inmarsat)
...
## Entities
- **CoastalStationInmarsat**: id, device_code, station_name, modem_type, frequency, coverage_zone, status (pending/approved/rejected), version
