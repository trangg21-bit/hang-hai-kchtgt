...
function pillBadge(count: number, activeColor: string, activeBg: string, zeroBg: string) {
  if (count === 0) {
    return <span style={{ display: 'inline-block', borderRadius: rPill, padding: '1px 8px', fontSize: fontSizeSm, background: zeroBg, color: ink3, fontWeight: 500 }}>0</span>;
  }
  return <span style={{ display: 'inline-block', borderRadius: rPill, padding: '1px 8px', fontSize: fontSizeSm, background: activeBg, color: activeColor, fontWeight: 500 }}>{count}</span>;
}

const infraColumns = [
  { title: '', dataIndex: 'loai', key: 'loai', width: 150 },
  { title: 'Tổng số lượng', dataIndex: 'tongSL', key: 'tongSL', width: 80, align: 'center' as const, render: (v: number) => <span style={{ fontWeight: 600, fontFamily: fontMono, color: ink }}>{v}</span> },
  { title: <span>Chưa khai thác/<br/>vận hành</span>, dataIndex: 'chuaKhaiThac', key: 'chuaKhaiThac', width: 70, align: 'center' as const, render: (v: number) => pillBadge(v, sea0, `${sea0}18`, surface) },
  { title: <span>Đang khai thác/<br/>vận hành</span>, dataIndex: 'dangKhaiThac', key: 'dangKhaiThac', width: 70, align: 'center' as const, render: (v: number) => pillBadge(v, surface, sea0, surface) },
  { title: <span>Dừng khai thác/<br/>vận hành</span>, dataIndex: 'dungKhaiThac', key: 'dungKhaiThac', width: 70, align: 'center' as const, render: (v: number) => pillBadge(v, sea0, sea3, surface) },
  { title: '', key: 'action', width: 40, align: 'center' as const, render: () => <EyeOutlined style={{ color: ink2, cursor: 'pointer' }} /> },
];
  ...
  return (
    <div style={CARD_BASE}>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 6, fontSize: fontSizeSm, color: ink2 }}>
        {stats.approved > 0 && (
          <span>
            <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: 2, background: approvalApproved, marginRight: 4, verticalAlign: 'middle' }} />
            Đã duyệt {stats.approved.toLocaleString('vi-VN')}
          </span>
        )}
        {stats.rejected > 0 && (
          <span>
            <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: 2, background: approvalRejected, marginRight: 4, verticalAlign: 'middle' }} />
      ...
      <div style={{ marginTop: 6 }}>
        {stats.pending === 0 ? (
          <span style={{ display: 'inline-block', borderRadius: rPill, padding: '2px 10px', fontSize: fontSizeSm, fontWeight: 500, background: pendingZeroBg, color: pendingZeroColor }}>✓ {stats.pending} chờ</span>
        ) : (
          <span style={{ display: 'inline-block', borderRadius: rPill, padding: '2px 10px', fontSize: fontSizeSm, fontWeight: 500, background: pendingActiveBg, color: pendingActiveColor }}>⏳ {stats.pending} chờ</span>
        )}
      </div>
    </div>
  );
...
function HeroCard({ heroKpi, year }: { heroKpi: any; year: number }) {
  ...
  return (
    <div style={{ ...CARD_BASE, background: `linear-gradient(135deg, ${navy}, ${sea0})`, border: 'none', color: '#eaf4fc' }}>
      <div style={{ fontSize: fontSizeSm, opacity: 0.7, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>SẢN LƯỢNG CHỦ ĐẠO · {year}</div>
      <div style={{ fontSize: fontSizeDisplay, fontFamily: fontMono, fontWeight: 600, lineHeight: 1.1 }}>{heroKpi.value.toLocaleString('vi-VN')}</div>
        ...
        <ReactECharts
          option={{
            ...
            series: [{
              ...
              areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: `${dataSea3}59` }, { offset: 1, color: `${dataSea3}05` }] } },
            }],
          }}
...
function MiniKpiCard({ card }: { card: any }) {
  ...
  return (
    <div style={CARD_BASE}>
      ...
      <div style={{ fontSize: fontSizeHeading, fontFamily: fontMono, color: ink, fontWeight: 600, lineHeight: 1.2 }}>{card.value}</div>
        ...
        <div style={{ marginTop: 6, height: 28 }}>
          <ReactECharts
            option={{
              ...
              yAxis: { type: 'value', show: false, min: (v: { min: number }) => v.min * 0.95 },
              series: [{
                type: card.sparklineType === 'bar' ? 'bar' : 'line', data: card.sparklineData, smooth: true, symbol: 'none',
                lineStyle: { color: dataSea1, width: 1.5 }, itemStyle: { color: dataSea1 },
                areaStyle: card.sparklineType !== 'bar' ? { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: `${dataSea1}33` }, { offset: 1, color: `${dataSea1}05` }] } } : undefined,
              }],
            }}
            style={{ height: '100%' }} notMerge
          />
...
function HomeDashboard() {
  ...
  const cargoOption: EChartsOption = {
    tooltip: { ...chartTooltip, trigger: 'axis', axisPointer: { type: 'line', lineStyle: { color: line, type: 'solid' } },
      formatter: (params: any) => {
        if (!Array.isArray(params)) return '';
        let total = 0;
        let html = `<div style="font-weight:600;margin-bottom:4px">Tháng ${params[0].axisValue}</div>`;
        params.forEach((p: any) => { const v = p.value ?? 0; total += v; html += `<div style="display:flex;justify-content:space-between;gap:12px"><span>${p.marker} ${p.seriesName}</span><span style="font-weight:600">${v.toLocaleString('vi-VN')}</span></div>`; });
    ...
    legend: { bottom: 0, icon: 'roundRect', itemWidth: 10, itemHeight: 10, textStyle: { ...chartTextStyle, fontSize: fontSizeSm } },
    grid: { ...chartGrid, bottom: 40 },
    xAxis: { type: 'category', data: CARGO_MONTHS, axisTick: { show: false }, axisLine: { lineStyle: { color: line } }, axisLabel: { ...chartTextStyle } },
    ...
    series: CARGO_SERIES.map((s, idx) => ({ type: 'bar' as const, name: s.name, stack: 'total', barWidth: '58%', data: s.data, itemStyle: { color: s.color, borderRadius: idx === CARGO_SERIES.length - 1 ? [radiusSm, radiusSm, 0, 0] : 0 } })),
  };
  ...
  const polarOption: EChartsOption = {
    ...
    legend: { bottom: 0, icon: 'roundRect', itemWidth: 10, itemHeight: 10, textStyle: { ...chartTextStyle, fontSize: fontSizeSm } },
    polar: { radius: ['18%', '78%'] },
    angleAxis: { type: 'category', data: ['T1','T2','T3','T4','T5','T6','T7','T8','T9','T10','T11','T12'], startAngle: 90, axisLabel: { ...chartTextStyle }, axisLine: { show: false }, axisTick: { show: false }, splitLine: { lineStyle: { color: line, type: 'dashed' } } },
    radiusAxis: { type: 'value', axisLabel: { show: false }, axisLine: { show: false }, axisTick: { show: false }, splitLine: { show: false } },
    series: [
      { type: 'bar', name: 'Đến cảng', coordinateSystem: 'polar', stack: 'a', data: passengerMonthly?.arrival || [], itemStyle: { color: { type: 'linear', x: 0, y: 0, x2: 1, y2: 0, colorStops: [{ offset: 0, color: dataSea0 }, { offset: 1, color: dataSea1 }] } }, emphasis: { itemStyle: { color: dataSea1 } } },
      { type: 'bar', name: 'Rời cảng', coordinateSystem: 'polar', stack: 'a', data: passengerMonthly?.departure || [], itemStyle: { borderRadius: [radiusSm, radiusSm, 0, 0], color: { type: 'linear', x: 0, y: 0, x2: 1, y2: 0, colorStops: [{ offset: 0, color: dataSea2 }, { offset: 1, color: dataSea2 }] } }, emphasis: { itemStyle: { color: dataSea2 } } },
    ],
  };
  ...
  const donutOption: EChartsOption = {
    ...
    legend: { bottom: 0, icon: 'roundRect', itemWidth: 10, itemHeight: 10, textStyle: { ...chartTextStyle, fontSize: fontSizeSm } },
    series: [{ type: 'pie', radius: ['55%', '78%'], center: ['50%', '45%'], avoidLabelOverlap: false, itemStyle: { borderRadius: radiusSm, borderColor: surface, borderWidth: 2 }, label: { show: false }, emphasis: { label: { show: true, fontSize: fontSizeLg, fontWeight: 'bold' } }, data: donutData.map((d) => ({ value: d.value, name: d.name, itemStyle: { color: d.color } })) }],
  };
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16, alignItems: 'stretch' }}>
        ...
        <Col xs={24} md={12}>
          <div style={{ ...CARD_BASE, height: '100%' }}>
            ...
            <Table columns={infraColumns} dataSource={INFRA_DATA} rowKey="stt" pagination={false} size="small" scroll={{ x: 480, y: 340 }} />
          </div>
        </Col>
      </Row>
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
