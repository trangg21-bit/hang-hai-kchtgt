...
const ROLE_MAP: Record<string, { color: string; bg: string; label: string }> = {
  admin:  { color: statusCritical, bg: `${statusCritical}15`, label: 'Quản lý' },
  member: { color: actionPrimary,  bg: `${actionPrimary}15`,  label: 'Thành viên' },
  viewer: { color: statusDraft,    bg: `${statusDraft}15`,    label: 'Xem' },
};
...
export default function GroupMembers() {
  ...
  const columns: ColumnsType<GroupMember> = [
    ...
    },
    {
      ...
      width: 120,
      render: (role: string) => {
        const r = ROLE_MAP[role] || { color: statusDraft, bg: `${statusDraft}15`, label: role };
        return <span style={roleBadgeStyle(r.color, r.bg)}>{r.label}</span>;
      },
    },
    {
      title: 'Tham gia từ',
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={[
          <Button key="cancel" onClick={() => { setAddModalOpen(false); form.resetFields(); }}
            style={{ borderRadius: radiusPill, height: 40, fontSize: fontSizeMd, borderColor: borderDefault, color: textSecondary }}>Hủy</Button>,
          <Button key="ok" type="primary" onClick={handleAddMember}
