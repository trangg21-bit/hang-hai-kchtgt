 ...
 * Verifies service-level scope contract enforcement and write guards across two distinct organizational units (Unit A and Unit B).
 *
 * Test Matrix:
 * 1. User A (belongs to Unit A) searching records -> Only Unit A records queried in repository; Unit B excluded.
 * 2. User A reading detail (getById) -> Unit A record succeeds; Unit B record throws IllegalArgumentException.
 * 3. User A updating record -> Unit A record succeeds; Unit B record throws IllegalArgumentException.
 * 4. User A deleting record -> Unit A record succeeds; Unit B record throws IllegalArgumentException.
 * 5. User A creating record -> Unit A record succeeds; Unit B target throws IllegalArgumentException.
 */
...
class DataScopeCrossUnitContractTest {
    ...
    @Mock private VtsSystemRepository vtsRepository;
    ...
    @Mock private VtsZoneRepository zoneRepository;
    ...
    void setUp() {
        vtsSystemService = new VtsSystemService(
                vtsRepository, historyRepository, gisSpatialObjectService,
        ...
        // Mock User A authentication with standard business permissions (not nationwide)
    ...
    void search_shouldLimitQueryToUnitAScope() {
        // Given: database query with scopeEnabled = true and scopeOrgUnitIds = [ORG_UNIT_A_ID]
        when(vtsRepository.search(
                ...
                isNull(),
                any(Pageable.class)
        )).thenReturn(new PageImpl<>(List.of(vtsSystemA)));
