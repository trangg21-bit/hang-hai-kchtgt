...
import com.hanghai.kchtg.assetmovement.controller.YeuCauTangTaiSanController;
...
@WebMvcTest(YeuCauTangTaiSanController.class)
@AutoConfigureMockMvc(addFilters = false)
class YeuCauTangTaiSanControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @MockBean
    private YeuCauTangTaiSanService yeuCauService;
    ...
    private YeuCauTangTaiSanResponse makeResponse() {
        return YeuCauTangTaiSanResponse.builder()
                .id(UUID.randomUUID())
                .taiSanId(UUID.randomUUID())
                .tenTaiSan("Tai san tang")
                .soLuong(10)
                .donViTinh("cai")
                .lyDo("Tang them de phuc vu")
    ...
    @Test
    @DisplayName("POST /api/v1/asset/yeu-cau-tang — creates and returns 201")
    void testCreateReturns201() throws Exception {
        YeuCauTangTaiSanResponse response = makeResponse();
        when(yeuCauService.create(any())).thenReturn(response);

        mockMvc.perform(post("/api/v1/asset/yeu-cau-tang")
    ...
    @Test
    @DisplayName("GET /api/v1/asset/yeu-cau-tang/{id} — returns 200")
    void testGetByIdReturns200() throws Exception {
        YeuCauTangTaiSanResponse response = makeResponse();
        when(yeuCauService.getById(any(UUID.class))).thenReturn(response);

        mockMvc.perform(get("/api/v1/asset/yeu-cau-tang/" + response.getId())
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("GET /api/v1/asset/yeu-cau-tang — returns 200 with paginated list")
    void testFindAllReturns200() throws Exception {
        Page<YeuCauTangTaiSanResponse> page = new PageImpl<>(List.of(makeResponse()));
        Pageable pageable = PageRequest.of(0, 20, Sort.by("createdAt").descending());
        when(yeuCauService.findAll(any(Pageable.class))).thenReturn(page);

        mockMvc.perform(get("/api/v1/asset/yeu-cau-tang")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    @Test
    @DisplayName("PUT /api/v1/asset/yeu-cau-tang/{id} — updates and returns 200")
    void testUpdateReturns200() throws Exception {
        YeuCauTangTaiSanResponse response = makeResponse();
        when(yeuCauService.update(any(UUID.class), any())).thenReturn(response);

        mockMvc.perform(put("/api/v1/asset/yeu-cau-tang/" + response.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                            { "taiSanId": "00000000-0000-0000-0000-000000000001", "soLuong": 10 }
                            """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    @Test
    @DisplayName("DELETE /api/v1/asset/yeu-cau-tang/{id} — deletes and returns 200")
    void testDeleteReturns200() throws Exception {
        doNothing().when(yeuCauService).delete(any(UUID.class));

        mockMvc.perform(delete("/api/v1/asset/yeu-cau-tang/" + UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
