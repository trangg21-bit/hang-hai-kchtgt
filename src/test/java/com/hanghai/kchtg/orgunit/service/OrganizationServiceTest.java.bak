...
import com.hanghai.kchtg.orgunit.service.MaterializedPathService;
...
class OrganizationServiceTest {
    ...
    class UniqueCodeTests {
        ...
        void shouldRejectDuplicateCodeOnUpdate() {
            ...
            when(orgUnitRepo.findById(updateId)).thenReturn(Optional.of(
                    makeUnit(updateId, "Old Name", "OLD_CODE", OrgUnitType.CUC)));
    ...
    class ApprovalWorkflowTests {
        ...
        void shouldTransitionDraftToPendingOnSubmit() {
            OrgUnit unit = makeUnit(rootId, "Cục", "CUC001", OrgUnitType.CUC);
    ...
    class HierarchyTests {
        ...
        void shouldRejectSelfAsParent() {
            OrgUnit unit = makeUnit(childId, "Cảng vụ", "CV001", OrgUnitType.CANG_VU);
            when(orgUnitRepo.findById(childId)).thenReturn(Optional.of(unit));
            when(materializedPathService.isSelfParent(childId, childId)).thenReturn(true);
        ...
        void shouldRejectCircularReference() {
            ...
            when(orgUnitRepo.findById(childId)).thenReturn(Optional.of(unit));
            when(materializedPathService.isAncestor(childId, parentId)).thenReturn(false); // parentId is ancestor
            when(materializedPathService.isAncestor(childId, rootId)).thenReturn(false);

            // When child tries to become parent of root — root is an ancestor
            when(materializedPathService.isAncestor(childId, rootId)).thenReturn(false);
            when(orgUnitRepo.findById(rootId)).thenReturn(Optional.of(
                    makeUnit(rootId, "Cục", "CUC001", OrgUnitType.CUC)));

            UpdateOrgUnitRequest request = new UpdateOrgUnitRequest();
            request.setParentId(rootId);

            // This should NOT throw — rootId is the actual ancestor, so isAncestor returns true
    ...
    class CoefficientTests {
        ...
        void shouldRejectNegativeCoefficient() {
            ...
            request.setCode("TEST001");
            request.setType(OrgUnitType.CUC);
            request.setCoefficient(-0.5);

            if (request.getCoefficient() != null && request.getCoefficient() <= 0) {
                fail("Negative coefficient should be rejected by @DecimalMin validation");
            }
        }
        ...
        void shouldAcceptValidCoefficient() {
            ...
            request.setCode("TEST001");
            request.setType(OrgUnitType.CUC);
            request.setCoefficient(1.50);

            assertNotNull(request.getCoefficient());
            assertTrue(request.getCoefficient() > 0);
        }
