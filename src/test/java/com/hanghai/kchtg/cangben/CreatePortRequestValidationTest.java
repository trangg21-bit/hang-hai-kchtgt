...
class CreatePortRequestValidationTest {
    ...
    private CreatePortRequest validRequest() {
        ...
        req.setLatitude(new BigDecimal("20.845"));
        req.setLongitude(new BigDecimal("106.688"));
    ...
    void latitude_91_violation() {
        ...
        req.setLatitude(new BigDecimal("91"));
        ...
        assertTrue(violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("latitude")),
                "Expected violation on latitude");
    }
    ...
    void latitude_90_accepted() {
        CreatePortRequest req = validRequest();
        req.setLatitude(new BigDecimal("90"));

        Set<ConstraintViolation<CreatePortRequest>> violations = validator.validate(req);

        assertTrue(violations.stream().noneMatch(v -> v.getPropertyPath().toString().equals("latitude")),
    ...
    void longitude_181_violation() {
        CreatePortRequest req = validRequest();
        req.setLongitude(new BigDecimal("181"));
    ...
    void longitude_180_accepted() {
        CreatePortRequest req = validRequest();
        req.setLongitude(new BigDecimal("180"));

        Set<ConstraintViolation<CreatePortRequest>> violations = validator.validate(req);

        assertTrue(violations.stream().noneMatch(v -> v.getPropertyPath().toString().equals("longitude")));
    }
