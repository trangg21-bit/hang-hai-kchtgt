...
import com.hanghai.kchtg.transmissionasset.service.VhfAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/vhf-assets")
@RequiredArgsConstructor
@DataScope
public class VhfAssetController {

    private final VhfAssetService service;
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'vhfasset:manage', 'vhfasset:create', 'vhf:create', 'vhf:manage')")
    public ResponseEntity<ApiResponse<TransmissionAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản HTTT liên lạc VHF đã được tạo", response));
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'vhfasset:manage', 'vhfasset:update', 'vhf:update', 'vhf:manage')")
    public ResponseEntity<ApiResponse<TransmissionAssetResponse>> update(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tài sản HTTT liên lạc VHF đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'vhfasset:manage', 'vhfasset:delete', 'vhf:delete', 'vhf:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản HTTT liên lạc VHF đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'vhfasset:manage', 'vhfasset:update', 'vhfasset:create', 'vhf:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'vhfasset:manage', 'vhfasset:update', 'vhf:update')")
