...
import com.hanghai.kchtg.transmissionasset.service.VtsAssistAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/vts-assist-assets")
@RequiredArgsConstructor
@DataScope
public class VtsAssistAssetController {

    private final VtsAssistAssetService service;
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'vtsassistasset:manage', 'vtsassistasset:create', 'vtsassist:create', 'vtsassist:manage')")
    public ResponseEntity<ApiResponse<TransmissionAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản hệ thống phụ trợ VTS đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<TransmissionAssetResponse>>> findAll(
        ...
        Page<TransmissionAssetResponse> result = service.findAll(
                ...
                transmissionId, assetCondition, approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'vtsassistasset:manage', 'vtsassistasset:update', 'vtsassist:update', 'vtsassist:manage')")
    public ResponseEntity<ApiResponse<TransmissionAssetResponse>> update(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tài sản hệ thống phụ trợ VTS đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'vtsassistasset:manage', 'vtsassistasset:delete', 'vtsassist:delete', 'vtsassist:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản hệ thống phụ trợ VTS đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'vtsassistasset:manage', 'vtsassistasset:update', 'vtsassistasset:create', 'vtsassist:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'vtsassistasset:manage', 'vtsassistasset:update', 'vtsassist:update')")
