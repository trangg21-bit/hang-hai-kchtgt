...
import com.hanghai.kchtg.coastalstationasset.service.InmarsatAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/inmarsat-assets")
@RequiredArgsConstructor
@DataScope
public class InmarsatAssetController {

    private final InmarsatAssetService service;
    ...
    public ResponseEntity<ApiResponse<CoastalStationAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản đài Inmarsat đã được tạo thành công", response));
    }
    ...
    public ResponseEntity<ApiResponse<CoastalStationAssetResponse>> update(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài Inmarsat đã được cập nhật thành công", response));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'inmarsatasset:manage', 'inmarsatasset:update', 'inmarsatasset:create', 'inmarsat:update')")
