...
import com.hanghai.kchtg.coastalstationasset.service.DaiTtdhAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/dai-ttdh-assets")
@RequiredArgsConstructor
@DataScope
public class DaiTtdhAssetController {

    private final DaiTtdhAssetService service;

    private static final java.util.Set<String> SORTABLE_FIELDS = CoastalStationAssetController.SORTABLE_FIELDS;
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'daittdhasset:manage', 'daittdhasset:create', 'daittdh:create', 'daittdh:manage')")
    public ResponseEntity<ApiResponse<CoastalStationAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản đài TTDH đã được tạo thành công", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<CoastalStationAssetResponse>>> findAll(
        ...
        Page<CoastalStationAssetResponse> result = service.findAll(
                ...
                assetCondition, approvalStatus, updatedFrom, updatedTo, pageable);
    ...
    public ResponseEntity<ApiResponse<Map<String, Long>>> countByApprovalStatus(
            ...
            @RequestParam(required = false) LocalDate updatedFrom,
            @RequestParam(required = false) LocalDate updatedTo) {
        Map<String, Long> result = service.getCounts(
                assetCode, assetName, parentOrgUnitId, orgUnitId, usingOrgUnitId, stationId,
                assetCondition, updatedFrom, updatedTo);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'daittdhasset:manage', 'daittdhasset:update', 'daittdh:update', 'daittdh:manage')")
    public ResponseEntity<ApiResponse<CoastalStationAssetResponse>> update(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài TTDH đã được cập nhật thành công", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'daittdhasset:manage', 'daittdhasset:delete', 'daittdh:delete', 'daittdh:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài TTDH đã được xóa thành công", null));
    }
    ...
    public ResponseEntity<ApiResponse<CoastalStationAssetAdjustment>> addAdjustment(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Đã gửi yêu cầu điều chỉnh", result));
    }

    // ── Attachment endpoints ─────────────────────────────────────────────

    @PostMapping(value = "/{id}/attachments", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("@auth.checkAny(authentication, 'daittdhasset:manage', 'daittdhasset:update', 'daittdhasset:create', 'daittdh:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'daittdhasset:manage', 'daittdhasset:update', 'daittdh:update')")
