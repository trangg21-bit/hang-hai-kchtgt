...
public interface BeaconStationRepository extends JpaRepository<BeaconStation, UUID> {
    ...
    @Query("SELECT b FROM BeaconStation b WHERE " +
           ...
           "((:status IS NULL) OR ((:status = 'DELETED' OR :status = 'ARCHIVED') AND (b.status = 'DELETED' OR b.status = 'ARCHIVED' OR b.deletedAt IS NOT NULL OR b.deletedBy IS NOT NULL OR b.approvalStatus = com.hanghai.kchtg.common.entity.ApprovalStatus.ARCHIVED)) OR (b.status = :status AND b.deletedAt IS NULL AND b.deletedBy IS NULL AND b.status != 'DELETED' AND b.status != 'ARCHIVED' AND (b.approvalStatus IS NULL OR b.approvalStatus != com.hanghai.kchtg.common.entity.ApprovalStatus.ARCHIVED))) AND " +
