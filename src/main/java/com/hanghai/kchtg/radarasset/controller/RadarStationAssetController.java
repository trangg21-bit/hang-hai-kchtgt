package com.hanghai.kchtg.radarasset.controller;
...
import com.hanghai.kchtg.radarasset.service.RadarStationAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/radar-assets")
@RequiredArgsConstructor
@DataScope
public class RadarStationAssetController {

    private static final Set<String> ALLOWED_SORT_FIELDS = Set.of(
            "assetCode", "assetName", "parentOrgUnitId", "orgUnitId", "usingOrgUnitId",
            "radarStationId", "assetType", "barcode", "assetCondition", "usageStatus",
    ...
    public ResponseEntity<ApiResponse<RadarStationAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản trạm radar đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<RadarStationAssetResponse>>> findAll(
        ...
        Page<RadarStationAssetResponse> result = service.findAll(assetCode, assetName, parentOrgUnitId,
                orgUnitId, usingOrgUnitId, radarStationId, assetType, assetCondition, approvalStatus,
    ...
    public ResponseEntity<ApiResponse<RadarStationAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody RadarStationAssetRequest request) {
        RadarStationAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản trạm radar đã được cập nhật", response));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản trạm radar đã được xóa", null));
    }
}
