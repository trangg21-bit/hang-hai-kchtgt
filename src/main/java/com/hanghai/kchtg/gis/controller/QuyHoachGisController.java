...
@Slf4j
public class QuyHoachGisController {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getPlanningFeatures(
        ...
        for (String type : types) {
            ...
            Object[] args;
            if (hasBbox) {
                sql = "SELECT fid, \"Ten_doi_tuong\", \"Tinh_thanh\", \"Color\", \"Type\", \"Chieu_dai\", \"Dien_tich\", " +
                      "\"trang_thai\", \"nguon_du_lieu\", \"ghi_chu\", \"co_quan_ql\", ST_AsGeoJSON(geom) as geojson, " +
            ...
            try {
                List<Map<String, Object>> rows = jdbcTemplate.query(sql, (rs, rowNum) -> {
                    ...
                    map.put("geomType", type);
                    map.put("name", rs.getString("Ten_doi_tuong"));
                    map.put("province", rs.getString("Tinh_thanh"));
                    map.put("color", rs.getInt("Color"));
                    map.put("type", rs.getString("Type"));
                    
                    double length = rs.getDouble("Chieu_dai");
                    map.put("length", rs.wasNull() ? null : length);
                    
                    double area = rs.getDouble("Dien_tich");
                    map.put("area", rs.wasNull() ? null : area);
                    
                    map.put("status", rs.getString("trang_thai"));
                    map.put("source", rs.getString("nguon_du_lieu"));
                    map.put("notes", rs.getString("ghi_chu"));
                    map.put("agency", rs.getString("co_quan_ql"));
    ...
    public ResponseEntity<ApiResponse<Void>> updateFeatureStatus(
        ...
        String sql = "UPDATE qhcb_all.\"" + type.toUpperCase() + "\" " +
                     "SET \"trang_thai\" = ?, \"Color\" = ? " +
                     "WHERE fid = ?";
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getFeaturesAtPoint(
        ...
        for (String type : types) {
            String sql = "SELECT fid, \"Ten_doi_tuong\", \"Tinh_thanh\", \"Color\", \"Type\", \"Chieu_dai\", \"Dien_tich\", " +
                         "\"trang_thai\", \"nguon_du_lieu\", \"ghi_chu\", \"co_quan_ql\", ST_AsGeoJSON(geom) as geojson, " +
                         "ST_Y(ST_Centroid(geom)) as lat, ST_X(ST_Centroid(geom)) as lon " +
                         "FROM qhcb_all.\"" + type + "\" " +
