...
public interface LineObjectRepository extends JpaRepository<LineObject, UUID> {
    ...
    List<LineObject> findByRefIdIsNull();

    List<LineObject> findByStatus(Status status);

    List<LineObject> findByStatusAndRefIdIsNull(Status status);

    @Query("SELECT l FROM LineObject l WHERE (:status IS NULL OR l.status = :status) AND (l.refType IS NULL OR l.refType = 0)")
    List<LineObject> findManualByStatus(@Param("status") Status status);

    @Query("SELECT l FROM LineObject l WHERE (l.refType IS NULL OR l.refType = 0)")
    List<LineObject> findManualAll();
    ...
    @Query(value = "SELECT * FROM gis_spatial_objects l WHERE " +
            "l.geometry_type = 2 AND l.deleted_at IS NULL AND (l.ref_type IS NULL OR l.ref_type = 0) AND " +
