    ...
    private void patchInfrastructureHistoryTable() {
        try {
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS infrastructure_history (" +
                    ...
                    ");");
            jdbcTemplate
                    .execute("ALTER TABLE infrastructure_history ADD COLUMN IF NOT EXISTS approval_level VARCHAR(32);");
            jdbcTemplate.execute("ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS reason TEXT;");
            jdbcTemplate.execute(
                    "ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS changed_field VARCHAR(1000);");
            jdbcTemplate
                    .execute("ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS previous_value TEXT;");
            jdbcTemplate.execute("ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS new_value TEXT;");
            jdbcTemplate
                    .execute("ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS approved_by UUID;");
            jdbcTemplate.execute(
                    "ALTER TABLE public.infrastructure_history ADD COLUMN IF NOT EXISTS approved_date TIMESTAMP;");
            jdbcTemplate.execute(
