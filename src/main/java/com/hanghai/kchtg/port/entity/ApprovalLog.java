 ...
 * Corresponds to table: approval_logs (renamed from phe_duyet_log).
 */
@Entity
@Table(name = "approval_logs",
        indexes = {
                @Index(name = "idx_approval_logs_entity", columnList = "entity_type, entity_id"),
                @Index(name = "idx_approval_logs_decided_at", columnList = "decided_at DESC"),
                @Index(name = "idx_approval_logs_decided_by", columnList = "decided_by")
        })
@Getter
@Setter
@NoArgsConstructor
