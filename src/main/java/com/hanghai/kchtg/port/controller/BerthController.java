...
public class BerthController {
    ...
    public ResponseEntity<ApiResponse<Void>> submitForApproval(@PathVariable UUID id) {
        berthApprovalService.submit(id, SecurityUtils.getCurrentUserId());
        return ResponseEntity.ok(ApiResponse.success("Gửi phê duyệt bến cảng thành công", null));
    }

    @PostMapping("/{id}/approve/c1")
    @PreAuthorize("@auth.check(authentication, 'berth:approvec1')")
    public ResponseEntity<ApiResponse<Void>> approveC1(
            @PathVariable UUID id,
            @RequestParam(required = false) String reason) {
        berthApprovalService.approveC1(id, reason, SecurityUtils.getCurrentUserId());
        return ResponseEntity.ok(ApiResponse.success("Phê duyệt cấp Chi cục thành công", null));
    }

    @PostMapping("/{id}/approve/c2")
    @PreAuthorize("@auth.check(authentication, 'berth:approvec2')")
    public ResponseEntity<ApiResponse<Void>> approveC2(
            @PathVariable UUID id,
            @RequestParam(required = false) String reason) {
        berthApprovalService.approveC2(id, reason, SecurityUtils.getCurrentUserId());
    ...
    public ResponseEntity<ApiResponse<List<AttachmentDto>>> uploadAttachments(
        ...
        List<AttachmentDto> result = berthService.uploadAttachments("BERTH", id, files, userId, skipHistory);
    ...
    public ResponseEntity<ApiResponse<Void>> deleteAttachment(
            ...
            Authentication authentication) {
        UUID userId = SecurityUtils.getCurrentUserId();
        berthService.deleteAttachment("BERTH", id, attId, userId, skipHistory);
    ...
    public ResponseEntity<org.springframework.core.io.Resource> downloadAttachment(
            @PathVariable UUID id,
            @PathVariable UUID attId) {
        com.hanghai.kchtg.port.entity.Attachment attachment = berthService.getAttachment("BERTH", id, attId);
