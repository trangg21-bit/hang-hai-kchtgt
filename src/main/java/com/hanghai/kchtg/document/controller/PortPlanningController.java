...
public class PortPlanningController {
    ...
    @PostMapping
    @PreAuthorize("@auth.check(authentication, 'portplanning:create')")
    ...
    @PutMapping("/{id}")
    @PreAuthorize("@auth.check(authentication, 'portplanning:update')")
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.check(authentication, 'portplanning:delete')")
    ...
    public ResponseEntity<ApiResponse<List<PortPlanningResponse>>> searchByName(
            ...
            @RequestParam(name = "page", required = false, defaultValue = "0") int page,
            @RequestParam(name = "size", required = false, defaultValue = "20") int size) {
        return ResponseEntity.ok(ApiResponse.success(portPlanningService.searchByProjectNameContaining(keyword, page, size).getContent()));
    }
    ...
    @GetMapping("/search")
    @PreAuthorize("@auth.check(authentication, 'portplanning:search')")
