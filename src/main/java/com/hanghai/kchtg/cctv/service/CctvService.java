...
public class CctvService {
  ...
  public CctvResponse update(UpdateCctvRequest request) {
    ...
    approvalService.assertCanEdit(entity, currentUserId, InfrastructureType.CCTV);
    ...
    // Track previous values for change history
    ...
    if (shouldClearLocation) {
      ...
      // 2. Xóa các trường bản đồ liên kết trên entity CCTV
    ...
    if (wasApproved) {
      approvalService.handleApprovedRecordEdit(entity, InfrastructureType.CCTV, request.getApprovalStatus(), currentUserId);
  ...
  private <T> void applyIfChanged(String field, T oldVal, T newVal, java.util.function.Consumer<T> setter,
      Map<String, String> previousValues, UpdateCctvRequest request) {
    if (request != null) {
      if (!request.isFieldPresent(field)) {
        return;
      }
    } else if (newVal == null) {
      return;
