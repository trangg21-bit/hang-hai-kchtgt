package com.hanghai.kchtg.assetmovement.dto;

import java.util.UUID;
import jakarta.validation.constraints.Max;
import jakarta.validation.constraints.Min;
import jakarta.validation.constraints.NotNull;
import lombok.Data;

import java.math.BigDecimal;

/**
 * Request DTO cho Khai Thac Tai San (create/update).
 */
@Data
public class KhaiThacTaiSanRequest {

    @NotNull(message = "Tài sản không được để trống")
    private UUID taiSanId;

    private String tenTaiSan;

    @NotNull(message = "Năm khai thác không được để trống")
    @Min(value = 1900, message = "Năm khai thác phải từ 1900 trở đi")
    @Max(value = 2100, message = "Năm khai thác không được lớn hơn 2100")
    private Integer namKhaiThac;

    @Min(value = 0, message = "Doanh thu không được âm")
    private BigDecimal doanhThu;

    @Min(value = 0, message = "Hao mòn không được âm")
    private BigDecimal haoMon;

    private String moTa;
}
