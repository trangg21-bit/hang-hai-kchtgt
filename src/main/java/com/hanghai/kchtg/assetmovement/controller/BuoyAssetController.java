...
import com.hanghai.kchtg.assetmovement.service.BuoyAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/buoy-assets")
@DataScope
public class BuoyAssetController {

    private final BuoyAssetService service;

    public BuoyAssetController(BuoyAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:create', 'buoy:create', 'buoy:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản phao tiêu đã được tạo", response));
    }

    @GetMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:read', 'buoy:read', 'data:read')")
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                buoyId, buoyStationId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:update', 'buoy:update', 'buoy:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản phao tiêu đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:delete', 'buoy:delete', 'buoy:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản phao tiêu đã được xóa", null));
    }

    @GetMapping("/{id}/history")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:history', 'buoy:history')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:update', 'buoyasset:create', 'buoy:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'buoyasset:manage', 'buoyasset:update', 'buoy:update')")
