...
import com.hanghai.kchtg.assetmovement.service.StormShelterAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/storm-shelter-assets")
@DataScope
public class StormShelterAssetController {

    private final StormShelterAssetService service;

    public StormShelterAssetController(StormShelterAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:create', 'stormshelter:create', 'stormshelter:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản khu tránh bão đã được tạo", response));
    }

    @GetMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:read', 'stormshelter:read', 'data:read')")
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                stormShelterId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:update', 'stormshelter:update', 'stormshelter:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu tránh bão đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:delete', 'stormshelter:delete', 'stormshelter:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu tránh bão đã được xóa", null));
    }

    @GetMapping("/{id}/history")
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:history', 'stormshelter:history')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:update', 'stormshelterasset:create', 'stormshelter:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'stormshelterasset:manage', 'stormshelterasset:update', 'stormshelter:update')")
