...
import com.hanghai.kchtg.assetmovement.service.TtxlttAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/ttxltt-assets")
@DataScope
public class TtxlttAssetController {

    private final TtxlttAssetService service;

    public TtxlttAssetController(TtxlttAssetService service) {
    ...
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản đài TTXLTT đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
            ...
            @RequestParam(required = false) UUID ttxlttId,
    ...
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài TTXLTT đã được cập nhật", response));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài TTXLTT đã được xóa", null));
    }

    @GetMapping("/{id}/history")
    @PreAuthorize("@auth.checkAny(authentication, 'ttxlttasset:manage', 'ttxlttasset:history', 'ttxltt:history')")
