...
import com.hanghai.kchtg.assetmovement.service.PortTerminalAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/berth-assets")
@DataScope
public class PortTerminalAssetController {

    private final PortTerminalAssetService service;

    public PortTerminalAssetController(PortTerminalAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:create', 'berth:create', 'berth:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản bến cảng đã được tạo", response));
    }

    @GetMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:read', 'berth:read', 'data:read')")
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                berthId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:update', 'berth:update', 'berth:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản bến cảng đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:delete', 'berth:delete', 'berth:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản bến cảng đã được xóa", null));
    }

    @GetMapping("/{id}/history")
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:history', 'portterminalasset:history', 'berth:history')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:update', 'berthasset:create', 'berth:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'berthasset:manage', 'berthasset:update', 'berth:update')")
