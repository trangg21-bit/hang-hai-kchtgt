...
import com.hanghai.kchtg.assetmovement.service.CospasSarsatAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/cospas-sarsat-assets")
@DataScope
public class CospasSarsatAssetController {

    private final CospasSarsatAssetService service;

    public CospasSarsatAssetController(CospasSarsatAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'cospassarsatasset:manage', 'cospassarsatasset:create', 'cospassarsat:create', 'cospassarsat:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản đài Cospas-Sarsat đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
            ...
            @RequestParam(required = false) UUID cospasSarsatId,
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                cospasSarsatId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'cospassarsatasset:manage', 'cospassarsatasset:update', 'cospassarsat:update', 'cospassarsat:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài Cospas-Sarsat đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'cospassarsatasset:manage', 'cospassarsatasset:delete', 'cospassarsat:delete', 'cospassarsat:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đài Cospas-Sarsat đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'cospassarsatasset:manage', 'cospassarsatasset:update', 'cospassarsatasset:create', 'cospassarsat:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'cospassarsatasset:manage', 'cospassarsatasset:update', 'cospassarsat:update')")
