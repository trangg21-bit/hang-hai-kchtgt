...
import com.hanghai.kchtg.assetmovement.service.AnchorageAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/anchorage-assets")
@DataScope
public class AnchorageAssetController {

    private final AnchorageAssetService service;

    public AnchorageAssetController(AnchorageAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'anchorageasset:manage', 'anchorageasset:create', 'anchorage:create', 'anchorage:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản khu nước, vùng nước đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                anchorageId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'anchorageasset:manage', 'anchorageasset:update', 'anchorage:update', 'anchorage:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu nước, vùng nước đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'anchorageasset:manage', 'anchorageasset:delete', 'anchorage:delete', 'anchorage:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu nước, vùng nước đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'anchorageasset:manage', 'anchorageasset:update', 'anchorageasset:create', 'anchorage:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'anchorageasset:manage', 'anchorageasset:update', 'anchorage:update')")
