...
import com.hanghai.kchtg.assetmovement.service.BuoyBerthAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/buoy-berth-assets")
@DataScope
public class BuoyBerthAssetController {

    private final BuoyBerthAssetService service;

    public BuoyBerthAssetController(BuoyBerthAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'buoyberthasset:manage', 'buoyberthasset:create', 'buoyberth:create', 'buoyberth:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản bến phao đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                buoyBerthId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyberthasset:manage', 'buoyberthasset:update', 'buoyberth:update', 'buoyberth:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản bến phao đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'buoyberthasset:manage', 'buoyberthasset:delete', 'buoyberth:delete', 'buoyberth:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản bến phao đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'buoyberthasset:manage', 'buoyberthasset:update', 'buoyberthasset:create', 'buoyberth:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'buoyberthasset:manage', 'buoyberthasset:update', 'buoyberth:update')")
