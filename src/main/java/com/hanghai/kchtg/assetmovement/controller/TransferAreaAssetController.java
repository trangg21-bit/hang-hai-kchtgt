...
import com.hanghai.kchtg.assetmovement.service.TransferAreaAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/transfer-area-assets")
@DataScope
public class TransferAreaAssetController {

    private final TransferAreaAssetService service;

    public TransferAreaAssetController(TransferAreaAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'transferareaasset:manage', 'transferareaasset:create', 'transferarea:create', 'transferarea:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản khu chuyển tải đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                transferAreaId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'transferareaasset:manage', 'transferareaasset:update', 'transferarea:update', 'transferarea:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu chuyển tải đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'transferareaasset:manage', 'transferareaasset:delete', 'transferarea:delete', 'transferarea:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản khu chuyển tải đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'transferareaasset:manage', 'transferareaasset:update', 'transferareaasset:create', 'transferarea:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'transferareaasset:manage', 'transferareaasset:update', 'transferarea:update')")
