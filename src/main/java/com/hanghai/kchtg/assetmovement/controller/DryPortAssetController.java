...
import com.hanghai.kchtg.assetmovement.service.DryPortAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/dry-port-assets")
@DataScope
public class DryPortAssetController {

    private final DryPortAssetService service;

    public DryPortAssetController(DryPortAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'dryportasset:manage', 'dryportasset:create', 'dryport:create', 'dryport:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản cảng cạn đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                dryPortId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'dryportasset:manage', 'dryportasset:update', 'dryport:update', 'dryport:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản cảng cạn đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'dryportasset:manage', 'dryportasset:delete', 'dryport:delete', 'dryport:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản cảng cạn đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'dryportasset:manage', 'dryportasset:update', 'dryportasset:create', 'dryport:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'dryportasset:manage', 'dryportasset:update', 'dryport:update')")
