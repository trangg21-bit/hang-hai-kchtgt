...
import com.hanghai.kchtg.assetmovement.service.ChannelAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/channel-assets")
@DataScope
public class ChannelAssetController {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'channelasset:manage', 'channelasset:create', 'channel:create', 'channel:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản luồng hàng hải đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                channelId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'channelasset:manage', 'channelasset:update', 'channel:update', 'channel:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản luồng hàng hải đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'channelasset:manage', 'channelasset:delete', 'channel:delete', 'channel:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản luồng hàng hải đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'channelasset:manage', 'channelasset:update', 'channelasset:create', 'channel:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'channelasset:manage', 'channelasset:update', 'channel:update')")
