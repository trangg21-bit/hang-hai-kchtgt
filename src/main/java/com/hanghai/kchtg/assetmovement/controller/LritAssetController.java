...
import com.hanghai.kchtg.assetmovement.service.LritAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/lrit-assets")
@DataScope
public class LritAssetController {

    private final LritAssetService service;

    public LritAssetController(LritAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'lritasset:manage', 'lritasset:create', 'lrit:create', 'lrit:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản trung tâm LRIT đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
            ...
            @RequestParam(required = false) UUID lritId,
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                lritId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'lritasset:manage', 'lritasset:update', 'lrit:update', 'lrit:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản trung tâm LRIT đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'lritasset:manage', 'lritasset:delete', 'lrit:delete', 'lrit:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản trung tâm LRIT đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'lritasset:manage', 'lritasset:update', 'lritasset:create', 'lrit:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'lritasset:manage', 'lritasset:update', 'lrit:update')")
