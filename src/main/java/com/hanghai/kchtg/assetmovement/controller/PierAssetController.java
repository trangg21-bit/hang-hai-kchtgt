...
import com.hanghai.kchtg.assetmovement.service.PierAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/pier-assets")
@DataScope
public class PierAssetController {

    private final PierAssetService service;

    public PierAssetController(PierAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:create', 'pier:create', 'pier:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản cầu cảng đã được tạo", response));
    }

    @GetMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:read', 'pier:read', 'data:read')")
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                pierId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:update', 'pier:update', 'pier:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản cầu cảng đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:delete', 'pier:delete', 'pier:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản cầu cảng đã được xóa", null));
    }

    @GetMapping("/{id}/history")
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:history', 'pier:history')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:update', 'pierasset:create', 'pier:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'pierasset:manage', 'pierasset:update', 'pier:update')")
