...
import com.hanghai.kchtg.assetmovement.service.DikeRevetmentAssetService;
...
@RestController
@RequestMapping("/api/v1/asset/dike-revetment-assets")
@DataScope
public class DikeRevetmentAssetController {

    private final DikeRevetmentAssetService service;

    public DikeRevetmentAssetController(DikeRevetmentAssetService service) {
    ...
    @PostMapping
    @PreAuthorize("@auth.checkAny(authentication, 'dikerevetmentasset:manage', 'dikerevetmentasset:create', 'dikerevetment:create', 'dikerevetment:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> create(
        ...
        return ResponseEntity.status(201).body(ApiResponse.success("Tài sản đê kè đã được tạo", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<InfraAssetResponse>>> findAll(
        ...
        Page<InfraAssetResponse> result = service.findAll(
                ...
                dikeRevetmentId, types, assetCondition,
                approvalStatus, updatedFrom, updatedTo, pageable);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'dikerevetmentasset:manage', 'dikerevetmentasset:update', 'dikerevetment:update', 'dikerevetment:manage')")
    public ResponseEntity<ApiResponse<InfraAssetResponse>> update(
            @PathVariable UUID id,
            @RequestBody InfraAssetRequest request) {
        InfraAssetResponse response = service.update(id, request);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đê kè đã được cập nhật", response));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.checkAny(authentication, 'dikerevetmentasset:manage', 'dikerevetmentasset:delete', 'dikerevetment:delete', 'dikerevetment:manage')")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok(ApiResponse.success("Tài sản đê kè đã được xóa", null));
    }
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'dikerevetmentasset:manage', 'dikerevetmentasset:update', 'dikerevetmentasset:create', 'dikerevetment:update')")
    ...
    @PreAuthorize("@auth.checkAny(authentication, 'dikerevetmentasset:manage', 'dikerevetmentasset:update', 'dikerevetment:update')")
