...
public interface GroupMemberRepository extends JpaRepository<GroupMember, UUID> {
      ...
      /**
       * Tim membership cua mot nguoi dung trong mot nhom cu the (khong phan biet
       * status).
       */
      Optional<GroupMember> findByUserIdAndUserGroupId(UUID userId, UUID groupId);
       ...
       */
      @Query(value = "SELECT DISTINCT gm FROM GroupMember gm "
                  + "LEFT JOIN FETCH gm.user u "
                  + "WHERE gm.userGroup.id = :groupId AND gm.status = :status", countQuery = "SELECT COUNT(gm) FROM GroupMember gm WHERE gm.userGroup.id = :groupId AND gm.status = :status")
      Page<GroupMember> findByGroupIdWithUser(@Param("groupId") UUID groupId,
                  @Param("status") GroupMemberStatus status,
                  Pageable pageable);

      /**
       * Liet ke thanh vien nhom kem tim kiem va loc (phan trang)
       */
