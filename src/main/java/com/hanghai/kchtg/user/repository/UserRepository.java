...
public interface UserRepository extends JpaRepository<User, UUID> {
        ...
        /**
         * Kiểm tra email đang được một tài khoản chưa xóa sử dụng, không phân biệt
         * hoa/thường.
         */
        ...
        /**
         * Tìm danh sách người dùng theo danh sách ID kèm thông tin OrgUnit (tránh N+1
         * query khi hiển thị lịch sử).
         */
        @Query("SELECT u FROM User u LEFT JOIN FETCH u.orgUnit WHERE u.id IN :ids")
        List<User> findAllByIdInWithOrgUnit(
        ...
        @Query(value = "SELECT u.id AS id, u.username AS username, u.email AS email, u.fullName AS fullName, " +
                        "u.phone AS phone, u.address AS address, u.department AS department, u.position AS position, u.note AS note, "
                        +
