-- V20260804210000: Force all condition_status, approval_status, operational_status columns across all infrastructure tables to SMALLINT
...
DECLARE
    ...
    tbls TEXT[] := ARRAY[
        ...
        'berth', 'pier', 'ports', 'dry_ports', 'water_zones', 'piers'
    ];
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            WHERE table_name = tbl_name AND column_name = 'approval_status' 
        ...
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = tbl_name AND column_name = 'condition_status' 
              AND data_type NOT IN ('smallint', 'integer')
        ) THEN
        ...
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = tbl_name AND column_name = 'operational_status' 
              AND data_type NOT IN ('smallint', 'integer')
        ) THEN
