-- ============================================================================
-- Migration: V20260916190000__ensure_user_read_for_all_active_users.sql
-- Description: Đảm bảo quyền user:read (Xem hồ sơ cá nhân / người dùng) cho tất cả tài khoản người dùng đang hoạt động
-- ============================================================================
...
BEGIN
    ...
    IF to_regclass('public.user_permission_override') IS NOT NULL AND to_regclass('public.app_users') IS NOT NULL THEN
        ...
        SET deleted_at = NULL,
            deleted_by = NULL,
            reason = 'Đảm bảo quyền đọc danh sách/profile người dùng (user:read)',
        ...
        SELECT gen_random_uuid(), u.id, 'user:read', 'Đảm bảo quyền đọc danh sách/profile người dùng (user:read)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, u.id, u.id
        FROM app_users u
