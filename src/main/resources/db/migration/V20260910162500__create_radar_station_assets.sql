-- V20260910162500__create_radar_station_assets.sql
-- Tạo bảng lưu trữ Tài sản trạm radar và các chỉ số nghiệp vụ

CREATE TABLE IF NOT EXISTS radar_station_assets (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_radar_station_assets_code
    ON radar_station_assets (asset_code)
    WHERE is_deleted = FALSE;

CREATE INDEX IF NOT EXISTS idx_radar_station_assets_org_unit ON radar_station_assets(org_unit_id);
CREATE INDEX IF NOT EXISTS idx_radar_station_assets_using_org ON radar_station_assets(using_org_unit_id);
CREATE INDEX IF NOT EXISTS idx_radar_station_assets_radar_station ON radar_station_assets(radar_station_id);
CREATE INDEX IF NOT EXISTS idx_radar_station_assets_status ON radar_station_assets(approval_status);
CREATE INDEX IF NOT EXISTS idx_radar_station_assets_updated_at ON radar_station_assets(updated_at DESC);
