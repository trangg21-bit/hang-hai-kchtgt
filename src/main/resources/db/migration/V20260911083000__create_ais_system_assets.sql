-- V20260911083000__create_ais_system_assets.sql
-- Tạo bảng lưu trữ Tài sản hệ thống AIS và các chỉ số nghiệp vụ

CREATE TABLE IF NOT EXISTS ais_system_assets (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_ais_system_assets_code
    ON ais_system_assets (asset_code)
    WHERE is_deleted = FALSE;

CREATE INDEX IF NOT EXISTS idx_ais_system_assets_org_unit ON ais_system_assets(org_unit_id);
CREATE INDEX IF NOT EXISTS idx_ais_system_assets_using_org ON ais_system_assets(using_org_unit_id);
CREATE INDEX IF NOT EXISTS idx_ais_system_assets_ais_system ON ais_system_assets(ais_system_id);
CREATE INDEX IF NOT EXISTS idx_ais_system_assets_status ON ais_system_assets(approval_status);
CREATE INDEX IF NOT EXISTS idx_ais_system_assets_updated_at ON ais_system_assets(updated_at DESC);
